...
static char *dcc_new_app_poll_cog_source_path(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_poll_file_contains(const char *path, const char *needle) {
    size_t len = 0U;
...
static int dcc_new_app_poll_insert_route(
    const char *path,
    const char *route,
    ...
    int has_route_marker = dcc_new_app_poll_file_contains(path, route_marker);
...
static int dcc_new_app_poll_append_command_json(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_poll_insert_handlers(const char *path, const dcc_new_app_options_t *options) {
    char handler[320];
    ...
    snprintf(yes_key, sizeof(yes_key), "%s.%s.yes", options->cog_name, options->command_name);
    snprintf(no_key, sizeof(no_key), "%s.%s.no", options->cog_name, options->command_name);
    ...
    snprintf(
        ...
        "\n"
        "    uint64_t yes = DCC_CTX_STORE_U64_OR(ctx, \"%s\", 0U);\n"
        "    uint64_t no = DCC_CTX_STORE_U64_OR(ctx, \"%s\", 0U);\n"
        "    char text[192];\n"
        "    snprintf(text, sizeof(text), \"## Poll\\nYes: %%llu\\nNo: %%llu\", (unsigned long long)yes, (unsigned long long)no);\n"
        "    dcc_message_builder_t message = DCC_MESSAGE_COMPONENTS_V2(\n"
        "        DCC_UI_CARD_ACCENT(\n"
        "            0x5865F2,\n"
        "            DCC_UI_TEXT(text),\n"
        "            DCC_UI_ROW(\n"
        "                DCC_UI_SUCCESS(\"Yes\", \"%s.%s.yes\"),\n"
        "                DCC_UI_DANGER(\"No\", \"%s.%s.no\"),\n"
        ...
        "        ++yes;\n"
        "    } else if (dcc_sugar_ctx_text_eq(action, \"no\")) {\n"
        "        ++no;\n"
        "    } else if (dcc_sugar_ctx_text_eq(action, \"reset\")) {\n"
        "        yes = 0U;\n"
        "        no = 0U;\n"
        "    } else {\n"
        "        (void)DCC_CTX_BAD_INPUT(ctx, \"Unknown poll action.\");\n"
        "        return;\n"
        "    }\n"
        "\n"
        "    DCC_CTX_TRY(ctx, DCC_CTX_STORE_SET_U64(ctx, \"%s\", yes));\n"
        "    DCC_CTX_TRY(ctx, DCC_CTX_STORE_SET_U64(ctx, \"%s\", no));\n"
        "    %s_render(ctx, 1U);\n"
        "}\n"
        "\n",
        handler,
...
static int dcc_new_app_poll_insert_routes(const char *path, const dcc_new_app_options_t *options) {
    char policy[1000];
    ...
    snprintf(
        slash_route,
        sizeof(slash_route),
        "        %s(\"%s\", \"Generated poll preset\", %s, user_data%s%s%s)\n",
    ...
    snprintf(slash_needle, sizeof(slash_needle), "\"%s\", \"Generated poll preset\"", options->command_name);
    int status = dcc_new_app_poll_insert_route(
    ...
    return dcc_new_app_poll_insert_route(
...
int dcc_new_app_generate_poll_preset(const dcc_new_app_options_t *options) {
    char *path = dcc_new_app_poll_cog_source_path(options);
    if (path == NULL) {
        return -1;
    }
    ...
    int status = dcc_new_app_poll_insert_handlers(path, options);
    if (status == 0) {
        status = dcc_new_app_poll_insert_routes(path, options);
    }
    free(path);
    if (status == 0) {
        status = dcc_new_app_poll_append_command_json(options);
    }
    if (status != 0) {
        return -1;
    }
    printf("created DCC poll preset %s in feature %s under %s\n", options->command_name, options->cog_name, options->path);
    return 0;
}
