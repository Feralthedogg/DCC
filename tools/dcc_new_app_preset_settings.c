...
static char *dcc_new_app_settings_cog_source_path(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_settings_file_contains(const char *path, const char *needle) {
    size_t len = 0U;
...
static int dcc_new_app_settings_insert_route(
    const char *path,
    const char *route,
    ...
    int has_route_marker = dcc_new_app_settings_file_contains(path, route_marker);
...
static int dcc_new_app_settings_append_command_json(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_settings_insert_handlers(const char *path, const dcc_new_app_options_t *options) {
    char handler[320];
    ...
    snprintf(
        ...
        "\n"
        "#define %s(PARAM, type) \\\n"
        "    PARAM(type, STRING, name, \"%s.%s.name\", \"settings\") \\\n"
        "    PARAM(type, BOOL, enabled, \"%s.%s.enabled\", 1U)\n"
        "\n"
        "DCC_HANDLER(%s) {\n"
        "    (void)user_data;\n"
        "\n"
        ...
        "            \"%s.%s.edit\",\n"
        "            \"Edit Settings\",\n"
        "            DCC_MODAL_V2_FIELD_TEXT_PLACEHOLDER(\"%s.%s.name\", \"Name\", \"Setting name\"),\n"
        "            DCC_MODAL_V2_FIELD_CHECKBOX(\"%s.%s.enabled\", \"Enabled\", 1U)\n"
        "        );\n"
        "        return;\n"
        "    }\n"
        "    if (dcc_sugar_ctx_text_eq(action, \"enable\") || dcc_sugar_ctx_text_eq(action, \"reset\")) {\n"
        "        (void)DCC_UPDATE(ctx, DCC_SETTINGS_PANEL_V2(\"## Generated settings\\nEnabled.\", \"%s.%s.edit\", \"%s.%s.enable\", \"%s.%s.disable\", \"%s.%s.reset\", 1U));\n"
        "        return;\n"
        "    }\n"
        "    if (dcc_sugar_ctx_text_eq(action, \"disable\")) {\n"
        "        (void)DCC_UPDATE(ctx, DCC_SETTINGS_PANEL_V2(\"## Generated settings\\nDisabled.\", \"%s.%s.edit\", \"%s.%s.enable\", \"%s.%s.disable\", \"%s.%s.reset\", 0U));\n"
        "        return;\n"
        "    }\n"
        "    (void)DCC_CTX_BAD_INPUT(ctx, \"Unknown settings action.\");\n"
        "}\n"
        "\n"
        "DCC_MODAL_ARGS_IMPL(%s, %s, form) {\n"
        "    (void)user_data;\n"
        "\n"
        "    const char *name = form != NULL && form->name != NULL ? form->name : \"settings\";\n"
        "    uint8_t enabled = form != NULL ? form->enabled : 1U;\n"
        "    (void)DCC_UPDATE(ctx, DCC_SETTINGS_PANEL_V2(\"## Generated settings\\nSaved.\", \"%s.%s.edit\", \"%s.%s.enable\", \"%s.%s.disable\", \"%s.%s.reset\", enabled));\n"
        "    (void)name;\n"
        "}\n"
        "\n",
        modal_handler,
...
static int dcc_new_app_settings_insert_routes(const char *path, const dcc_new_app_options_t *options) {
    char policy[1000];
    ...
    snprintf(
        slash_route,
        sizeof(slash_route),
        "        %s(\"%s\", \"Generated settings preset\", %s, user_data%s%s%s)\n",
    ...
    snprintf(slash_needle, sizeof(slash_needle), "\"%s\", \"Generated settings preset\"", options->command_name);
    int status = dcc_new_app_settings_insert_route(
    ...
    status = dcc_new_app_settings_insert_route(
    ...
    snprintf(
        modal_route,
        sizeof(modal_route),
        "        %s(\"%s.%s.edit\", %s, %s, user_data, %s%s%s)\n",
    ...
    snprintf(modal_needle, sizeof(modal_needle), "\"%s.%s.edit\", %s", options->cog_name, options->command_name, modal_args);
    return dcc_new_app_settings_insert_route(
...
int dcc_new_app_generate_settings_preset(const dcc_new_app_options_t *options) {
    char *path = dcc_new_app_settings_cog_source_path(options);
    if (path == NULL) {
        return -1;
    }
    ...
    int status = dcc_new_app_settings_insert_handlers(path, options);
    if (status == 0) {
        status = dcc_new_app_settings_insert_routes(path, options);
    }
    free(path);
    if (status == 0) {
        status = dcc_new_app_settings_append_command_json(options);
    }
    if (status != 0) {
        return -1;
    }
    printf("created DCC settings preset %s in feature %s under %s\n", options->command_name, options->cog_name, options->path);
    return 0;
}
