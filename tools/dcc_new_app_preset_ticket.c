...
static char *dcc_new_app_ticket_cog_source_path(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_ticket_file_contains(const char *path, const char *needle) {
    size_t len = 0U;
...
static int dcc_new_app_ticket_insert_route(
    const char *path,
    const char *route,
    ...
    int has_route_marker = dcc_new_app_ticket_file_contains(path, route_marker);
...
static int dcc_new_app_ticket_append_command_json(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_ticket_insert_handlers(const char *path, const dcc_new_app_options_t *options) {
    char handler[320];
    ...
    snprintf(count_key, sizeof(count_key), "%s.%s.ticket_count", options->cog_name, options->command_name);
    ...
    snprintf(
        ...
        "\n"
        "#define %s(PARAM, type) \\\n"
        "    PARAM(type, REQUIRED_STRING, subject, \"%s.%s.subject\") \\\n"
        "    PARAM(type, STRING, details, \"%s.%s.details\", \"\")\n"
        "\n"
        "DCC_HANDLER(%s) {\n"
        "    (void)user_data;\n"
        "\n"
        ...
        "            DCC_UI_CARD_ACCENT(\n"
        "                0x5865F2,\n"
        "                DCC_UI_TEXT(\"## Support ticket\\nOpen a ticket without wiring modal routes by hand.\"),\n"
        "                DCC_UI_ROW(DCC_UI_PRIMARY(\"Open ticket\", \"%s.%s.open\"))\n"
        "            )\n"
        "        )\n"
        "    );\n"
        "}\n"
        ...
        "    if (dcc_sugar_ctx_text_eq(action, \"open\")) {\n"
        "        (void)DCC_SHOW_MODAL_V2(\n"
        "            ctx,\n"
        "            \"%s.%s.create\",\n"
        "            \"Open Ticket\",\n"
        "            DCC_MODAL_V2_FIELD_TEXT_PLACEHOLDER(\"%s.%s.subject\", \"Subject\", \"What do you need help with?\"),\n"
        "            DCC_MODAL_V2_FIELD_PARAGRAPH_OPTIONAL_PLACEHOLDER(\"%s.%s.details\", \"Details\", \"Add context for the team\")\n"
        "        );\n"
        "        return;\n"
        "    }\n"
        "    if (dcc_sugar_ctx_text_eq(action, \"close\")) {\n"
        "        (void)DCC_UPDATE(\n"
        "            ctx,\n"
        "            DCC_MESSAGE_COMPONENTS_V2(\n"
        "                DCC_UI_CARD_ACCENT(0x57F287, DCC_UI_TEXT(\"## Ticket closed\\nThe generated ticket flow is complete.\"))\n"
        "            )\n"
        "        );\n"
        "        return;\n"
        "    }\n"
        "    (void)DCC_CTX_BAD_INPUT(ctx, \"Unknown ticket action.\");\n"
        "}\n"
        "\n"
        "DCC_MODAL_ARGS_IMPL(%s, %s, form) {\n"
        "    (void)user_data;\n"
        "\n"
        "    const char *subject = form != NULL && form->subject != NULL ? form->subject : \"\";\n"
        "    const char *details = form != NULL && form->details != NULL ? form->details : \"\";\n"
        "    uint64_t ticket_id = DCC_CTX_STORE_U64_OR(ctx, \"%s\", 0U) + 1U;\n"
        "    if (DCC_CTX_STORE(ctx) != NULL) {\n"
        "        DCC_CTX_TRY(ctx, DCC_CTX_STORE_SET_U64(ctx, \"%s\", ticket_id));\n"
        "    } else {\n"
        "        ticket_id = 1U;\n"
        "    }\n"
        "\n"
        ...
        "        (unsigned long long)DCC_CTX_USER_ID(ctx),\n"
        "        subject,\n"
        "        details[0] != '\\0' ? details : \"-\"\n"
        "    );\n"
        "    (void)DCC_REPLY_EPHEMERAL_V2(\n"
        "        ctx,\n"
        "        DCC_UI_CARD_ACCENT(\n"
        "            0x5865F2,\n"
        "            DCC_UI_TEXT(text),\n"
        "            DCC_UI_ROW(DCC_UI_DANGER(\"Close\", \"%s.%s.close\"))\n"
        "        )\n"
        "    );\n"
        "}\n"
        "\n",
...
static int dcc_new_app_ticket_insert_routes(const char *path, const dcc_new_app_options_t *options) {
    char policy[1000];
    ...
    snprintf(
        slash_route,
        sizeof(slash_route),
        "        %s(\"%s\", \"Generated ticket preset\", %s, user_data%s%s%s)\n",
    ...
    snprintf(slash_needle, sizeof(slash_needle), "\"%s\", \"Generated ticket preset\"", options->command_name);
    int status = dcc_new_app_ticket_insert_route(
    ...
    status = dcc_new_app_ticket_insert_route(
    ...
    return dcc_new_app_ticket_insert_route(
...
int dcc_new_app_generate_ticket_preset(const dcc_new_app_options_t *options) {
    char *path = dcc_new_app_ticket_cog_source_path(options);
    if (path == NULL) {
        return -1;
    }
    ...
    int status = dcc_new_app_ticket_insert_handlers(path, options);
    if (status == 0) {
        status = dcc_new_app_ticket_insert_routes(path, options);
    }
    free(path);
    if (status == 0) {
        status = dcc_new_app_ticket_append_command_json(options);
    }
    if (status != 0) {
        return -1;
    }
    printf("created DCC ticket preset %s in feature %s under %s\n", options->command_name, options->cog_name, options->path);
    return 0;
}
