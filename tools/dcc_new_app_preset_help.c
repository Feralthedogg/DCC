...
static char *dcc_new_app_help_cog_source_path(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_help_file_contains(const char *path, const char *needle) {
    size_t len = 0U;
...
static int dcc_new_app_help_insert_route(
    const char *path,
    const char *route,
    ...
    int has_route_marker = dcc_new_app_help_file_contains(path, route_marker);
...
static int dcc_new_app_help_append_command_json(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_help_insert_handlers(const char *path, const dcc_new_app_options_t *options) {
    char handler[320];
    ...
    snprintf(
        insertion,
        sizeof(insertion),
        "static const char *%s_page_text(const char *page) {\n"
        "    if (dcc_sugar_ctx_text_eq(page, \"components\")) {\n"
        "        return \"## Help: Components\\nButtons, selects, modals, and views are routed through feature component handlers.\";\n"
        "    }\n"
        "    if (dcc_sugar_ctx_text_eq(page, \"about\")) {\n"
        "        return \"## Help: About\\nThis generated help center is ready for your bot-specific docs.\";\n"
        "    }\n"
        "    return \"## Help: Commands\\nList your slash commands, message commands, and admin flows here.\";\n"
        "}\n"
        "\n"
        "static void %s_render(dcc_ctx_t *ctx, const char *page, uint8_t update) {\n"
        "    dcc_message_builder_t message = DCC_MESSAGE_COMPONENTS_V2(\n"
        "        DCC_UI_CARD_ACCENT(\n"
        "            0x5865F2,\n"
        "            DCC_UI_TEXT(%s_page_text(page)),\n"
        "            DCC_UI_ROW(\n"
        "                DCC_UI_PRIMARY(\"Commands\", \"%s.%s.commands\"),\n"
        "                DCC_UI_SECONDARY(\"Components\", \"%s.%s.components\"),\n"
        "                DCC_UI_SECONDARY(\"About\", \"%s.%s.about\")\n"
        "            )\n"
        "        )\n"
        "    );\n"
        "    if (update) {\n"
        ...
        "DCC_HANDLER(%s) {\n"
        "    (void)user_data;\n"
        "    %s_render(ctx, \"commands\", 0U);\n"
        "}\n"
        "\n"
        "DCC_HANDLER(%s) {\n"
        "    (void)user_data;\n"
        "\n"
        "    const char *page = DCC_CTX_ACTION(ctx, \"%s.%s.\");\n"
        "    if (dcc_sugar_ctx_text_eq(page, \"commands\") ||\n"
        "        dcc_sugar_ctx_text_eq(page, \"components\") ||\n"
        "        dcc_sugar_ctx_text_eq(page, \"about\")) {\n"
        "        %s_render(ctx, page, 1U);\n"
        "        return;\n"
        "    }\n"
        "    (void)DCC_CTX_BAD_INPUT(ctx, \"Unknown help page.\");\n"
        "}\n"
        "\n",
        handler,
        handler,
...
static int dcc_new_app_help_insert_routes(const char *path, const dcc_new_app_options_t *options) {
    char policy[1000];
    ...
    snprintf(
        slash_route,
        sizeof(slash_route),
        "        %s(\"%s\", \"Generated help preset\", %s, user_data%s%s%s)\n",
    ...
    snprintf(slash_needle, sizeof(slash_needle), "\"%s\", \"Generated help preset\"", options->command_name);
    int status = dcc_new_app_help_insert_route(
    ...
    return dcc_new_app_help_insert_route(
...
int dcc_new_app_generate_help_preset(const dcc_new_app_options_t *options) {
    char *path = dcc_new_app_help_cog_source_path(options);
    if (path == NULL) {
        return -1;
    }
    ...
    int status = dcc_new_app_help_insert_handlers(path, options);
    if (status == 0) {
        status = dcc_new_app_help_insert_routes(path, options);
    }
    free(path);
    if (status == 0) {
        status = dcc_new_app_help_append_command_json(options);
    }
    if (status != 0) {
        return -1;
    }
    printf("created DCC help preset %s in feature %s under %s\n", options->command_name, options->cog_name, options->path);
    return 0;
}
