...
static char *dcc_new_app_wizard_cog_source_path(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_wizard_file_contains(const char *path, const char *needle) {
    size_t len = 0U;
...
static int dcc_new_app_wizard_insert_route(
    const char *path,
    const char *route,
    ...
    int has_route_marker = dcc_new_app_wizard_file_contains(path, route_marker);
...
static int dcc_new_app_wizard_append_command_json(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_wizard_insert_handlers(const char *path, const dcc_new_app_options_t *options) {
    char handler[320];
    ...
    snprintf(
        ...
        "            DCC_V2_CONTAINER(\n"
        "                DCC_V2_TEXT(\"## Generated wizard\\nStart the setup flow.\"),\n"
        "                DCC_V2_ACTION_ROW(\n"
        "                    DCC_V2_BUTTON_PRIMARY(\"Start\", \"%s.%s.start\"),\n"
        "                    DCC_V2_BUTTON_SECONDARY(\"Cancel\", \"%s.%s.cancel\")\n"
        "                )\n"
        "            )\n"
        "        )\n"
        "    );\n"
        ...
        "    if (dcc_sugar_ctx_text_eq(action, \"start\")) {\n"
        "        (void)DCC_SHOW_MODAL_V2(\n"
        "            ctx,\n"
        "            \"%s.%s.step1\",\n"
        "            \"Wizard Step 1\",\n"
        "            DCC_MODAL_V2_FIELD_TEXT_PLACEHOLDER(\"%s.%s.name\", \"Name\", \"Enter a name\"),\n"
        "            DCC_MODAL_V2_FIELD_PARAGRAPH_OPTIONAL_PLACEHOLDER(\"%s.%s.note\", \"Note\", \"Optional note\")\n"
        "        );\n"
        "        return;\n"
        "    }\n"
        "    if (dcc_sugar_ctx_text_eq(action, \"finish\")) {\n"
        "        (void)DCC_UPDATE(ctx, DCC_MESSAGE_COMPONENTS_V2(DCC_V2_CONTAINER(DCC_V2_TEXT(\"## Generated wizard\\nFinished.\"), DCC_V2_ACTION_ROW(DCC_V2_BUTTON_SECONDARY(\"Start over\", \"%s.%s.start\")))));\n"
        "        return;\n"
        "    }\n"
        "    if (dcc_sugar_ctx_text_eq(action, \"cancel\")) {\n"
        "        (void)DCC_UPDATE(ctx, DCC_MESSAGE_TEXT(\"Wizard cancelled.\"));\n"
        "        return;\n"
        "    }\n"
        "    (void)DCC_CTX_BAD_INPUT(ctx, \"Unknown wizard action.\");\n"
        "}\n"
        "\n"
        ...
        "        DCC_MESSAGE_COMPONENTS_V2(\n"
        "            DCC_V2_CONTAINER(\n"
        "                DCC_V2_TEXT(\"## Review wizard input\\nUse Finish to accept or Edit to change it.\"),\n"
        "                DCC_V2_ACTION_ROW(\n"
        "                    DCC_V2_BUTTON_PRIMARY(\"Finish\", \"%s.%s.finish\"),\n"
        "                    DCC_V2_BUTTON_SECONDARY(\"Edit\", \"%s.%s.start\"),\n"
        "                    DCC_V2_BUTTON_DANGER(\"Cancel\", \"%s.%s.cancel\")\n"
        "                )\n"
        "            )\n"
        "        )\n"
        "    );\n"
...
static int dcc_new_app_wizard_insert_routes(const char *path, const dcc_new_app_options_t *options) {
    char policy[1000];
    ...
    snprintf(
        slash_route,
        sizeof(slash_route),
        "        %s(\"%s\", \"Generated wizard preset\", %s, user_data%s%s%s)\n",
    ...
    snprintf(slash_needle, sizeof(slash_needle), "\"%s\", \"Generated wizard preset\"", options->command_name);
    int status = dcc_new_app_wizard_insert_route(
    ...
    status = dcc_new_app_wizard_insert_route(
    ...
    snprintf(
        modal_route,
        sizeof(modal_route),
        "        %s(\"%s.%s.step1\", %s, %s, user_data, %s%s%s)\n",
    ...
    snprintf(modal_needle, sizeof(modal_needle), "\"%s.%s.step1\", %s", options->cog_name, options->command_name, modal_args);
    return dcc_new_app_wizard_insert_route(
...
int dcc_new_app_generate_wizard_preset(const dcc_new_app_options_t *options) {
    char *path = dcc_new_app_wizard_cog_source_path(options);
    if (path == NULL) {
        return -1;
    }
    ...
    int status = dcc_new_app_wizard_insert_handlers(path, options);
    if (status == 0) {
        status = dcc_new_app_wizard_insert_routes(path, options);
    }
    free(path);
    if (status == 0) {
        status = dcc_new_app_wizard_append_command_json(options);
    }
    if (status != 0) {
        return -1;
    }
    printf("created DCC wizard preset %s in feature %s under %s\n", options->command_name, options->cog_name, options->path);
    return 0;
}
