...
static char *dcc_new_app_profile_cog_source_path(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_profile_file_contains(const char *path, const char *needle) {
    size_t len = 0U;
...
static int dcc_new_app_profile_insert_route(
    const char *path,
    const char *route,
    ...
    int has_route_marker = dcc_new_app_profile_file_contains(path, route_marker);
...
static int dcc_new_app_profile_append_command_json(const dcc_new_app_options_t *options) {
...
static int dcc_new_app_profile_insert_handlers(const char *path, const dcc_new_app_options_t *options) {
    char handler[320];
    ...
    snprintf(
        ...
        "    const char *username = user != NULL && user->username != NULL ? user->username : \"unknown\";\n"
        "    const char *display = member != NULL && member->nick != NULL ? member->nick :\n"
        "        (user != NULL && user->global_name != NULL ? user->global_name : username);\n"
        "    const char *guild_name = guild != NULL && guild->name != NULL ? guild->name : \"direct message\";\n"
        "    const char *channel_name = channel != NULL && channel->name != NULL ? channel->name : \"unknown\";\n"
        "\n"
        "    char text[640];\n"
        "    snprintf(\n"
        "        text,\n"
        ...
        "            0x5865F2,\n"
        "            DCC_UI_TEXT(text),\n"
        "            DCC_UI_ROW(DCC_UI_SECONDARY(\"Refresh\", \"%s.%s.refresh\"))\n"
        "        )\n"
        "    );\n"
        "    if (update) {\n"
        ...
        "    if (dcc_sugar_ctx_text_eq(action, \"refresh\")) {\n"
        "        %s_render(ctx, 1U);\n"
        "        return;\n"
        "    }\n"
        "    (void)DCC_CTX_BAD_INPUT(ctx, \"Unknown profile action.\");\n"
        "}\n"
        "\n",
        handler,
        options->cog_name,
...
static int dcc_new_app_profile_insert_routes(const char *path, const dcc_new_app_options_t *options) {
    char policy[1000];
    ...
    snprintf(
        slash_route,
        sizeof(slash_route),
        "        %s(\"%s\", \"Generated profile preset\", %s, user_data%s%s%s)\n",
    ...
    snprintf(slash_needle, sizeof(slash_needle), "\"%s\", \"Generated profile preset\"", options->command_name);
    int status = dcc_new_app_profile_insert_route(
    ...
    return dcc_new_app_profile_insert_route(
...
int dcc_new_app_generate_profile_preset(const dcc_new_app_options_t *options) {
    char *path = dcc_new_app_profile_cog_source_path(options);
    if (path == NULL) {
        return -1;
    }
    ...
    int status = dcc_new_app_profile_insert_handlers(path, options);
    if (status == 0) {
        status = dcc_new_app_profile_insert_routes(path, options);
    }
    free(path);
    if (status == 0) {
        status = dcc_new_app_profile_append_command_json(options);
    }
    if (status != 0) {
        return -1;
    }
    printf("created DCC profile preset %s in feature %s under %s\n", options->command_name, options->cog_name, options->path);
    return 0;
}
