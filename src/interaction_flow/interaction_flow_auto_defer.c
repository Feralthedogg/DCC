...
dcc_status_t dcc_flow_auto_defer(dcc_interaction_flow_t *flow,
                                 uint64_t after_ms) {
  if (flow == NULL || after_ms == 0U ||
      after_ms > UINT64_MAX / UINT64_C(1000000)) {
...
dcc_status_t dcc_flow_auto_defer_ephemeral(dcc_interaction_flow_t *flow,
                                           uint64_t after_ms) {
  if (flow == NULL || after_ms == 0U ||
      after_ms > UINT64_MAX / UINT64_C(1000000)) {
    return DCC_ERR_INVALID_ARG;
...
dcc_status_t dcc_flow_maybe_auto_defer(dcc_interaction_flow_t *flow,
                                       uint64_t now_ms, dcc_rest_result_fn cb,
                                       void *user_data) {
  if (flow == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
