...
dcc_status_t dcc_flow_edit_original(dcc_interaction_flow_t *flow,
                                    const dcc_message_builder_t *message,
                                    dcc_rest_result_fn cb, void *user_data) {
  if (flow != NULL && flow->queue != NULL) {
    return dcc_app_interaction_queue_edit_original(flow, message, cb,
                                                   user_data);
  }
  dcc_status_t status = dcc_flow_require_context(flow);
  if (status == DCC_OK &&
      (message == NULL || !dcc_flow_can_edit_original(flow))) {
...
dcc_status_t dcc_flow_followup(dcc_interaction_flow_t *flow,
                               const dcc_message_builder_t *message,
                               dcc_rest_result_fn cb, void *user_data) {
  if (flow != NULL && flow->queue != NULL) {
    return dcc_app_interaction_queue_followup(flow, message, cb, user_data);
  }
  dcc_status_t status = dcc_flow_require_context(flow);
  ...
  if (status == DCC_OK) {
    ...
    options.user_data = user_data;
    if (status == DCC_OK)
      status = dcc_rest_interaction_followup_create(
          flow->client, flow->interaction->application_id,
          flow->interaction->token, &payload, &options, NULL);
  }
...
dcc_status_t dcc_flow_reply(dcc_interaction_flow_t *flow,
                            const dcc_message_builder_t *message,
                            dcc_rest_result_fn cb, void *user_data) {
  if (flow != NULL && flow->queue != NULL) {
    return dcc_app_interaction_queue_reply(flow, message, cb, user_data);
  }
  dcc_status_t status = dcc_flow_require_context(flow);
  ...
  if (!dcc_flow_initial_sent(flow)) {
    ...
    return dcc_flow_mark_initial(flow, DCC_INTERACTION_FLOW_REPLIED, status);
  }
