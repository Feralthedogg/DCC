...
dcc_status_t dcc_client_wait_for_event_sequence(
    dcc_client_t *client,
    ...
    dcc_status_t status = dcc_event_waiter_set_sequence(
        &waiter,
        types,
        type_count,
        out_snapshots,
    ...
    return dcc_event_waiter_policy_wait(
        ...
        &waiter,
        timeout_ms,
        DCC_EVENT_WAITER_POLICY_COUNT_SEQUENCE,
        out_completed
    );
}

dcc_status_t dcc_client_wait_for_event_set(
    dcc_client_t *client,
    const dcc_event_type_t *types,
    size_t type_count,
    uint32_t timeout_ms,
