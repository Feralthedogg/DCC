...
static dcc_status_t dcc_app_routes_reserve(dcc_app_t *app, size_t need) {
  ...
  dcc_app_route_t *next =
      (dcc_app_route_t *)realloc(app->routes, next_cap * sizeof(*app->routes));
  if (next == NULL) {
    return DCC_ERR_NOMEM;
  }
  app->routes = next;
...
static dcc_status_t dcc_app_middlewares_reserve(dcc_app_t *app, size_t need) {
  ...
  dcc_app_middleware_t *next = (dcc_app_middleware_t *)realloc(
      app->middlewares, next_cap * sizeof(*app->middlewares));
  if (next == NULL) {
    return DCC_ERR_NOMEM;
  }
...
static dcc_status_t dcc_app_route_middlewares_reserve(dcc_app_route_t *route,
                                                      size_t need) {
  ...
  dcc_app_middleware_t *next = (dcc_app_middleware_t *)realloc(
      route->middlewares, next_cap * sizeof(*route->middlewares));
  if (next == NULL) {
    return DCC_ERR_NOMEM;
  }
...
dcc_status_t dcc_app_use_internal(dcc_app_t *app,
  ...
  dcc_status_t status =
      dcc_app_middlewares_reserve(app, app->middleware_count + 1U);
  if (status != DCC_OK) {
    dcc_app_listener_unlock(app);
    return status;
  }
...
dcc_status_t dcc_app_use(dcc_app_t *app, dcc_app_middleware_fn middleware,
                         void *user_data) {
...
dcc_status_t dcc_app_add_route(dcc_app_t *app, dcc_event_type_t type,
  ...
  return dcc_app_add_route_with_cleanup(app, type, key, prefix, handler,
                                        user_data, NULL, out_route);
}
...
dcc_status_t dcc_app_remove_route_internal(dcc_app_t *app,
  ...
  if (index + 1U < app->route_count) {
    memmove(&app->routes[index], &app->routes[index + 1U],
...
static int dcc_app_route_matches(const dcc_app_route_t *route,
                                 dcc_event_type_t type, const char *value) {
  if (route == NULL || route->type != type || value == NULL ||
      route->key == NULL) {
    return 0;
  }
  if (!route->prefix) {
...
static dcc_app_route_t *
dcc_app_find_route(dcc_app_t *app, dcc_event_type_t type, const char *value) {
  if (app == NULL || value == NULL) {
    return NULL;
  }
  for (size_t i = 0; i < app->route_count; ++i) {
    if (!app->routes[i].prefix &&
        dcc_app_route_matches(&app->routes[i], type, value)) {
      return &app->routes[i];
    }
  }
  for (size_t i = 0; i < app->route_count; ++i) {
...
static dcc_app_route_t *dcc_app_find_route_by_id(dcc_app_t *app,
                                                 dcc_app_route_id_t route_id) {
  if (app == NULL || route_id == DCC_APP_ROUTE_INVALID) {
    return NULL;
  }
...
static int dcc_app_event_uses_interaction_name(dcc_event_type_t type) {
  return type == DCC_EVENT_SLASH_COMMAND || type == DCC_EVENT_AUTOCOMPLETE ||
         type == DCC_EVENT_USER_CONTEXT_MENU ||
         type == DCC_EVENT_MESSAGE_CONTEXT_MENU;
}

static const char *
dcc_app_interaction_operation(const dcc_event_t *event,
  ...
  return event != NULL ? dcc_event_type_name(dcc_event_type(event))
                       : "interaction handler";
}
...
dcc_status_t dcc_app_route_use_internal(dcc_app_t *app,
  ...
  dcc_status_t status =
      dcc_app_route_middlewares_reserve(route, route->middleware_count + 1U);
  if (status != DCC_OK) {
    dcc_app_listener_unlock(app);
    return status;
  }
...
dcc_status_t dcc_app_route_use(dcc_app_t *app, dcc_app_route_id_t route_id,
                               dcc_app_middleware_fn middleware,
                               void *user_data) {
...
static dcc_status_t
dcc_app_run_middlewares(dcc_ctx_t *ctx, const dcc_app_middleware_t *middlewares,
...
static dcc_status_t
dcc_app_snapshot_middlewares(dcc_app_t *app,
  ...
  *out_middleware_count = 0U;
  ...
  if (middleware_count != 0U) {
    middlewares =
        (dcc_app_middleware_t *)malloc(middleware_count * sizeof(*middlewares));
    if (middlewares == NULL) {
      dcc_app_listener_unlock(app);
      return DCC_ERR_NOMEM;
    }
    memcpy(middlewares, app->middlewares,
...
dcc_status_t dcc_app_dispatch_handler(
    ...
    void *user_data) {
  if (app == NULL || client == NULL || event == NULL || interaction == NULL ||
      handler == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
  dcc_ctx_t ctx = {
  ...
  dcc_status_t status = dcc_app_snapshot_middlewares(app, &app_middlewares,
                                                     &app_middleware_count);
  if (status != DCC_OK) {
    dcc_app_callback_frame_leave(&callback_frame);
    return status;
  }
  status = dcc_flow_create(client, interaction, &ctx.flow);
    if (status != DCC_OK) {
      free(app_middlewares);
      dcc_app_callback_frame_leave(&callback_frame);
      return status == DCC_ERR_STATE ? DCC_OK : status;
  }
  ...
  if (status == DCC_OK) {
    status = dcc_app_run_middlewares(&ctx, route_middlewares,
                                     route_middleware_count);
  }
  if (status != DCC_OK) {
    (void)dcc_app_report_handler_error(
        app, &ctx, status, dcc_app_interaction_operation(event, interaction),
...
void dcc_app_dispatch_event(dcc_client_t *client, const dcc_event_t *event,
  ...
  const char *key = dcc_app_event_uses_interaction_name(type)
                        ? interaction->name
  ...
  if ((type == DCC_EVENT_SLASH_COMMAND || type == DCC_EVENT_AUTOCOMPLETE) &&
      dcc_ctx_subcommand_path(&(dcc_ctx_t){.interaction = interaction},
                              subcommand_path,
                              sizeof(subcommand_path)) == DCC_OK &&
      dcc_app_subcommand_key(interaction->name, subcommand_path, subcommand_key,
                             sizeof(subcommand_key)) == DCC_OK) {
  ...
  if (route == NULL ||
      (route->handler == NULL && route->legacy_handler == NULL)) {
    dcc_app_listener_unlock(app);
    return;
  }
  ...
  if (route->middleware_count != 0U) {
    ...
    memcpy(route_middlewares, route->middlewares,
           route->middleware_count * sizeof(*route_middlewares));
  }
  ...
  if (route_snapshot.handler != NULL) {
    (void)dcc_app_dispatch_canonical_handler(
        ...
        route_snapshot.middleware_count, route_snapshot.handler,
        route_snapshot.user_data, route_snapshot.context_user_data);
  } else {
...
dcc_status_t
dcc_app_slash_builder(dcc_app_t *app,
...
dcc_status_t
dcc_app_slash_builder_route(dcc_app_t *app,
                            const dcc_application_command_builder_t *command,
                            dcc_app_legacy_handler_fn handler, void *user_data,
                            dcc_app_route_id_t *out_route) {
  ...
  dcc_status_t status =
      dcc_command_registry_add_builder(&app->registry, command);
  if (status != DCC_OK) {
    return status;
  }
  return dcc_app_add_route(app, DCC_EVENT_SLASH_COMMAND, command->name, 0U,
                           handler, user_data, out_route);
}

dcc_status_t dcc_app_slash(dcc_app_t *app, const char *name,
                           const char *description,
                           dcc_app_legacy_handler_fn handler, void *user_data) {
  return dcc_app_slash_route(app, name, description, handler, user_data, NULL);
}

dcc_status_t dcc_app_slash_route(dcc_app_t *app, const char *name,
                                 const char *description,
                                 dcc_app_legacy_handler_fn handler,
                                 void *user_data,
                                 dcc_app_route_id_t *out_route) {
  ...
  dcc_status_t status =
      dcc_application_command_builder_set_name(&command, name);
  if (status == DCC_OK) {
    status = dcc_application_command_builder_set_description(
        &command, description != NULL ? description : "");
  }
  if (status == DCC_OK) {
    status = dcc_application_command_builder_set_type(
        &command, DCC_APPLICATION_COMMAND_CHAT_INPUT);
  }
  return status == DCC_OK ? dcc_app_slash_builder_route(app, &command, handler,
                                                        user_data, out_route)
                          : status;
}

static dcc_status_t
dcc_app_context_menu_route(dcc_app_t *app, const char *name,
  ...
  return dcc_app_add_route(app, event_type, command.name, 0U, handler,
                           user_data, out_route);
}

dcc_status_t dcc_app_user_context_menu(dcc_app_t *app, const char *name,
                                       dcc_app_legacy_handler_fn handler,
                                       void *user_data) {
  return dcc_app_user_context_menu_route(app, name, handler, user_data, NULL);
}

dcc_status_t dcc_app_user_context_menu_route(dcc_app_t *app, const char *name,
                                             dcc_app_legacy_handler_fn handler,
                                             void *user_data,
                                             dcc_app_route_id_t *out_route) {
  return dcc_app_context_menu_route(app, name, DCC_APPLICATION_COMMAND_USER,
                                    DCC_EVENT_USER_CONTEXT_MENU, handler,
                                    user_data, out_route);
}

dcc_status_t dcc_app_message_context_menu(dcc_app_t *app, const char *name,
                                          dcc_app_legacy_handler_fn handler,
                                          void *user_data) {
  return dcc_app_message_context_menu_route(app, name, handler, user_data,
                                            NULL);
}

dcc_status_t dcc_app_message_context_menu_route(
    dcc_app_t *app, const char *name, dcc_app_legacy_handler_fn handler,
    void *user_data, dcc_app_route_id_t *out_route) {
  return dcc_app_context_menu_route(app, name, DCC_APPLICATION_COMMAND_MESSAGE,
                                    DCC_EVENT_MESSAGE_CONTEXT_MENU, handler,
                                    user_data, out_route);
}

dcc_status_t dcc_app_autocomplete(dcc_app_t *app, const char *name,
                                  dcc_app_legacy_handler_fn handler,
                                  void *user_data) {
  return dcc_app_autocomplete_route(app, name, handler, user_data, NULL);
}

dcc_status_t dcc_app_autocomplete_route(dcc_app_t *app, const char *name,
                                        dcc_app_legacy_handler_fn handler,
                                        void *user_data,
                                        dcc_app_route_id_t *out_route) {
  return dcc_app_add_route(app, DCC_EVENT_AUTOCOMPLETE, name, 0U, handler,
                           user_data, out_route);
}

dcc_status_t dcc_app_button(dcc_app_t *app, const char *custom_id,
                            dcc_app_legacy_handler_fn handler,
                            void *user_data) {
  return dcc_app_button_route(app, custom_id, handler, user_data, NULL);
}

dcc_status_t dcc_app_button_route(dcc_app_t *app, const char *custom_id,
                                  dcc_app_legacy_handler_fn handler,
                                  void *user_data,
                                  dcc_app_route_id_t *out_route) {
  return dcc_app_add_route(app, DCC_EVENT_BUTTON_CLICK, custom_id, 0U, handler,
                           user_data, out_route);
}

dcc_status_t dcc_app_button_prefix(dcc_app_t *app, const char *custom_id_prefix,
                                   dcc_app_legacy_handler_fn handler,
                                   void *user_data) {
  return dcc_app_button_prefix_route(app, custom_id_prefix, handler, user_data,
                                     NULL);
}

dcc_status_t dcc_app_button_prefix_route(dcc_app_t *app,
                                         const char *custom_id_prefix,
                                         dcc_app_legacy_handler_fn handler,
                                         void *user_data,
                                         dcc_app_route_id_t *out_route) {
  return dcc_app_add_route(app, DCC_EVENT_BUTTON_CLICK, custom_id_prefix, 1U,
                           handler, user_data, out_route);
}

dcc_status_t dcc_app_select(dcc_app_t *app, const char *custom_id,
                            dcc_app_legacy_handler_fn handler,
                            void *user_data) {
  return dcc_app_select_route(app, custom_id, handler, user_data, NULL);
}

dcc_status_t dcc_app_select_route(dcc_app_t *app, const char *custom_id,
                                  dcc_app_legacy_handler_fn handler,
                                  void *user_data,
                                  dcc_app_route_id_t *out_route) {
  return dcc_app_add_route(app, DCC_EVENT_SELECT_CLICK, custom_id, 0U, handler,
                           user_data, out_route);
}

dcc_status_t dcc_app_select_prefix(dcc_app_t *app, const char *custom_id_prefix,
                                   dcc_app_legacy_handler_fn handler,
                                   void *user_data) {
  return dcc_app_select_prefix_route(app, custom_id_prefix, handler, user_data,
                                     NULL);
}

dcc_status_t dcc_app_select_prefix_route(dcc_app_t *app,
                                         const char *custom_id_prefix,
                                         dcc_app_legacy_handler_fn handler,
                                         void *user_data,
                                         dcc_app_route_id_t *out_route) {
  return dcc_app_add_route(app, DCC_EVENT_SELECT_CLICK, custom_id_prefix, 1U,
                           handler, user_data, out_route);
}

dcc_status_t dcc_app_modal(dcc_app_t *app, const char *custom_id,
                           dcc_app_legacy_handler_fn handler, void *user_data) {
  return dcc_app_modal_route(app, custom_id, handler, user_data, NULL);
}

dcc_status_t dcc_app_modal_route(dcc_app_t *app, const char *custom_id,
                                 dcc_app_legacy_handler_fn handler,
                                 void *user_data,
                                 dcc_app_route_id_t *out_route) {
  return dcc_app_add_route(app, DCC_EVENT_FORM_SUBMIT, custom_id, 0U, handler,
                           user_data, out_route);
}

dcc_status_t dcc_app_modal_prefix(dcc_app_t *app, const char *custom_id_prefix,
                                  dcc_app_legacy_handler_fn handler,
                                  void *user_data) {
  return dcc_app_modal_prefix_route(app, custom_id_prefix, handler, user_data,
                                    NULL);
}

dcc_status_t dcc_app_modal_prefix_route(dcc_app_t *app,
                                        const char *custom_id_prefix,
                                        dcc_app_legacy_handler_fn handler,
                                        void *user_data,
                                        dcc_app_route_id_t *out_route) {
  return dcc_app_add_route(app, DCC_EVENT_FORM_SUBMIT, custom_id_prefix, 1U,
                           handler, user_data, out_route);
}
