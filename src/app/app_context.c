...
dcc_event_type_t dcc_ctx_event_type(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->event != NULL ? dcc_event_type(ctx->event)
                                           : DCC_EVENT_MAX;
}
...
dcc_snowflake_t dcc_ctx_guild_id(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->guild_id
                                                 : 0U;
}

dcc_snowflake_t dcc_ctx_channel_id(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->channel_id
                                                 : 0U;
}
...
static const dcc_channel_t *dcc_ctx_cached_channel(const dcc_ctx_t *ctx) {
  ...
  return client != NULL && channel_id != 0U
             ? dcc_cache_get_channel(client, channel_id)
             : NULL;
}
...
uint8_t dcc_ctx_channel_is_type(const dcc_ctx_t *ctx, uint32_t channel_type) {
  uint32_t actual = 0U;
  return dcc_ctx_channel_type(ctx, &actual) == DCC_OK && actual == channel_type
             ? 1U
             : 0U;
}
...
dcc_snowflake_t dcc_ctx_command_id(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->command_id
                                                 : 0U;
}

dcc_snowflake_t dcc_ctx_message_id(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->message_id
                                                 : 0U;
}

dcc_snowflake_t dcc_ctx_target_id(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->target_id
                                                 : 0U;
}

dcc_snowflake_t dcc_ctx_user_id(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->user_id
                                                 : 0U;
}
...
const dcc_guild_t *dcc_ctx_guild(const dcc_ctx_t *ctx) {
  ...
  return client != NULL && guild_id != 0U
             ? dcc_cache_get_guild(client, guild_id)
             : NULL;
}
...
const dcc_user_t *dcc_ctx_resolved_user(const dcc_ctx_t *ctx,
                                        dcc_snowflake_t id) {
  if (ctx == NULL || id == 0U) {
    return NULL;
  }
...
const dcc_member_t *dcc_ctx_resolved_member(const dcc_ctx_t *ctx,
  ...
  const dcc_member_t *member =
      dcc_interaction_resolved_member(ctx->interaction, user_id);
  if (member != NULL) {
    return member;
  }
  ...
  dcc_snowflake_t guild_id = dcc_ctx_guild_id(ctx);
  return client != NULL && guild_id != 0U
             ? dcc_cache_get_member(client, guild_id, user_id)
             : NULL;
}

const dcc_role_t *dcc_ctx_resolved_role(const dcc_ctx_t *ctx,
                                        dcc_snowflake_t id) {
  if (ctx == NULL || id == 0U) {
    return NULL;
  }
  ...
  dcc_snowflake_t guild_id = dcc_ctx_guild_id(ctx);
  return client != NULL && guild_id != 0U
             ? dcc_cache_get_role(client, guild_id, id)
             : NULL;
}

const dcc_channel_t *dcc_ctx_resolved_channel(const dcc_ctx_t *ctx,
                                              dcc_snowflake_t id) {
  if (ctx == NULL || id == 0U) {
    return NULL;
  }
  ...
  const dcc_channel_t *channel =
      dcc_interaction_resolved_channel(ctx->interaction, id);
  if (channel != NULL) {
    return channel;
  }
...
const dcc_message_t *dcc_ctx_resolved_message(const dcc_ctx_t *ctx,
                                              dcc_snowflake_t id) {
  if (ctx == NULL || id == 0U) {
    return NULL;
  }
  ...
  const dcc_message_t *message =
      dcc_interaction_resolved_message(ctx->interaction, id);
  if (message != NULL) {
    return message;
  }
...
const dcc_attachment_t *dcc_ctx_resolved_attachment(const dcc_ctx_t *ctx,
                                                    dcc_snowflake_t id) {
...
uint8_t dcc_ctx_has_member_permissions(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL
             ? ctx->interaction->has_member_permissions
             : 0U;
}

dcc_permission_t dcc_ctx_member_permissions(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL
             ? ctx->interaction->member_permissions
             : 0U;
}

uint8_t dcc_ctx_member_has_permissions(const dcc_ctx_t *ctx,
                                       dcc_permission_t permissions) {
  return dcc_ctx_has_member_permissions(ctx) &&
                 dcc_permissions_can(dcc_ctx_member_permissions(ctx),
                                     permissions)
             ? 1U
             : 0U;
}

static uint8_t dcc_ctx_member_role_list_has(const dcc_snowflake_t *role_ids,
...
static uint8_t dcc_ctx_member_object_has_role(const dcc_member_t *member,
                                              dcc_snowflake_t role_id) {
  return member != NULL ? dcc_ctx_member_role_list_has(
                              member->role_ids, member->role_ids_count, role_id)
                        : 0U;
}
...
uint8_t dcc_ctx_member_has_role(const dcc_ctx_t *ctx, dcc_snowflake_t role_id) {
  ...
  for (size_t i = 0U; interaction->resolved.members != NULL &&
                      i < interaction->resolved.members_count;
       ++i) {
    const dcc_member_t *member = &interaction->resolved.members[i];
    if (member->user.id == user_id &&
        (member->guild_id == 0U || guild_id == 0U ||
...
uint8_t dcc_ctx_member_has_any_role(const dcc_ctx_t *ctx,
...
uint8_t dcc_ctx_member_has_all_roles(const dcc_ctx_t *ctx,
                                     const dcc_snowflake_t *role_ids,
                                     size_t role_id_count) {
  if (role_ids == NULL || role_id_count == 0U) {
    return 0U;
...
const dcc_component_session_result_t *
dcc_ctx_component_session_result(const dcc_ctx_t *ctx) {
  return ctx != NULL ? ctx->component_session : NULL;
}

dcc_component_session_verify_status_t
dcc_ctx_component_session_status(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->component_session != NULL
...
const char *dcc_ctx_component_session_action(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->component_session != NULL
             ? ctx->component_session->action
             : NULL;
}

const char *dcc_ctx_token(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->token
                                                 : NULL;
}

const char *dcc_ctx_name(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->name
                                                 : NULL;
}

const char *dcc_ctx_custom_id(const dcc_ctx_t *ctx) {
  return ctx != NULL && ctx->interaction != NULL ? ctx->interaction->custom_id
                                                 : NULL;
}
...
uint8_t dcc_ctx_followed_up(const dcc_ctx_t *ctx) {
  return dcc_ctx_response_state(ctx) == DCC_INTERACTION_FLOW_FOLLOWED_UP ? 1U
                                                                         : 0U;
}
...
static dcc_status_t
dcc_ctx_flow_reply_auto(dcc_ctx_t *ctx, const dcc_message_builder_t *message,
...
static dcc_status_t
dcc_ctx_claim_initial_auto(dcc_ctx_t *ctx,
                           dcc_app_response_state_t response_state) {
  if (ctx == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
...
dcc_status_t dcc_ctx_reply_text(dcc_ctx_t *ctx, const char *content,
                                dcc_rest_result_fn cb, void *user_data) {
  if (ctx == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
...
dcc_status_t dcc_ctx_reply_ephemeral_text(dcc_ctx_t *ctx, const char *content,
...
dcc_status_t dcc_ctx_reply_embed(dcc_ctx_t *ctx, const char *title,
                                 const char *description, uint32_t color,
                                 uint8_t ephemeral, dcc_rest_result_fn cb,
                                 void *user_data) {
...
dcc_status_t dcc_ctx_reply_error(dcc_ctx_t *ctx, const char *title,
                                 const char *description, dcc_rest_result_fn cb,
                                 void *user_data) {
  return dcc_ctx_reply_embed(ctx, title != NULL ? title : "Error", description,
                             0xED4245U, 1U, cb, user_data);
}

dcc_status_t dcc_ctx_reply_success(dcc_ctx_t *ctx, const char *title,
                                   const char *description,
                                   dcc_rest_result_fn cb, void *user_data) {
  return dcc_ctx_reply_embed(ctx, title != NULL ? title : "Done", description,
                             0x57F287U, 1U, cb, user_data);
}

dcc_status_t dcc_ctx_defer(dcc_ctx_t *ctx, dcc_rest_result_fn cb,
                           void *user_data) {
  dcc_status_t status =
      dcc_ctx_claim_initial_auto(ctx, DCC_APP_RESPONSE_DEFERRED);
  if (status != DCC_OK) {
    return status;
  }
...
dcc_status_t dcc_ctx_defer_ephemeral(dcc_ctx_t *ctx, dcc_rest_result_fn cb,
                                     void *user_data) {
  dcc_status_t status =
      dcc_ctx_claim_initial_auto(ctx, DCC_APP_RESPONSE_DEFERRED);
  if (status != DCC_OK) {
...
dcc_status_t dcc_ctx_update_message(dcc_ctx_t *ctx,
  ...
  dcc_status_t status =
      dcc_ctx_claim_initial_auto(ctx, DCC_APP_RESPONSE_REPLIED);
  if (status != DCC_OK) {
    return status;
  }
...
dcc_status_t dcc_ctx_show_modal(dcc_ctx_t *ctx,
...
dcc_status_t dcc_ctx_show_modal_flow(dcc_ctx_t *ctx,
                                     const dcc_app_modal_flow_t *flow,
                                     dcc_rest_result_fn cb, void *user_data) {
  dcc_builder_abi_view_t modal_view;
  if (flow == NULL || flow->size < offsetof(dcc_app_modal_flow_t, route) ||
      sizeof(flow->route) >
          flow->size - offsetof(dcc_app_modal_flow_t, route) ||
      dcc_modal_builder_abi_validate(&flow->modal, &modal_view) != DCC_OK ||
      !dcc_builder_abi_view_has(&modal_view,
                                DCC_MODAL_BUILDER_PRESENT_CUSTOM_ID) ||
      !dcc_builder_abi_view_has(&modal_view, DCC_MODAL_BUILDER_PRESENT_TITLE) ||
      flow->modal.custom_id == NULL || flow->modal.custom_id[0] == '\0' ||
      flow->modal.title == NULL || flow->modal.title[0] == '\0' ||
      flow->route.custom_id == NULL || flow->route.prefix != 0U ||
...
dcc_status_t
dcc_ctx_reply_autocomplete(dcc_ctx_t *ctx,
...
dcc_status_t
dcc_ctx_reply_autocomplete_matching(dcc_ctx_t *ctx,
                                    const dcc_autocomplete_choice_t *choices,
                                    size_t choices_count, size_t max_choices,
                                    dcc_rest_result_fn cb, void *user_data) {
  ...
  dcc_status_t status = dcc_ctx_autocomplete_filter_choices(
      ctx, choices, choices_count, filtered, max_choices, &filtered_count);
  if (status != DCC_OK) {
    return status;
  }
  ...
  status = dcc_autocomplete_builder_set_choices(&autocomplete, filtered,
                                                filtered_count);
  if (status != DCC_OK) {
    return status;
  }
...
dcc_status_t dcc_ctx_edit_original(dcc_ctx_t *ctx,
                                   const dcc_message_builder_t *message,
                                   dcc_rest_result_fn cb, void *user_data) {
  if (ctx == NULL) {
    return DCC_ERR_INVALID_ARG;
...
dcc_status_t dcc_ctx_followup_text(dcc_ctx_t *ctx, const char *content,
                                   dcc_rest_result_fn cb, void *user_data) {
  if (ctx == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
...
dcc_status_t dcc_ctx_followup_ephemeral_text(dcc_ctx_t *ctx,
                                             const char *content,
                                             dcc_rest_result_fn cb,
                                             void *user_data) {
  if (ctx == NULL) {
...
static const dcc_interaction_option_t *
dcc_ctx_find_option(const dcc_interaction_option_t *options, size_t count,
                    const char *name) {
  if (name == NULL) {
    return NULL;
  }
...
const dcc_interaction_option_t *dcc_ctx_option(const dcc_ctx_t *ctx,
                                               const char *name) {
  return ctx != NULL && ctx->interaction != NULL
             ? dcc_ctx_find_option(ctx->interaction->options,
                                   ctx->interaction->options_count, name)
             : NULL;
}
...
const char *dcc_ctx_focused_option_string(const dcc_ctx_t *ctx,
                                          const char *fallback) {
  const dcc_interaction_option_t *option = dcc_ctx_focused_option(ctx);
  return option != NULL &&
...
const char *dcc_ctx_option_string(const dcc_ctx_t *ctx, const char *name,
                                  const char *fallback) {
...
int64_t dcc_ctx_option_int(const dcc_ctx_t *ctx, const char *name,
                           int64_t fallback) {
  const dcc_interaction_option_t *option = dcc_ctx_option(ctx, name);
  return option != NULL &&
...
double dcc_ctx_option_number(const dcc_ctx_t *ctx, const char *name,
                             double fallback) {
  const dcc_interaction_option_t *option = dcc_ctx_option(ctx, name);
  return option != NULL &&
...
uint8_t dcc_ctx_option_bool(const dcc_ctx_t *ctx, const char *name,
                            uint8_t fallback) {
  const dcc_interaction_option_t *option = dcc_ctx_option(ctx, name);
  return option != NULL &&
...
dcc_snowflake_t dcc_ctx_option_snowflake(const dcc_ctx_t *ctx, const char *name,
...
static dcc_snowflake_t dcc_ctx_option_resolved_id(const dcc_ctx_t *ctx,
                                                  const char *name) {
  const dcc_interaction_option_t *option = dcc_ctx_option(ctx, name);
  return option != NULL &&
                 option->value_type == DCC_INTERACTION_OPTION_VALUE_SNOWFLAKE
...
const dcc_member_t *dcc_ctx_option_member(const dcc_ctx_t *ctx,
                                          const char *name) {
...
const dcc_channel_t *dcc_ctx_option_channel(const dcc_ctx_t *ctx,
                                            const char *name) {
  return dcc_ctx_resolved_channel(ctx, dcc_ctx_option_resolved_id(ctx, name));
}

const dcc_message_t *dcc_ctx_option_message(const dcc_ctx_t *ctx,
                                            const char *name) {
  return dcc_ctx_resolved_message(ctx, dcc_ctx_option_resolved_id(ctx, name));
}

const dcc_attachment_t *dcc_ctx_option_attachment(const dcc_ctx_t *ctx,
                                                  const char *name) {
  return dcc_ctx_resolved_attachment(ctx,
                                     dcc_ctx_option_resolved_id(ctx, name));
}
...
static uint8_t
dcc_ctx_autocomplete_starts_with_ignore_case(const char *value,
                                             const char *prefix) {
...
static uint8_t dcc_ctx_autocomplete_choice_matches_query(
    const dcc_autocomplete_choice_t *choice, const char *query) {
  ...
  return choice->value_type == DCC_AUTOCOMPLETE_CHOICE_STRING
             ? dcc_ctx_autocomplete_starts_with_ignore_case(
                   choice->value_string, query)
             : 0U;
}
...
static int dcc_ctx_autocomplete_choice_output_covers(
  ...
  return out_view->present == 0U &&
         ...
         DCC_BUILDER_ABI_FIELD(dcc_autocomplete_choice_t, out_view,
                               value_string) &&
         DCC_BUILDER_ABI_FIELD(dcc_autocomplete_choice_t, out_view,
                               value_integer) &&
         DCC_BUILDER_ABI_FIELD(dcc_autocomplete_choice_t, out_view,
...
dcc_status_t dcc_ctx_autocomplete_filter_choices(
  ...
  *out_count = 0U;
  ...
  size_t stride = 0U;
  dcc_status_t status =
      dcc_autocomplete_choice_array_begin(choices, choices_count, &stride);
  if (status != DCC_OK) {
    return status;
  }
  for (size_t i = 0U; i < choices_count; ++i) {
    const dcc_autocomplete_choice_t *choice =
        (const dcc_autocomplete_choice_t *)dcc_builder_abi_array_at(choices,
                                                                    stride, i);
    dcc_builder_abi_view_t source_view;
    if (dcc_autocomplete_choice_semantic_validate(choice, &source_view) !=
            DCC_OK ||
  ...
  status = dcc_autocomplete_choice_array_begin(out_choices, out_capacity,
                                               &out_stride);
  if (status != DCC_OK) {
    return status;
  }
  for (size_t i = 0U; i < out_capacity; ++i) {
    dcc_autocomplete_choice_t *out =
        (dcc_autocomplete_choice_t *)dcc_builder_abi_array_at(out_choices,
                                                              out_stride, i);
  ...
  for (size_t i = 0U; i < choices_count; ++i) {
    ...
    if (matched < out_capacity &&
      ...
      dcc_autocomplete_choice_t *out =
          (dcc_autocomplete_choice_t *)dcc_builder_abi_array_at(
              out_choices, out_stride, matched);
      dcc_builder_abi_view_t out_view;
      if (dcc_autocomplete_choice_abi_validate(out, &out_view) != DCC_OK ||
...
const dcc_interaction_form_field_t *dcc_ctx_form_field(const dcc_ctx_t *ctx,
...
const char *dcc_ctx_form_string(const dcc_ctx_t *ctx, const char *custom_id,
                                const char *fallback) {
  const dcc_interaction_form_field_t *field =
      dcc_ctx_form_field(ctx, custom_id);
  return field != NULL &&
...
uint8_t dcc_ctx_form_bool(const dcc_ctx_t *ctx, const char *custom_id,
                          uint8_t fallback) {
  const dcc_interaction_form_field_t *field =
      dcc_ctx_form_field(ctx, custom_id);
  return field != NULL &&
...
const char *dcc_ctx_form_value(const dcc_ctx_t *ctx, const char *custom_id,
                               size_t index) {
  const dcc_interaction_form_field_t *field =
      dcc_ctx_form_field(ctx, custom_id);
  if (field == NULL || field->value_type != DCC_INTERACTION_FORM_VALUE_VALUES ||
      field->values == NULL || index >= field->values_count) {
    return NULL;
  }
  return field->values[index];
}

const char *dcc_ctx_form_first_value(const dcc_ctx_t *ctx,
                                     const char *custom_id,
                                     const char *fallback) {
...
const char **dcc_ctx_form_values(const dcc_ctx_t *ctx, const char *custom_id,
                                 size_t *out_count) {
  const dcc_interaction_form_field_t *field =
      dcc_ctx_form_field(ctx, custom_id);
  if (field == NULL || field->value_type != DCC_INTERACTION_FORM_VALUE_VALUES ||
...
const char *dcc_ctx_component_first_value(const dcc_ctx_t *ctx,
                                          const char *fallback) {
