...
dcc_status_t dcc_ctx_add_member_role(dcc_ctx_t *ctx, dcc_snowflake_t user_id,
                                     dcc_snowflake_t role_id, dcc_rest_cb cb,
                                     void *user_data) {
  if (ctx == NULL || dcc_ctx_app(ctx) == NULL || dcc_ctx_guild_id(ctx) == 0U ||
      user_id == 0U || role_id == 0U) {
    return DCC_ERR_INVALID_ARG;
  }
  DCC_ENDPOINT_LEGACY_RETURN(cb, user_data, dcc_rest_add_guild_member_role,
                             dcc_app_client(dcc_ctx_app(ctx)),
                             dcc_ctx_guild_id(ctx), user_id, role_id);
}
...
dcc_status_t dcc_ctx_remove_member_role(dcc_ctx_t *ctx, dcc_snowflake_t user_id,
  ...
  DCC_ENDPOINT_LEGACY_RETURN(cb, user_data, dcc_rest_remove_guild_member_role,
                             dcc_app_client(dcc_ctx_app(ctx)),
                             dcc_ctx_guild_id(ctx), user_id, role_id);
}
...
dcc_ctx_create_thread_from_message(dcc_ctx_t *ctx,
  ...
  DCC_ENDPOINT_LEGACY_RETURN(cb, user_data, dcc_rest_create_thread_from_message,
                             dcc_app_client(dcc_ctx_app(ctx)),
...
dcc_ctx_modify_current_thread(dcc_ctx_t *ctx, uint64_t present,
                              uint8_t archived, uint8_t locked, dcc_rest_cb cb,
                              void *user_data) {
  if (ctx == NULL || dcc_ctx_app(ctx) == NULL ||
  ...
  DCC_ENDPOINT_LEGACY_RETURN(cb, user_data, dcc_rest_modify_channel,
                             dcc_app_client(dcc_ctx_app(ctx)),
                             dcc_ctx_channel_id(ctx), &params);
}

dcc_status_t dcc_ctx_archive_current_thread(dcc_ctx_t *ctx, dcc_rest_cb cb,
                                            void *user_data) {
  return dcc_ctx_modify_current_thread(ctx, DCC_CHANNEL_THREAD_PRESENT_ARCHIVED,
                                       1U, 0U, cb, user_data);
}

dcc_status_t dcc_ctx_lock_current_thread(dcc_ctx_t *ctx, dcc_rest_cb cb,
                                         void *user_data) {
...
dcc_status_t dcc_ctx_unlock_current_thread(dcc_ctx_t *ctx, dcc_rest_cb cb,
                                           void *user_data) {
...
dcc_status_t dcc_ctx_ok(dcc_ctx_t *ctx, const char *description, dcc_rest_cb cb,
                        void *user_data) {
