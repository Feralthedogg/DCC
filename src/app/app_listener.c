...
#if defined(_WIN32)
  (void)SleepConditionVariableCS(&app->listener_cond, &app->listener_mutex,
                                 INFINITE);
#else
...
static uint8_t dcc_app_listener_has_full_value(size_t size, uint32_t version,
                                               size_t required,
...
static void dcc_app_listener_entry_free(dcc_app_listener_entry_t *entry) {
  ...
  for (size_t i = 0U; i < entry->binding_count; ++i) {
    ...
    if (entry->form_bindings != NULL) {
      ...
      for (size_t j = 0U; j < entry->form_bindings[i].fallback_values_count;
           ++j) {
    ...
    if (entry->component_bindings != NULL) {
      ...
      for (size_t j = 0U;
           j < entry->component_bindings[i].fallback_values_count; ++j) {
...
static dcc_status_t dcc_app_listener_copy_string(const char *value,
                                                 const char **out) {
  if (out == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
...
static dcc_status_t
dcc_app_listener_copy_string_array(dcc_app_t *app, const char *const *values,
...
static dcc_status_t dcc_app_listener_stage_pointer_array(
    void **array, size_t count, size_t *cap, size_t item_size,
    size_t initial_cap, size_t need, void **out_original,
    size_t *out_original_cap, uint8_t *out_active) {
  *out_original = NULL;
  *out_original_cap = 0U;
  *out_active = 0U;
  if (need <= *cap) {
...
static dcc_status_t
dcc_app_listener_stage_listener_array(dcc_app_t *app,
                                      dcc_app_listener_array_stage_t *stage) {
  memset(stage, 0, sizeof(*stage));
  return dcc_app_listener_stage_pointer_array(
      (void **)&app->listeners, app->listener_count, &app->listener_cap,
      sizeof(*app->listeners), 8U, app->listener_count + 1U,
      (void **)&stage->original, &stage->original_cap, &stage->active);
}

static dcc_status_t
dcc_app_listener_stage_route_array(dcc_app_t *app,
                                   dcc_app_route_array_stage_t *stage) {
  memset(stage, 0, sizeof(*stage));
  return dcc_app_listener_stage_pointer_array(
      (void **)&app->routes, app->route_count, &app->route_cap,
      sizeof(*app->routes), 8U, app->route_count + 1U,
      (void **)&stage->original, &stage->original_cap, &stage->active);
}

static dcc_status_t
dcc_app_listener_stage_schedule_array(dcc_app_t *app,
                                      dcc_app_schedule_array_stage_t *stage) {
  memset(stage, 0, sizeof(*stage));
  return dcc_app_listener_stage_pointer_array(
      (void **)&app->schedules, app->schedule_count, &app->schedule_cap,
      sizeof(*app->schedules), 4U, app->schedule_count + 1U,
      (void **)&stage->original, &stage->original_cap, &stage->active);
}

static void dcc_app_listener_finish_listener_array(
    dcc_app_t *app, dcc_app_listener_array_stage_t *stage, uint8_t commit) {
  if (!stage->active) {
    return;
  }
  if (commit) {
...
static void dcc_app_listener_finish_route_array(
    dcc_app_t *app, dcc_app_route_array_stage_t *stage, uint8_t commit) {
  if (!stage->active) {
    return;
  }
  if (commit) {
...
static void dcc_app_listener_finish_schedule_array(
    dcc_app_t *app, dcc_app_schedule_array_stage_t *stage, uint8_t commit) {
  if (!stage->active) {
    return;
  }
  if (commit) {
...
static void dcc_app_listener_retire_locked(dcc_app_t *app,
  ...
  for (size_t i = 0U; i < app->listener_count; ++i) {
    ...
    if (i + 1U < app->listener_count) {
      memmove(&app->listeners[i], &app->listeners[i + 1U],
...
static dcc_status_t
dcc_app_listener_detach_source(dcc_app_listener_entry_t *entry,
...
static dcc_status_t dcc_app_listener_finalize(dcc_app_listener_entry_t *entry,
                                              uint8_t wait_for_completion) {
  if (entry == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
  ...
  if (entry->finalize_started) {
    ...
    dcc_status_t status =
        entry->finalize_done ? entry->finalize_status : DCC_OK;
    dcc_app_listener_unlock(app);
    return status;
  }
...
dcc_app_listener_validate_policy(const dcc_listener_route_policy_t *policy) {
  ...
  if (!dcc_app_listener_has_full_value(policy->size, policy->version,
      ...
      policy->guild_only > 1U || policy->dm_only > 1U ||
      policy->nsfw_only > 1U ||
      (policy->dm_only != 0U &&
  ...
  if (policy->middleware_count > SIZE_MAX / sizeof(*policy->middlewares) ||
      policy->owner_user_id_count >
          SIZE_MAX / sizeof(*policy->owner_user_ids) ||
      ...
      policy->required_role_id_count >
          SIZE_MAX / sizeof(*policy->required_role_ids) ||
  ...
  for (size_t i = 0U; i < policy->middleware_count; ++i) {
    ...
    if (!dcc_app_listener_has_full_value(item->size, item->version,
                                         sizeof(*item),
                                         DCC_LISTENER_MIDDLEWARE_VERSION) ||
        item->callback == NULL) {
      return DCC_ERR_INVALID_ARG;
    }
  }
  ...
  if (cooldown->bucket < DCC_LISTENER_COOLDOWN_GLOBAL ||
      cooldown->bucket > DCC_LISTENER_COOLDOWN_GUILD || cooldown->limit == 0U ||
      cooldown->window_ms == 0U) {
    return DCC_ERR_INVALID_ARG;
  }
  return DCC_OK;
}
...
static uint8_t dcc_app_listener_binding_fallbacks_are_valid(
    dcc_listener_binding_kind_t kind, const dcc_listener_binding_t *item) {
  ...
  if (kind == DCC_LISTENER_BIND_FORM) {
    ...
    return item->type == DCC_CTX_FORM_BIND_VALUES &&
           item->fallback_string == NULL && item->fallback_boolean == 0U;
  }
...
static uint8_t dcc_app_listener_range_fits(size_t offset, size_t width,
                                           size_t total) {
  return offset <= total && width <= total - offset;
}

static size_t dcc_app_listener_binding_width(dcc_listener_binding_kind_t kind,
                                             uint32_t type) {
  if (kind == DCC_LISTENER_BIND_OPTIONS) {
    ...
    return type == DCC_CTX_COMPONENT_BIND_COMPONENT_TYPE ? sizeof(uint32_t)
                                                         : sizeof(void *);
  }
  return 0U;
}
...
static size_t
dcc_app_listener_validator_width(dcc_listener_validator_kind_t kind) {
  switch (kind) {
  case DCC_LISTENER_VALIDATE_INTEGER_RANGE:
    return sizeof(int64_t);
  case DCC_LISTENER_VALIDATE_NUMBER_RANGE:
...
dcc_app_listener_validate_bindings(const dcc_listener_t *listener) {
  ...
  if (!dcc_app_listener_has_full_value(bindings->size, bindings->version,
      ...
      !dcc_app_listener_has_full_value(validators->size, validators->version,
                                       sizeof(*validators),
                                       DCC_LISTENER_VALIDATORS_VERSION) ||
      !dcc_app_listener_has_full_value(
          validation->size, validation->version, sizeof(*validation),
  ...
  if (listener->args_size == 0U) {
    return bindings->kind == DCC_LISTENER_BIND_NONE && bindings->count == 0U &&
                   ...
                   validators->items == NULL &&
                   validation->suppress_response == 0U
               ? DCC_OK
               : DCC_ERR_INVALID_ARG;
  }
  if (bindings->kind < DCC_LISTENER_BIND_OPTIONS ||
      bindings->kind > DCC_LISTENER_BIND_COMPONENT || bindings->count == 0U ||
      bindings->items.options == NULL ||
  ...
  for (size_t i = 0U; i < bindings->count; ++i) {
    ...
    if (!dcc_app_listener_has_full_value(item->size, item->version,
                                         sizeof(*item),
                                         DCC_LISTENER_BINDING_VERSION) ||
        item->type == 0U || item->has_count_offset > 1U ||
        item->required > 1U || item->fallback_boolean > 1U) {
      return DCC_ERR_INVALID_ARG;
    }
    if (bindings->kind == DCC_LISTENER_BIND_OPTIONS &&
    ...
    uint8_t supports_count = (bindings->kind == DCC_LISTENER_BIND_FORM &&
                              item->type == DCC_CTX_FORM_BIND_VALUES) ||
                             (bindings->kind == DCC_LISTENER_BIND_COMPONENT &&
                              item->type == DCC_CTX_COMPONENT_BIND_VALUES);
    if (width == 0U ||
        !dcc_app_listener_range_fits(item->field_offset, width,
                                     listener->args_size) ||
        (item->has_count_offset != 0U &&
         (!supports_count ||
          !dcc_app_listener_range_fits(item->count_offset, sizeof(size_t),
                                       listener->args_size))) ||
        (item->has_count_offset == 0U && item->count_offset != 0U) ||
        !dcc_app_listener_binding_fallbacks_are_valid(bindings->kind, item)) {
      return DCC_ERR_INVALID_ARG;
    }
    if (item->fallback_values_count >
        SIZE_MAX / sizeof(*item->fallback_values)) {
      return DCC_ERR_NOMEM;
    }
  ...
  for (size_t i = 0U; i < validators->count; ++i) {
    ...
    if (!dcc_app_listener_has_full_value(item->size, item->version,
                                         sizeof(*item),
                                         DCC_LISTENER_VALIDATOR_VERSION) ||
        ...
        !dcc_app_listener_range_fits(
            item->field_offset, dcc_app_listener_validator_width(item->kind),
            listener->args_size) ||
...
static dcc_status_t dcc_app_listener_validate_command_schema(
  ...
  if (dcc_app_listener_is_component_kind(kind) ||
      ...
      (dcc_builder_abi_view_has(
           &view, DCC_APPLICATION_COMMAND_BUILDER_PRESENT_DM_PERMISSION) &&
  ...
  dcc_application_command_type_t expected_type =
...
static dcc_status_t
dcc_app_listener_validate_target(const dcc_listener_t *listener) {
  if (dcc_app_listener_is_route_kind(listener->kind)) {
    const dcc_listener_route_target_t *target = &listener->target.route;
    if (!dcc_app_listener_has_full_value(target->size, target->version,
                                         sizeof(*target),
                                         DCC_LISTENER_TARGET_VERSION)) {
      return DCC_ERR_INVALID_ARG;
    }
    dcc_status_t command_status = dcc_app_listener_validate_command_schema(
        listener->kind, target->command);
    if (command_status != DCC_OK) {
      return command_status;
    }
    if (listener->kind == DCC_LISTENER_SUBCOMMAND) {
      return target->name == NULL && target->command_name != NULL &&
                     target->command_name[0] != '\0' &&
                     target->subcommand_path != NULL &&
                     target->subcommand_path[0] != '\0' &&
    ...
    if (target->command != NULL) {
      return target->name == NULL && target->description == NULL &&
                     target->command->name != NULL &&
                     target->command->name[0] != '\0'
                 ? DCC_OK
                 : DCC_ERR_INVALID_ARG;
    }
  ...
  if (listener->kind == DCC_LISTENER_EVENT ||
      listener->kind == DCC_LISTENER_READY ||
  ...
  if (listener->kind == DCC_LISTENER_MESSAGE_COMMAND) {
    const dcc_listener_message_command_target_t *target =
        &listener->target.message_command;
    return dcc_app_listener_has_full_value(target->size, target->version,
                                           sizeof(*target),
                                           DCC_LISTENER_TARGET_VERSION) &&
  ...
  if (listener->kind == DCC_LISTENER_TASK) {
    ...
    if (target->kind == DCC_LISTENER_SCHEDULE_INTERVAL) {
      return target->interval_ms != 0U && target->hour == 0U &&
                     target->minute == 0U
                 ? DCC_OK
                 : DCC_ERR_INVALID_ARG;
    }
    return target->kind == DCC_LISTENER_SCHEDULE_DAILY_KST &&
                   target->interval_ms == 0U && target->hour <= 23U &&
                   target->minute <= 59U
               ? DCC_OK
               : DCC_ERR_INVALID_ARG;
  }
...
static uint8_t dcc_app_listener_handler_is_set(const dcc_listener_t *listener) {
  if (dcc_app_listener_is_route_kind(listener->kind)) {
    return listener->args_size == 0U ? listener->handler.plain != NULL
...
static dcc_status_t dcc_app_listener_validate(const dcc_listener_t *listener) {
  if (listener == NULL || listener->size < sizeof(*listener) ||
      ...
      listener->kind <= DCC_LISTENER_NONE ||
...
static dcc_status_t
dcc_app_listener_copy_bindings(dcc_app_listener_entry_t *entry,
  ...
  if (entry->binding_kind == DCC_LISTENER_BIND_OPTIONS) {
    ...
    for (size_t i = 0U; i < count; ++i) {
      entry->option_bindings[i] = (dcc_ctx_option_field_binding_t){
          ...
          .field_offset = items[i].field_offset,
      ...
      dcc_status_t status = dcc_app_listener_copy_string(
          items[i].name, &entry->option_bindings[i].name);
      if (status == DCC_OK) {
    ...
    entry->form_bindings = (dcc_ctx_form_field_binding_t *)calloc(
        count, sizeof(*entry->form_bindings));
    if (entry->form_bindings == NULL) {
      return DCC_ERR_NOMEM;
    }
    for (size_t i = 0U; i < count; ++i) {
      entry->form_bindings[i] = (dcc_ctx_form_field_binding_t){
          ...
          .field_offset = items[i].field_offset,
          .count_offset = items[i].count_offset,
          .has_count_offset = items[i].has_count_offset,
          .required = items[i].required,
          .fallback_boolean = items[i].fallback_boolean,
      };
      dcc_status_t status = dcc_app_listener_copy_string(
          items[i].name, &entry->form_bindings[i].custom_id);
      if (status == DCC_OK) {
        status = dcc_app_listener_copy_string(
            items[i].fallback_string, &entry->form_bindings[i].fallback_string);
      }
      if (status == DCC_OK) {
        status = dcc_app_listener_copy_string_array(
            entry->app, items[i].fallback_values,
    ...
    entry->component_bindings = (dcc_ctx_component_field_binding_t *)calloc(
        count, sizeof(*entry->component_bindings));
    if (entry->component_bindings == NULL) {
      return DCC_ERR_NOMEM;
    }
    for (size_t i = 0U; i < count; ++i) {
  ...
  for (size_t i = 0U; i < entry->validator_count; ++i) {
    ...
    entry->validators[i] = (dcc_ctx_field_validator_t){
        ...
        .min_integer = item->min_integer,
        .max_integer = item->max_integer,
        .min_number = item->min_number,
        .max_number = item->max_number,
    };
    dcc_status_t status =
        dcc_app_listener_copy_string(item->field, &entry->validators[i].field);
    if (status != DCC_OK) {
      return status;
    }
  }
...
static dcc_status_t dcc_app_listener_bind_args(dcc_app_listener_entry_t *entry,
                                               dcc_ctx_t *ctx, void *args) {
  if (entry->binding_kind == DCC_LISTENER_BIND_OPTIONS) {
    return dcc_ctx_bind_option_fields(ctx, args, entry->option_bindings,
                                      entry->binding_count);
  }
  if (entry->binding_kind == DCC_LISTENER_BIND_FORM) {
    return dcc_ctx_bind_form_fields(ctx, args, entry->form_bindings,
                                    entry->binding_count);
  }
  return dcc_ctx_bind_component_fields(ctx, args, entry->component_bindings,
                                       entry->binding_count);
}

static dcc_status_t dcc_app_listener_route_dispatch(dcc_ctx_t *ctx,
                                                    void *user_data) {
  ...
  if (status == DCC_OK) {
    validated = 1U;
    status = dcc_ctx_validate_fields(args, entry->validators,
                                     entry->validator_count, &error);
  }
...
dcc_app_listener_apply_policy(dcc_app_t *app, dcc_app_route_id_t route_id,
  ...
  if (policy->middleware_count != 0U) {
    middlewares = (dcc_app_extension_middleware_t *)calloc(
        policy->middleware_count, sizeof(*middlewares));
    if (middlewares == NULL) {
      return DCC_ERR_NOMEM;
    }
    for (size_t i = 0U; i < policy->middleware_count; ++i) {
  ...
  if (policy->check_count != 0U) {
    ...
    for (size_t i = 0U; i < policy->check_count; ++i) {
      checks[i] = (dcc_app_check_t){
          .size = sizeof(checks[i]),
          .check = policy->checks[i].callback,
  ...
  dcc_status_t status =
      dcc_app_route_apply_policy_internal(app, route_id, &legacy);
  free(middlewares);
  free(checks);
  return status;
}
...
static dcc_status_t dcc_app_listener_prepare_command_schema(
    dcc_app_t *app, const dcc_listener_t *listener,
  ...
  if (target->command != NULL) {
    return dcc_command_registry_add_prepare(transaction, &app->registry,
                                            target->command);
  }
  ...
  const char *command_name =
      listener->kind == DCC_LISTENER_SUBCOMMAND ||
          ...
          : target->name;
  dcc_status_t status =
      dcc_application_command_builder_set_name(&command, command_name);
  if (status == DCC_OK) {
    status = dcc_application_command_builder_set_type(
        &command, listener->kind == DCC_LISTENER_USER_CONTEXT_MENU
  ...
  if (status == DCC_OK) {
    status =
        dcc_command_registry_add_prepare(transaction, &app->registry, &command);
  }
  return status;
}

static dcc_status_t dcc_app_listener_route_key(const dcc_listener_t *listener,
                                               const char **out_key,
                                               char *key_buffer,
  ...
  if (listener->kind == DCC_LISTENER_SUBCOMMAND ||
      (listener->kind == DCC_LISTENER_AUTOCOMPLETE &&
       target->command_name != NULL)) {
    dcc_status_t status =
        dcc_app_subcommand_key(target->command_name, target->subcommand_path,
...
static dcc_status_t
dcc_app_listener_register_route(dcc_app_listener_entry_t *entry,
                                const dcc_listener_t *listener) {
  char key_buffer[256];
  const char *key = NULL;
  dcc_status_t status = dcc_app_listener_route_key(listener, &key, key_buffer,
                                                   sizeof(key_buffer));
  dcc_app_route_id_t previous_route_id = entry->app->next_route_id;
  if (status == DCC_OK) {
    status = dcc_app_add_canonical_route_with_cleanup(
        entry->app, dcc_app_listener_route_event(listener->kind), key,
        dcc_app_listener_route_prefix(listener->kind),
        dcc_app_listener_route_dispatch, entry, entry->user_data, entry, NULL,
        &entry->route_id);
  }
  if (status == DCC_OK) {
...
static void dcc_app_listener_report_status(dcc_app_listener_entry_t *entry,
                                           dcc_client_t *client,
  ...
  if (ctx.interaction != NULL) {
    if (dcc_flow_create(client, ctx.interaction, &ctx.flow) != DCC_OK)
      ctx.interaction = NULL;
  }
  const char *operation = event != NULL
                              ? dcc_event_type_name(dcc_event_type(event))
                          : entry->kind == DCC_LISTENER_TASK ? "scheduled task"
                                                             : "App listener";
  (void)dcc_app_report_handler_error(
      entry->app, ctx.interaction != NULL ? &ctx : NULL, status, operation,
      dcc_status_string(status));
  dcc_flow_destroy(ctx.flow);
}

static const char *
dcc_app_listener_message_command_args(const dcc_app_listener_entry_t *entry,
                                      const char *content) {
...
static void dcc_app_listener_event_dispatch(dcc_client_t *client,
                                            const dcc_event_t *event,
                                            void *user_data) {
  dcc_app_listener_entry_t *entry = (dcc_app_listener_entry_t *)user_data;
  ...
  if (entry->kind == DCC_LISTENER_EVENT) {
    ...
    if (message != NULL) {
      status =
          entry->handler.message(entry->app, message, event, entry->user_data);
    }
    ...
    const char *args =
        message != NULL
            ...
            : NULL;
    if (args != NULL) {
      status = entry->handler.message_command(entry->app, message, args, event,
                                              entry->user_data);
    }
  }
...
static dcc_status_t
dcc_app_listener_register_event(dcc_app_listener_entry_t *entry,
                                const dcc_listener_t *listener) {
  if (listener->kind == DCC_LISTENER_EVENT) {
  ...
  return dcc_client_on(entry->app->client, entry->event_type,
                       dcc_app_listener_event_dispatch, entry,
...
static dcc_status_t dcc_app_listener_copy_message_command(
  ...
  entry->message_command_text =
      (char *)malloc(entry->prefix_len + entry->name_len + 2U);
  if (entry->message_command_text == NULL) {
    return DCC_ERR_NOMEM;
  }
  memcpy(entry->message_command_text, target->prefix, entry->prefix_len + 1U);
  memcpy(entry->message_command_text + entry->prefix_len + 1U, target->name,
         entry->name_len + 1U);
  return DCC_OK;
}

static dcc_status_t
dcc_app_listener_register_task(dcc_app_listener_entry_t *entry,
                               const dcc_listener_t *listener) {
  const dcc_listener_schedule_target_t *target = &listener->target.schedule;
  return dcc_app_add_canonical_schedule(
      ...
      target->interval_ms, target->hour, target->minute,
      dcc_app_listener_run_task, entry, &entry->schedule);
}

dcc_status_t dcc_app_listen(dcc_app_t *app, const dcc_listener_t *listener,
  ...
  dcc_app_listener_entry_t *entry =
      (dcc_app_listener_entry_t *)calloc(1U, sizeof(*entry));
  if (entry == NULL) {
    return DCC_ERR_NOMEM;
  }
  entry->app = app;
  ...
  if (status == DCC_OK && listener->kind == DCC_LISTENER_MESSAGE_COMMAND) {
    status = dcc_app_listener_copy_message_command(
        entry, &listener->target.message_command);
  }
  if (status != DCC_OK) {
  ...
  if (app->listener_destroying || app->listener_count == SIZE_MAX ||
    ...
    status = dcc_app_listener_prepare_command_schema(app, listener,
                                                     &command_transaction);
  }
  if (status == DCC_OK) {
  ...
  dcc_app_listener_finish_schedule_array(app, &schedule_stage,
                                         status == DCC_OK);
  dcc_app_listener_finish_route_array(app, &route_stage, status == DCC_OK);
  dcc_app_listener_finish_listener_array(app, &listener_stage,
                                         status == DCC_OK);
  dcc_app_listener_unlock(app);
  if (status != DCC_OK) {
    dcc_app_listener_entry_free(entry);
...
dcc_status_t dcc_app_unlisten(dcc_app_t *app, dcc_listener_id_t id) {
  ...
  dcc_status_t status =
      dcc_app_listener_detach_source(entry, self_unlisten ? 0U : 1U);
  if (self_unlisten) {
...
void dcc_app_listener_destroy_all(dcc_app_t *app) {
  ...
  for (dcc_app_listener_entry_t *entry = app->retired_listeners; entry != NULL;
