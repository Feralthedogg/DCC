...
static dcc_status_t
dcc_app_message_id_status(const dcc_rest_response_t *response,
...
static dcc_status_t
dcc_app_response_id_status(const dcc_rest_response_t *response,
...
static int dcc_app_thread_archive_duration_valid(
    dcc_channel_auto_archive_duration_t duration) {
  return duration == 0 || duration == DCC_CHANNEL_AUTO_ARCHIVE_1_HOUR ||
...
static void
dcc_app_message_thread_state_free(dcc_app_message_thread_state_t *state) {
  if (state == NULL) {
    return;
  }
...
static dcc_status_t
dcc_app_message_thread_state_copy(dcc_app_message_thread_state_t *state,
                                  const dcc_thread_params_t *thread) {
  if (state == NULL || thread == NULL || thread->size < sizeof(*thread) ||
      thread->version != DCC_THREAD_PARAMS_VERSION || thread->name == NULL ||
      thread->name[0] == '\0' || strlen(thread->name) > 100U ||
...
static void dcc_app_send_message_id_cb(dcc_client_t *client,
...
static dcc_rest_response_t
dcc_app_result_response(const dcc_rest_result_t *result) {
...
static void dcc_app_send_thread_created_result(dcc_client_t *client,
                                               const dcc_rest_result_t *result,
                                               void *user_data) {
  (void)client;
  dcc_app_message_thread_state_t *state =
      (dcc_app_message_thread_state_t *)user_data;
  if (state == NULL) {
    return;
  }

  dcc_rest_response_t response = dcc_app_result_response(result);
  dcc_snowflake_t thread_id = 0U;
  dcc_status_t status = dcc_app_response_id_status(&response, &thread_id);
  if (state->cb != NULL) {
    state->cb(state->app, &response, state->message_id, thread_id, status,
              state->user_data);
  }
  dcc_app_message_thread_state_free(state);
}

static void dcc_app_send_thread_message_result(dcc_client_t *client,
                                               const dcc_rest_result_t *result,
                                               void *user_data) {
  (void)client;
  dcc_app_message_thread_state_t *state =
  ...
  options.callback = dcc_app_send_thread_created_result;
  options.user_data = state;
  status = dcc_rest_create_thread_from_message(dcc_app_client(state->app),
                                               state->channel_id, message_id,
                                               &state->thread, &options, NULL);
  if (status != DCC_OK) {
    if (state->cb != NULL) {
      state->cb(state->app, &response, message_id, 0U, status,
                state->user_data);
    }
    dcc_app_message_thread_state_free(state);
  }
...
static void dcc_app_managed_message_store_publish_cb(
  ...
  if (state->cb != NULL) {
    state->cb(client, result, state->user_data);
  }
  free(state);
}

dcc_status_t dcc_app_send(dcc_app_t *app, dcc_snowflake_t channel_id,
                          const dcc_message_builder_t *message, dcc_rest_cb cb,
                          void *user_data) {
  ...
  DCC_ENDPOINT_LEGACY_RETURN(cb, user_data, dcc_rest_create_message,
                             dcc_app_client(app), channel_id, &payload);
}

dcc_status_t dcc_app_send_with_id(dcc_app_t *app, dcc_snowflake_t channel_id,
  ...
  dcc_status_t status =
      dcc_app_send(app, channel_id, message, dcc_app_send_message_id_cb, state);
  if (status != DCC_OK) {
    free(state);
  }
  return status;
...
dcc_status_t dcc_app_send_with_thread(dcc_app_t *app,
                                      dcc_snowflake_t channel_id,
                                      const dcc_message_builder_t *message,
                                      const dcc_thread_params_t *thread,
                                      dcc_app_message_thread_cb cb,
  ...
  if (status == DCC_OK) {
    ...
    dcc_rest_message_payload_init(&payload, message);
    dcc_rest_call_options_t options = DCC_REST_CALL_OPTIONS_INIT;
    options.callback = dcc_app_send_thread_message_result;
    options.user_data = state;
    status = dcc_rest_create_message(dcc_app_client(app), channel_id, &payload,
                                     &options, NULL);
  }
  if (status != DCC_OK) {
    dcc_app_message_thread_state_free(state);
...
dcc_status_t dcc_app_send_with_thread_name(dcc_app_t *app,
                                           dcc_snowflake_t channel_id,
                                           const dcc_message_builder_t *message,
  ...
  return dcc_app_send_with_thread(app, channel_id, message, &thread, cb,
                                  user_data);
}

dcc_status_t dcc_app_send_text(dcc_app_t *app, dcc_snowflake_t channel_id,
                               const char *content, dcc_rest_cb cb,
                               void *user_data) {
  if (content == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
...
dcc_status_t dcc_app_send_text_with_id(dcc_app_t *app,
                                       dcc_snowflake_t channel_id,
                                       const char *content,
                                       dcc_app_message_id_cb cb,
                                       void *user_data) {
...
dcc_status_t
dcc_app_send_text_with_thread(dcc_app_t *app, dcc_snowflake_t channel_id,
                              const char *content, const char *thread_name,
                              dcc_app_message_thread_cb cb, void *user_data) {
  if (content == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
  dcc_message_builder_t message = {
  ...
  return dcc_app_send_with_thread_name(app, channel_id, &message, thread_name,
                                       cb, user_data);
}

dcc_status_t dcc_app_send_json(dcc_app_t *app, dcc_snowflake_t channel_id,
  ...
  return status == DCC_OK
             ? dcc_endpoint_submit_legacy_raw(dcc_app_client(app),
                                              DCC_REST_POST, path, NULL,
...
dcc_status_t dcc_app_managed_message_publish_latest(
    dcc_app_t *app, const dcc_managed_message_options_t *options,
    dcc_managed_message_publish_fn cb, void *user_data) {
  if (app == NULL || options == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
  return dcc_managed_message_publish_latest(dcc_app_client(app), options, cb,
                                            user_data);
}

dcc_status_t dcc_app_managed_message_publish_latest_store(
    dcc_app_t *app, const char *key, dcc_snowflake_t channel_id,
    const dcc_message_builder_t *message, dcc_managed_message_publish_fn cb,
    void *user_data) {
  if (app == NULL || key == NULL || key[0] == '\0' || channel_id == 0U ||
      message == NULL) {
    return DCC_ERR_INVALID_ARG;
  }
  ...
  dcc_status_t status = dcc_app_managed_message_publish_latest(
      app, &options, dcc_app_managed_message_store_publish_cb, state);
  if (status != DCC_OK) {
    free(state);
  }
  return status;
