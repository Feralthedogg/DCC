...
static dcc_status_t dcc_modal_json_buffer_reserve(dcc_modal_json_buffer_t *buffer, size_t extra) {
...
dcc_status_t dcc_modal_json_append(dcc_modal_json_buffer_t *buffer, const void *data, size_t len) {
    ...
    dcc_status_t status = dcc_modal_json_buffer_reserve(buffer, len + 1U);
    if (status != DCC_OK) {
        return status;
    }
    if (len != 0) {
...
dcc_status_t dcc_modal_json_append_cstr(dcc_modal_json_buffer_t *buffer, const char *value) {
    ...
    return dcc_modal_json_append(buffer, value, strlen(value));
}
