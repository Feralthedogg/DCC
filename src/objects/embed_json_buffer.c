...
void dcc_embed_json_buffer_init_count(dcc_embed_json_buffer_t *buffer) {
    if (buffer != NULL) {
        *buffer = (dcc_embed_json_buffer_t){0};
...
static dcc_status_t dcc_embed_json_buffer_reserve(dcc_embed_json_buffer_t *buffer, size_t extra) {
...
dcc_status_t dcc_embed_json_append(dcc_embed_json_buffer_t *buffer, const void *data, size_t len) {
    ...
    dcc_status_t status = dcc_embed_json_buffer_reserve(buffer, len + 1U);
    if (status != DCC_OK) {
        return status;
    }
    if (len != 0) {
...
dcc_status_t dcc_embed_json_append_cstr(dcc_embed_json_buffer_t *buffer, const char *value) {
    ...
    return dcc_embed_json_append(buffer, value, strlen(value));
}
