    ...
    for (const unsigned char *p = (const unsigned char *)value; *p != '\0'; ++p) {
        ...
        switch (c) {
            ...
            case '\\':
                ...
                break;
            case '\b':
                status = dcc_component_json_append_cstr(buffer, "\\b");
                break;
            case '\f':
                status = dcc_component_json_append_cstr(buffer, "\\f");
                break;
            case '\n':
                status = dcc_component_json_append_cstr(buffer, "\\n");
                break;
            case '\r':
                status = dcc_component_json_append_cstr(buffer, "\\r");
                break;
            case '\t':
                status = dcc_component_json_append_cstr(buffer, "\\t");
                break;
            default:
                if (c < 0x20U) {
                    ...
                    status = dcc_component_json_append(buffer, escaped, sizeof(escaped));
                } else {
                    status = dcc_component_json_append(buffer, &c, 1U);
                }
                break;
        }
        if (status != DCC_OK) {
