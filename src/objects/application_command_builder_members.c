...
dcc_status_t dcc_command_json_member_prefix(
    ...
    dcc_status_t status = dcc_command_json_append_cstr(buffer, *first ? "\"" : ",\"");
    if (status != DCC_OK) {
        return status;
    }
    status = dcc_command_json_append_cstr(buffer, name);
    if (status != DCC_OK) {
        return status;
    }
    *first = 0;
...
dcc_status_t dcc_command_json_append_string_member(
    ...
    dcc_status_t status = dcc_command_json_member_prefix(buffer, first, name);
    if (status != DCC_OK) {
        return status;
    }
    return dcc_command_json_append_escaped_string(buffer, value);
}
...
dcc_status_t dcc_command_json_append_raw_member(
    ...
    return dcc_command_json_append_cstr(buffer, value);
}
