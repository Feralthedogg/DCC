...
static dcc_status_t dcc_allowed_mentions_append_parse(
    ...
    dcc_status_t status = dcc_allowed_mentions_builder_abi_validate(builder, &view);
    if (status != DCC_OK) {
        return status;
    }
    status = dcc_message_json_member_prefix(buffer, first, "parse");
    if (status != DCC_OK) {
        return status;
    }
    ...
    if (status == DCC_OK &&
        dcc_builder_abi_view_has(&view, DCC_ALLOWED_MENTIONS_BUILDER_PRESENT_PARSE_EVERYONE) &&
        builder->parse_everyone) {
        if (!parse_first) {
            status = dcc_message_json_append_cstr(buffer, ",");
        }
        if (status == DCC_OK) {
...
dcc_status_t dcc_allowed_mentions_validate_for_json(const dcc_allowed_mentions_builder_t *builder) {
    dcc_builder_abi_view_t view;
    if (dcc_allowed_mentions_builder_abi_validate(builder, &view) != DCC_OK) {
        return DCC_ERR_INVALID_ARG;
    }
    const int has_users = dcc_builder_abi_view_has(
        &view, DCC_ALLOWED_MENTIONS_BUILDER_PRESENT_USERS
    );
    ...
    if ((has_users && builder->user_count != 0U && builder->users == NULL) ||
        ...
        (dcc_builder_abi_view_has(&view, DCC_ALLOWED_MENTIONS_BUILDER_PRESENT_PARSE_ROLES) &&
            builder->parse_roles && has_roles && builder->role_count != 0U)) {
        return DCC_ERR_INVALID_ARG;
    }
    return DCC_OK;
}
...
dcc_status_t dcc_allowed_mentions_append_json(
    ...
    int first = 1;
    dcc_builder_abi_view_t view;
    status = dcc_allowed_mentions_builder_abi_validate(builder, &view);
    if (status == DCC_OK) {
        status = dcc_allowed_mentions_append_parse(builder, buffer, &first);
    }
    if (status == DCC_OK &&
        dcc_builder_abi_view_has(&view, DCC_ALLOWED_MENTIONS_BUILDER_PRESENT_USERS)) {
    ...
    if (status == DCC_OK &&
        dcc_builder_abi_view_has(&view, DCC_ALLOWED_MENTIONS_BUILDER_PRESENT_ROLES)) {
        status = dcc_allowed_mentions_append_snowflakes(
            buffer,
            &first,
            "roles",
    ...
    if (status == DCC_OK &&
        dcc_builder_abi_view_has(&view, DCC_ALLOWED_MENTIONS_BUILDER_PRESENT_REPLIED_USER)) {
