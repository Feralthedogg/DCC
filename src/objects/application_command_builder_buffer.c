...
static dcc_status_t dcc_command_json_buffer_reserve(
...
dcc_status_t dcc_command_json_append(
    ...
    dcc_status_t status = dcc_command_json_buffer_reserve(buffer, len + 1U);
    if (status != DCC_OK) {
        return status;
    }
    if (len != 0) {
...
dcc_status_t dcc_command_json_append_cstr(
    ...
    return dcc_command_json_append(buffer, value, strlen(value));
}
