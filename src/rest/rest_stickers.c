...
dcc_status_t dcc_rest_get_sticker(
    ...
    char path[64];
    dcc_status_t status = dcc_rest_format_path(path, sizeof(path), "/stickers/%llu", (unsigned long long)sticker_id);
...
dcc_status_t dcc_rest_get_sticker_packs(
    ...
    return dcc_rest_request_method(client, DCC_REST_GET, "/sticker-packs", NULL, cb, user_data);
}
