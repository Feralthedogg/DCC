...
dcc_status_t dcc_rest_delete_webhook_message(
    ...
    status = dcc_rest_request_with_query(client, DCC_REST_DELETE, path, query, NULL, cb, user_data);
    free(path);
    return status;
}

dcc_status_t dcc_rest_delete_webhook_message_thread(
    dcc_client_t *client,
    dcc_snowflake_t webhook_id,
    const char *webhook_token,
    dcc_snowflake_t message_id,
    ...
    if (status == DCC_OK) {
        status = dcc_rest_delete_webhook_message(
            client,
            webhook_id,
            webhook_token,
            message_id,
