...
dcc_status_t dcc_rest_create_guild_command(
  ...
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT_OR_BEARER, DCC_ENDPOINT_AUDIT_REASON_DENIED,
                        DCC_REST_ROUTE_DPP_GUILD_COMMAND_CREATE, DCC_REST_POST);
  if (out_request != NULL) *out_request = NULL;
  char path[256];
  dcc_status_t status = dcc_rest_format_path(path, sizeof(path), DCC_REST_ROUTE_DPP_GUILD_COMMAND_CREATE, (unsigned long long)application_id, (unsigned long long)guild_id);
  char *body = NULL;
  if (status == DCC_OK) status = dcc_task9_command_body(params, 1, 1, &body);
  if (status != DCC_OK) { free(body); return status; }
  status = dcc_task9_submit_owned(client, "dcc_rest_create_guild_command", DCC_REST_POST,
