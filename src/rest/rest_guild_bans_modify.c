...
dcc_status_t dcc_rest_create_guild_ban(dcc_client_t *client,
  ...
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT,
                        ...
                        DCC_REST_ROUTE_DPP_GUILD_BAN_ADD, DCC_REST_PUT);
  ...
  char *json = NULL;
  status = dcc_task8_build_ban_create(body, &json);
  char path[112];
  if (status == DCC_OK)
    status = dcc_rest_format_path(
        path, sizeof(path), DCC_REST_ROUTE_DPP_GUILD_BAN_ADD,
        (unsigned long long)guild_id, (unsigned long long)user_id);
  return dcc_task8_submit_built(status, client, "dcc_rest_create_guild_ban",
                                DCC_REST_PUT, path, json, &resolved,
                                out_request);
}
dcc_status_t dcc_rest_delete_guild_ban(dcc_client_t *client,
                                       dcc_snowflake_t guild_id,
                                       dcc_snowflake_t user_id,
                                       const dcc_rest_call_options_t *options,
                                       dcc_rest_request_t **out_request) {
  ...
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT,
                        DCC_ENDPOINT_AUDIT_REASON_ALLOWED,
                        DCC_REST_ROUTE_DPP_GUILD_BAN_DELETE, DCC_REST_DELETE);
  dcc_rest_call_options_t resolved;
  dcc_status_t status = dcc_task8_prepare(
      client, options, DCC_ENDPOINT_AUTH_POLICY_BOT,
      DCC_ENDPOINT_AUDIT_REASON_ALLOWED, out_request, &resolved);
  ...
  char path[112];
  status = dcc_rest_format_path(
      path, sizeof(path), DCC_REST_ROUTE_DPP_GUILD_BAN_DELETE,
      (unsigned long long)guild_id, (unsigned long long)user_id);
  return dcc_task8_submit_built(status, client, "dcc_rest_delete_guild_ban",
