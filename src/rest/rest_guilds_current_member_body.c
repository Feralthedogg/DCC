#include "internal/rest/dcc_rest_buffer_internal.h"
#include "internal/rest/dcc_rest_guild_current_member_internal.h"
#include "internal/rest/dcc_rest_json_internal.h"

dcc_status_t dcc_rest_build_current_guild_member_body(
    const dcc_current_guild_member_params_t *params,
    char **out
) {
    if (out == NULL) {
        return DCC_ERR_INVALID_ARG;
    }
    *out = NULL;
    if (params == NULL || params->size < sizeof(*params) || params->guild_id == 0) {
        return DCC_ERR_INVALID_ARG;
    }

    dcc_rest_buffer_t body = {0};
    dcc_status_t status = dcc_rest_buffer_append_cstr(&body, "{");
    int first = 1;
    if (status == DCC_OK) {
        status = dcc_rest_json_append_nullable_string_member(&body, &first, "nick", params->nick);
    }
    if (status == DCC_OK) {
        status = dcc_rest_json_append_nullable_string_member(&body, &first, "banner", params->banner);
    }
    if (status == DCC_OK) {
        status = dcc_rest_json_append_nullable_string_member(&body, &first, "avatar", params->avatar);
    }
    if (status == DCC_OK) {
        status = dcc_rest_json_append_nullable_string_member(&body, &first, "bio", params->bio);
    }
    if (status == DCC_OK) {
        status = dcc_rest_buffer_append_cstr(&body, "}");
    }
    if (status == DCC_OK) {
        *out = body.data;
        return DCC_OK;
    }
    dcc_rest_buffer_deinit(&body);
    return status;
}

dcc_status_t dcc_rest_build_member_nickname_body(
    const char *nick,
    char **out
) {
    if (out == NULL) {
        return DCC_ERR_INVALID_ARG;
    }
    *out = NULL;

    dcc_rest_buffer_t body = {0};
    dcc_status_t status = dcc_rest_buffer_append_cstr(&body, "{");
    int first = 1;
    if (status == DCC_OK) {
        status = dcc_rest_json_append_nullable_string_member(&body, &first, "nick", nick);
    }
    if (status == DCC_OK) {
        status = dcc_rest_buffer_append_cstr(&body, "}");
    }
    if (status == DCC_OK) {
        *out = body.data;
        return DCC_OK;
    }
    dcc_rest_buffer_deinit(&body);
    return status;
}
