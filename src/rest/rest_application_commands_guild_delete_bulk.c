...
dcc_status_t dcc_rest_bulk_overwrite_guild_commands(
  ...
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT_OR_BEARER, DCC_ENDPOINT_AUDIT_REASON_DENIED,
                        DCC_REST_ROUTE_DPP_GUILD_BULK_COMMAND_CREATE, DCC_REST_PUT);
  if (out_request != NULL) *out_request = NULL;
  char path[256];
  dcc_status_t status = dcc_rest_format_path(path, sizeof(path), DCC_REST_ROUTE_DPP_GUILD_BULK_COMMAND_CREATE, (unsigned long long)application_id, (unsigned long long)guild_id);
  char *body = NULL;
  if (status == DCC_OK) status = dcc_task9_command_array_body(commands, command_count, 1, &body);
  if (status != DCC_OK) { free(body); return status; }
  status = dcc_task9_submit_owned(client, "dcc_rest_bulk_overwrite_guild_commands", DCC_REST_PUT,
...
dcc_status_t dcc_rest_delete_guild_command(
  ...
  (void)DCC_ENDPOINT_PATH_PUBLIC;
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT_OR_BEARER, DCC_ENDPOINT_AUDIT_REASON_DENIED,
                        DCC_REST_ROUTE_DPP_GUILD_COMMAND_DELETE, DCC_REST_DELETE);
  if (out_request != NULL) *out_request = NULL;
  char path[256];
  dcc_status_t status = dcc_rest_format_path(path, sizeof(path), DCC_REST_ROUTE_DPP_GUILD_COMMAND_DELETE, (unsigned long long)application_id, (unsigned long long)guild_id, (unsigned long long)command_id);
  if (status != DCC_OK) { return status; }
  status = dcc_task9_submit_owned(client, "dcc_rest_delete_guild_command", DCC_REST_DELETE,
