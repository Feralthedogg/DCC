...
dcc_status_t
dcc_rest_create_guild_role(dcc_client_t *client, dcc_snowflake_t guild_id,
  ...
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT,
                        ...
                        DCC_REST_ROUTE_DPP_ROLE_CREATE, DCC_REST_POST);
  ...
  char *json = NULL;
  status = dcc_task8_build_role_create(body, &json);
  char path[80];
  if (status == DCC_OK)
    status =
        dcc_rest_format_path(path, sizeof(path), DCC_REST_ROUTE_DPP_ROLE_CREATE,
                             (unsigned long long)guild_id);
  return dcc_task8_submit_built(status, client, "dcc_rest_create_guild_role",
