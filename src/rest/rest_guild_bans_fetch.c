...
dcc_status_t dcc_rest_get_guild_bans(
    dcc_client_t *client,
    ...
    char path[72];
    dcc_status_t status = dcc_rest_format_path(path, sizeof(path), "/guilds/%llu/bans", (unsigned long long)guild_id);
    return status == DCC_OK ? dcc_rest_request_with_query(client, DCC_REST_GET, path, query, NULL, cb, user_data) : status;
}

dcc_status_t dcc_rest_get_guild_bans_page(
    dcc_client_t *client,
    dcc_snowflake_t guild_id,
    ...
    if (status == DCC_OK) {
        status = dcc_rest_get_guild_bans(client, guild_id, query.data, cb, user_data);
    }
    dcc_rest_buffer_deinit(&query);
    return status;
}

dcc_status_t dcc_rest_get_guild_ban(
    dcc_client_t *client,
    dcc_snowflake_t guild_id,
