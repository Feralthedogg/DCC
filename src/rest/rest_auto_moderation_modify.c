...
dcc_status_t dcc_rest_modify_auto_moderation_rule(
  ...
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT, DCC_ENDPOINT_AUDIT_REASON_ALLOWED,
                        DCC_REST_ROUTE_DPP_AUTOMOD_RULE_EDIT, DCC_REST_PATCH);
  if (out_request != NULL) *out_request = NULL;
  char path[256];
  dcc_status_t status = dcc_rest_format_path(path, sizeof(path), DCC_REST_ROUTE_DPP_AUTOMOD_RULE_EDIT, (unsigned long long)guild_id, (unsigned long long)rule_id);
  char *body = NULL;
  if (status == DCC_OK) status = dcc_rest_build_auto_moderation_rule_body(params, 1, &body);
  if (status != DCC_OK) { free(body); return status; }
  status = dcc_task9_submit_owned(client, "dcc_rest_modify_auto_moderation_rule", DCC_REST_PATCH,
