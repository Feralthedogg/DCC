...
dcc_status_t dcc_rest_get_guild_roles(dcc_client_t *client,
  ...
  DCC_ENDPOINT_CONTRACT(DCC_ENDPOINT_AUTH_POLICY_BOT,
                        ...
                        DCC_REST_ROUTE_DPP_ROLES_GET, DCC_REST_GET);
  ...
  char path[80];
  status =
      dcc_rest_format_path(path, sizeof(path), DCC_REST_ROUTE_DPP_ROLES_GET,
                           (unsigned long long)guild_id);
  return status == DCC_OK ? dcc_task8_submit(client, "dcc_rest_get_guild_roles",
                                             DCC_REST_GET, path, NULL,
