...
dcc_status_t dcc_rest_get_guild_onboarding(
    ...
    char path[88];
    dcc_status_t status = dcc_rest_format_path(path, sizeof(path), "/guilds/%llu/onboarding", (unsigned long long)guild_id);
...
dcc_status_t dcc_rest_modify_guild_onboarding_params(
    dcc_client_t *client,
    ...
    if (status == DCC_OK) {
        status = dcc_rest_modify_guild_onboarding(client, params->guild_id, body, cb, user_data);
    }
    free(body);
    return status;
}
