...
#define CHECK_PREFIX(type_, first_field_) \
    _Static_assert(offsetof(type_, size) == offsetof(builder_v2_prefix_t, size), \
                   #type_ " size offset"); \
    _Static_assert(offsetof(type_, version) == offsetof(builder_v2_prefix_t, version), \
                   #type_ " version offset"); \
    _Static_assert(offsetof(type_, present) == offsetof(builder_v2_prefix_t, present), \
...
#define OPTION_HISTORICAL_STRIDE \
    ALIGN_UP_TO( \
        offsetof(dcc_application_command_option_builder_t, name_localizations_json), \
...
_Static_assert(CHOICE_HISTORICAL_STRIDE % _Alignof(dcc_autocomplete_choice_t) == 0U,
...
_Static_assert(CHOICE_HISTORICAL_STRIDE >= FIELD_END(dcc_autocomplete_choice_t, name) &&
...
_Static_assert(OPTION_HISTORICAL_STRIDE %
                       _Alignof(dcc_application_command_option_builder_t) == 0U,
...
_Static_assert(COMMAND_HISTORICAL_STRIDE %
                       _Alignof(dcc_application_command_builder_t) == 0U,
               "command historical stride alignment");
_Static_assert(COMMAND_HISTORICAL_STRIDE < sizeof(dcc_application_command_builder_t),
               "command historical stride must be smaller");
_Static_assert(COMMAND_HISTORICAL_STRIDE >=
                       FIELD_END(dcc_application_command_builder_t, name) &&
                   COMMAND_HISTORICAL_STRIDE >=
                       FIELD_END(dcc_application_command_builder_t, description) &&
                   COMMAND_HISTORICAL_STRIDE >=
...
_Static_assert(EMBED_HISTORICAL_STRIDE >= FIELD_END(dcc_embed_builder_t, title) &&
...
static int check_initializer_contracts(void) {
    ...
    dcc_autocomplete_choice_t named_choice = DCC_AUTOCOMPLETE_CHOICE_NAMED_INIT("name");
...
#define CHECK_INITIALIZED(value_, version_, label_) \
    CHECK((value_).size == sizeof(value_) && (value_).version == (version_) && \
              (value_).present == 0U, \
          (label_))
    CHECK_INITIALIZED(message, DCC_MESSAGE_BUILDER_VERSION, "message constant initializer");
    CHECK_INITIALIZED(poll, DCC_POLL_BUILDER_VERSION, "poll constant initializer");
    CHECK_INITIALIZED(
        mentions, DCC_ALLOWED_MENTIONS_BUILDER_VERSION,
        "allowed mentions constant initializer"
    );
    CHECK_INITIALIZED(embed, DCC_EMBED_BUILDER_VERSION, "embed constant initializer");
    CHECK_INITIALIZED(modal, DCC_MODAL_BUILDER_VERSION, "modal constant initializer");
    CHECK_INITIALIZED(
        choice, DCC_AUTOCOMPLETE_CHOICE_VERSION,
    ...
    CHECK_INITIALIZED(
        autocomplete, DCC_AUTOCOMPLETE_BUILDER_VERSION,
        "autocomplete builder constant initializer"
    );
    CHECK_INITIALIZED(
        option, DCC_APPLICATION_COMMAND_OPTION_BUILDER_VERSION,
        "command option constant initializer"
    );
    CHECK_INITIALIZED(
        command, DCC_APPLICATION_COMMAND_BUILDER_VERSION,
...
#undef CHECK_INITIALIZED
    ...
    CHECK(message.content == NULL && message.nonce == NULL &&
              ...
              message.allowed_mentions_json == NULL && message.embeds_json == NULL &&
              message.components_json == NULL && message.attachments_json == NULL &&
              message.poll_json == NULL && message.embeds == NULL &&
              message.embeds_count == 0U && message.components == NULL &&
              message.components_count == 0U && message.components_v2 == NULL &&
    ...
    CHECK(poll.question.text == NULL && poll.question.emoji.name == NULL &&
              poll.question.emoji.id == 0U && poll.question.emoji.animated == 0U &&
    ...
    CHECK(embed.title == NULL && embed.type == NULL && embed.description == NULL &&
              embed.url == NULL && embed.timestamp == NULL &&
              embed.footer.text == NULL && embed.footer.icon_url == NULL &&
              embed.image.url == NULL && embed.thumbnail.url == NULL &&
              embed.author.name == NULL && embed.author.url == NULL &&
              embed.author.icon_url == NULL && embed.fields == NULL &&
              embed.field_count == 0U && embed.color == 0U,
    ...
    CHECK(option.name == NULL && option.description == NULL && option.type == 0U &&
              option.name_localizations_json == NULL &&
    ...
    CHECK(command.name == NULL && command.name_localizations_json == NULL &&
              ...
              command.options_count == 0U && command.integration_types_json == NULL &&
              command.contexts_json == NULL && command.type == 0U &&
              command.default_member_permissions == 0U && command.dm_permission == 0U &&
              command.nsfw == 0U && command.default_member_permissions_null == 0U,
    ...
    CHECK(message.size == sizeof(message) && message.present == 0U, "message init function");
    ...
    CHECK(embed.size == sizeof(embed) && embed.present == 0U, "embed init function");
    CHECK(modal.size == sizeof(modal) && modal.present == 0U, "modal init function");
    ...
    CHECK(option.size == sizeof(option) && option.present == 0U, "option init function");
    CHECK(command.size == sizeof(command) && command.present == 0U, "command init function");
...
#define EXERCISE_ABI_MATRIX(type_, value_, validate_, version_, first_bit_, label_) \
    do { \
...
static int check_all_type_abi_matrix(void) {
    ...
    EXERCISE_ABI_MATRIX(
        dcc_message_builder_t, message, dcc_message_builder_abi_validate,
        DCC_MESSAGE_BUILDER_VERSION, DCC_MESSAGE_BUILDER_PRESENT_CONTENT, "message"
    );
    EXERCISE_ABI_MATRIX(
        dcc_poll_builder_t, poll, dcc_poll_builder_abi_validate,
        DCC_POLL_BUILDER_VERSION, DCC_POLL_BUILDER_PRESENT_QUESTION, "poll"
    );
    EXERCISE_ABI_MATRIX(
        dcc_allowed_mentions_builder_t, mentions,
        dcc_allowed_mentions_builder_abi_validate,
        DCC_ALLOWED_MENTIONS_BUILDER_VERSION,
        DCC_ALLOWED_MENTIONS_BUILDER_PRESENT_USERS, "allowed mentions"
    );
    EXERCISE_ABI_MATRIX(
        dcc_embed_builder_t, embed, dcc_embed_builder_abi_validate,
        DCC_EMBED_BUILDER_VERSION, DCC_EMBED_BUILDER_PRESENT_TITLE, "embed"
    );
    EXERCISE_ABI_MATRIX(
        dcc_modal_builder_t, modal, dcc_modal_builder_abi_validate,
        DCC_MODAL_BUILDER_VERSION, DCC_MODAL_BUILDER_PRESENT_CUSTOM_ID, "modal"
    );
    EXERCISE_ABI_MATRIX(
    ...
    EXERCISE_ABI_MATRIX(
        dcc_autocomplete_builder_t, autocomplete,
        dcc_autocomplete_builder_abi_validate,
        DCC_AUTOCOMPLETE_BUILDER_VERSION,
    ...
    EXERCISE_ABI_MATRIX(
        ...
        DCC_APPLICATION_COMMAND_OPTION_BUILDER_PRESENT_NAME, "command option"
    );
    EXERCISE_ABI_MATRIX(
        dcc_application_command_builder_t, command,
...
static int check_historical_poison_serialization(void) {
    ...
    dcc_autocomplete_builder_t autocomplete;
    dcc_autocomplete_builder_init(&autocomplete);
    CHECK(dcc_autocomplete_builder_set_choices(&autocomplete, &choice, 1U) == DCC_OK,
    ...
    json = NULL;
    CHECK(dcc_application_command_option_builder_build_array_json(&option, 1U, &json) ==
              DCC_OK &&
...
static int check_presence_and_zero_semantics(void) {
    ...
    dcc_autocomplete_builder_init(&autocomplete);
    CHECK(dcc_autocomplete_builder_set_choices(&autocomplete, &choice, 1U) == DCC_OK &&
              dcc_autocomplete_builder_build_json(&autocomplete, &json) == DCC_OK &&
              json_has(json, "\"value\":0"),
          "choice present zero value");
    dcc_autocomplete_builder_json_free(json);
    CHECK(dcc_autocomplete_builder_set_choices(&autocomplete, NULL, 0U) == DCC_OK &&
              (autocomplete.present & DCC_AUTOCOMPLETE_BUILDER_PRESENT_CHOICES) == 0U,
    ...
    dcc_application_command_builder_t command;
    dcc_application_command_builder_init(&command);
    CHECK(dcc_application_command_builder_set_default_member_permissions(&command, 0U) ==
              DCC_OK &&
              dcc_application_command_builder_set_dm_permission(&command, 0U) == DCC_OK &&
              dcc_application_command_builder_set_nsfw(&command, 0U) == DCC_OK &&
              dcc_application_command_builder_build_json(&command, &json) == DCC_OK &&
...
static int check_all_current_setters(void) {
    ...
    CHECK(dcc_message_builder_set_content(&message, "content") == DCC_OK &&
              ...
              dcc_message_builder_set_sticker_ids(&message, &sticker, 1U) == DCC_OK &&
              dcc_message_builder_set_message_reference_json(
                  &message, "{\"message_id\":\"1\"}"
              ) == DCC_OK &&
              dcc_message_builder_set_allowed_mentions_json(&message, "{}") == DCC_OK &&
              dcc_message_builder_set_allowed_mentions(&message, &mentions) == DCC_OK &&
              dcc_message_builder_set_embeds(&message, &message_embed, 1U) == DCC_OK &&
              dcc_message_builder_set_components(&message, &component, 1U) == DCC_OK &&
              ...
              dcc_message_builder_set_components_v2_json(&message, "[]") == DCC_OK &&
    ...
    CHECK(dcc_message_builder_set_content(&message, NULL) == DCC_OK &&
              dcc_message_builder_set_nonce(&message, NULL) == DCC_OK &&
              dcc_message_builder_set_sticker_ids(&message, NULL, 0U) == DCC_OK &&
              dcc_message_builder_set_message_reference_json(&message, NULL) == DCC_OK &&
              dcc_message_builder_set_allowed_mentions_json(&message, NULL) == DCC_OK &&
              ...
              dcc_message_builder_set_components_v2_json(&message, NULL) == DCC_OK &&
    ...
    dcc_embed_builder_t embed;
    dcc_embed_builder_init(&embed);
    CHECK(dcc_embed_builder_set_title(&embed, "title") == DCC_OK &&
              ...
              dcc_embed_builder_set_timestamp(&embed, "2026-08-11T00:00:00Z") == DCC_OK &&
              ...
              dcc_embed_builder_set_author(&embed, "author", "url", "icon") == DCC_OK &&
              dcc_embed_builder_set_fields(&embed, &field, 1U) == DCC_OK,
          "every current embed setter accepts a valid builder");
    const uint64_t embed_bits = DCC_EMBED_BUILDER_PRESENT_TITLE |
    ...
    CHECK(dcc_embed_builder_set_title(&embed, NULL) == DCC_OK &&
              ...
              (embed.present & (embed_bits & ~DCC_EMBED_BUILDER_PRESENT_COLOR)) == 0U,
    ...
    CHECK(dcc_modal_builder_set_custom_id(&modal, "modal") == DCC_OK &&
              ...
              dcc_modal_builder_set_components(&modal, &component, 1U) == DCC_OK &&
              dcc_modal_builder_set_components_v2(&modal, &component_v2, 1U) == DCC_OK &&
    ...
    CHECK((choice.present & DCC_AUTOCOMPLETE_CHOICE_PRESENT_NAME) != 0U &&
              ...
              dcc_autocomplete_choice_set_name_localizations_json(&choice, NULL) == DCC_OK &&
    ...
    CHECK(dcc_autocomplete_choice_set_string_value(&choice, "value") == DCC_OK &&
              ...
              (autocomplete.present & DCC_AUTOCOMPLETE_BUILDER_PRESENT_CHOICES) != 0U &&
              dcc_autocomplete_builder_set_choices(&autocomplete, NULL, 0U) == DCC_OK &&
              autocomplete.present == 0U,
    ...
    dcc_application_command_builder_t command;
    dcc_application_command_builder_init(&command);
    CHECK(dcc_application_command_builder_set_name(&command, "command") == DCC_OK &&
              ...
              dcc_application_command_builder_set_description_localizations_json(
                  &command, "{\"ko\":\"description\"}"
              ) == DCC_OK &&
              dcc_application_command_builder_set_type(
                  &command, DCC_APPLICATION_COMMAND_CHAT_INPUT
              ) == DCC_OK &&
              dcc_application_command_builder_set_options_json(&command, "[]") == DCC_OK &&
              (command.present & DCC_APPLICATION_COMMAND_BUILDER_PRESENT_OPTIONS_JSON) !=
                  0U &&
              dcc_application_command_builder_set_options(
                  &command, &command_option, 1U
              ) == DCC_OK &&
              (command.present & DCC_APPLICATION_COMMAND_BUILDER_PRESENT_OPTIONS) != 0U &&
              (command.present & DCC_APPLICATION_COMMAND_BUILDER_PRESENT_OPTIONS_JSON) == 0U &&
              ...
              dcc_application_command_builder_set_contexts_json(&command, "[0]") == DCC_OK,
          "every current command setter accepts a valid builder");
    CHECK(dcc_application_command_builder_set_name(&command, NULL) == DCC_OK &&
              dcc_application_command_builder_set_name_localizations_json(
                  &command, NULL
              ) == DCC_OK &&
              dcc_application_command_builder_set_description(&command, NULL) == DCC_OK &&
              dcc_application_command_builder_set_description_localizations_json(
                  &command, NULL
              ) == DCC_OK &&
              dcc_application_command_builder_set_options(&command, NULL, 0U) == DCC_OK &&
              dcc_application_command_builder_set_options_json(&command, NULL) == DCC_OK &&
              dcc_application_command_builder_set_integration_types_json(
                  &command, NULL
              ) == DCC_OK &&
              dcc_application_command_builder_set_contexts_json(&command, NULL) == DCC_OK &&
              (command.present &
...
#define SNAPSHOT_AND_REJECT(value_, expression_, label_) \
    ...
    SNAPSHOT_AND_REJECT(
        message, dcc_message_builder_set_content(&message, "no-write"),
    ...
    CHECK(dcc_message_builder_set_content(&guarded.value, "no-write") ==
              DCC_ERR_INVALID_ARG &&
              memcmp(guarded_value_before, &guarded.value, sizeof(guarded.value)) == 0 &&
    ...
    SNAPSHOT_AND_REJECT(
        embed, dcc_embed_builder_set_title(&embed, "no-write"),
    ...
    modal.version++;
    SNAPSHOT_AND_REJECT(
        modal, dcc_modal_builder_set_title(&modal, "no-write"),
    ...
    SNAPSHOT_AND_REJECT(
        choice, dcc_autocomplete_choice_set_integer_value(&choice, 7),
...
static dcc_autocomplete_choice_t *choice_record(
    aligned_builder_bytes_t *storage,
    size_t stride,
    size_t index
) {
...
static dcc_application_command_option_builder_t *option_record(
    aligned_builder_bytes_t *storage,
    size_t stride,
    size_t index
) {
    return (dcc_application_command_option_builder_t *)(storage->bytes + stride * index);
}

static dcc_application_command_builder_t *command_record(
    aligned_builder_bytes_t *storage,
    size_t stride,
    size_t index
) {
...
static dcc_embed_builder_t *embed_record(
    aligned_builder_bytes_t *storage,
    size_t stride,
    size_t index
) {
...
static void init_historical_choice(
    dcc_autocomplete_choice_t *choice,
    size_t stride,
    const char *name,
    const char *value
...
static void init_historical_option(
    dcc_application_command_option_builder_t *option,
    size_t stride,
    const char *name,
...
static int check_versioned_stride_arrays(void) {
    ...
    dcc_autocomplete_choice_t *choice0 = choice_record(
        &choices_storage, CHOICE_HISTORICAL_STRIDE, 0U
    );
    dcc_autocomplete_choice_t *choice1 = choice_record(
        &choices_storage, CHOICE_HISTORICAL_STRIDE, 1U
    );
    ...
    dcc_autocomplete_builder_t autocomplete;
    dcc_autocomplete_builder_init(&autocomplete);
    CHECK(dcc_autocomplete_builder_set_choices(&autocomplete, choice0, 2U) == DCC_OK,
    ...
    choice1->size = CHOICE_HISTORICAL_STRIDE + _Alignof(dcc_autocomplete_choice_t);
    json = NULL;
    CHECK(dcc_autocomplete_builder_build_json(&autocomplete, &json) == DCC_ERR_INVALID_ARG &&
              json == NULL,
    ...
    dcc_application_command_option_builder_t *option0 = option_record(
        &options_storage, OPTION_HISTORICAL_STRIDE, 0U
    );
    dcc_application_command_option_builder_t *option1 = option_record(
        &options_storage, OPTION_HISTORICAL_STRIDE, 1U
    );
    init_historical_option(
        option0, OPTION_HISTORICAL_STRIDE, "first", "First option",
        DCC_APPLICATION_COMMAND_OPTION_STRING
    );
    init_historical_option(
        option1, OPTION_HISTORICAL_STRIDE, "second", "Second option",
        DCC_APPLICATION_COMMAND_OPTION_INTEGER
    );
    json = NULL;
    CHECK(dcc_application_command_option_builder_build_array_json(option0, 2U, &json) ==
              DCC_OK &&
    ...
    CHECK(dcc_application_command_builder_set_name(&nested_command, "nested") == DCC_OK &&
              dcc_application_command_builder_set_description(
                  &nested_command, "Nested stride"
              ) == DCC_OK &&
              dcc_application_command_builder_set_type(
                  &nested_command, DCC_APPLICATION_COMMAND_CHAT_INPUT
              ) == DCC_OK &&
              dcc_application_command_builder_set_options(
                  &nested_command, option0, 2U
              ) == DCC_OK,
          "build command with historical recursive options");
    json = NULL;
    CHECK(dcc_application_command_builder_build_json(&nested_command, &json) == DCC_OK &&
              json_has(json, "\"name\":\"first\"") &&
              json_has(json, "\"name\":\"second\""),
    ...
    dcc_application_command_builder_t *command0 = command_record(
        &commands_storage, COMMAND_HISTORICAL_STRIDE, 0U
    );
    dcc_application_command_builder_t *command1 = command_record(
        &commands_storage, COMMAND_HISTORICAL_STRIDE, 1U
    );
    ...
    dcc_embed_builder_t *embed0 = embed_record(
        &embeds_storage, EMBED_HISTORICAL_STRIDE, 0U
    );
    dcc_embed_builder_t *embed1 = embed_record(
        &embeds_storage, EMBED_HISTORICAL_STRIDE, 1U
    );
    for (size_t i = 0U; i < 2U; ++i) {
    ...
    CHECK(dcc_application_command_builder_build_array_json(
              &overflow_command, SIZE_MAX / stride, &json
          ) == DCC_ERR_INVALID_ARG,
          "array address span overflow");
    _Alignas(dcc_application_command_builder_t) unsigned char misaligned_storage[
...
static void init_filter_output(
    aligned_builder_bytes_t *storage,
    size_t stride,
    size_t count,
...
static dcc_status_t matching_intercept(
    dcc_client_t *client,
    const char *method,
    const char *path,
    const void *body,
    ...
    (void)path;
    (void)content_type;
    matching_intercept_state_t *state = (matching_intercept_state_t *)intercept_user_data;
    if (state != NULL) {
        state->calls++;
...
static int check_autocomplete_filter_stride_and_atomicity(void) {
    ...
    dcc_autocomplete_choice_t *input0 = choice_record(
        &inputs, CHOICE_HISTORICAL_STRIDE, 0U
    );
    dcc_autocomplete_choice_t *input1 = choice_record(
        &inputs, CHOICE_HISTORICAL_STRIDE, 1U
    );
...
#undef CHECK_FILTER_REJECTS_UNCHANGED
    ...
    init_filter_output(&outputs, CHOICE_HISTORICAL_STRIDE, 2U, 0xD7);
    dcc_autocomplete_choice_t *output1 = choice_record(
        &outputs, CHOICE_HISTORICAL_STRIDE, 1U
    );
...
static void registry_fixture_init(registry_fixture_t *fixture, const char *command_name) {
    memset(fixture, 0, sizeof(*fixture));
    (void)snprintf(fixture->command_name, sizeof(fixture->command_name), "%s", command_name);
    (void)snprintf(
        fixture->command_description, sizeof(fixture->command_description),
        "Deep copied command"
    );
    (void)snprintf(
        fixture->command_name_localizations, sizeof(fixture->command_name_localizations),
        "{\"ko\":\"copied\"}"
    );
    (void)snprintf(fixture->option_name, sizeof(fixture->option_name), "group");
    (void)snprintf(
        fixture->option_description, sizeof(fixture->option_description), "Group option"
    );
    (void)snprintf(fixture->nested_name, sizeof(fixture->nested_name), "query");
    (void)snprintf(
        fixture->nested_description, sizeof(fixture->nested_description), "Query option"
    );
    (void)snprintf(fixture->choice_name, sizeof(fixture->choice_name), "Original choice");
    (void)snprintf(fixture->choice_value, sizeof(fixture->choice_value), "original-value");
    (void)snprintf(
        fixture->choice_localizations, sizeof(fixture->choice_localizations),
        "{\"ko\":\"original\"}"
    );
    (void)snprintf(fixture->integration_types, sizeof(fixture->integration_types), "[0]");
    ...
    (void)dcc_autocomplete_choice_set_string_value(
        &fixture->choices[0], fixture->choice_value
    );
    ...
    (void)dcc_application_command_builder_set_name(
        &fixture->command, fixture->command_name
    );
    ...
    (void)dcc_application_command_builder_set_options(
        &fixture->command, fixture->options, 2U
    );
    (void)dcc_application_command_builder_set_integration_types_json(
        &fixture->command, fixture->integration_types
    );
    (void)dcc_application_command_builder_set_contexts_json(
        &fixture->command, fixture->contexts
    );
}
...
static int check_registry_copy_lifetime_and_oom(void) {
    ...
    for (size_t fail_after = 0U; fail_after < 128U; ++fail_after) {
        ...
        dcc_status_t status = dcc_command_registry_add_builder(
            &registry, &fixture.command
        );
        ...
        CHECK(status == DCC_ERR_NOMEM && dcc_command_registry_count(&registry) == 0U,
    ...
    registry_fixture_t *lifetime = (registry_fixture_t *)malloc(sizeof(*lifetime));
    CHECK(lifetime != NULL, "allocate caller-owned registry fixture");
    registry_fixture_init(lifetime, "copy-lifetime");
    dcc_command_registry_t registry;
    dcc_command_registry_init(&registry);
    CHECK(dcc_command_registry_add_builder(&registry, &lifetime->command) == DCC_OK,
    ...
    CHECK(dcc_command_registry_add_builder(&registry, &fixture.command) == DCC_ERR_NOMEM &&
...
static int check_registry_historical_stride_normalization(void) {
    ...
    dcc_application_command_option_builder_t *option0 = option_record(
        &option_storage, OPTION_HISTORICAL_STRIDE, 0U
    );
    dcc_application_command_option_builder_t *option1 = option_record(
        &option_storage, OPTION_HISTORICAL_STRIDE, 1U
    );
    init_historical_option(
        option0, OPTION_HISTORICAL_STRIDE, "history-one", "History one",
        DCC_APPLICATION_COMMAND_OPTION_STRING
    );
    init_historical_option(
        option1, OPTION_HISTORICAL_STRIDE, "history-two", "History two",
        DCC_APPLICATION_COMMAND_OPTION_INTEGER
    );
    dcc_application_command_builder_t command;
    dcc_application_command_builder_init(&command);
    CHECK(dcc_application_command_builder_set_name(&command, "historical-options") ==
              DCC_OK &&
              dcc_application_command_builder_set_description(
                  &command, "Historical option stride"
              ) == DCC_OK &&
              dcc_application_command_builder_set_type(
                  &command, DCC_APPLICATION_COMMAND_CHAT_INPUT
              ) == DCC_OK &&
              dcc_application_command_builder_set_options(&command, option0, 2U) == DCC_OK &&
    ...
    dcc_autocomplete_choice_t *choice0 = choice_record(
        &choice_storage, CHOICE_HISTORICAL_STRIDE, 0U
    );
    dcc_autocomplete_choice_t *choice1 = choice_record(
        &choice_storage, CHOICE_HISTORICAL_STRIDE, 1U
    );
    init_historical_choice(
        choice0, CHOICE_HISTORICAL_STRIDE, choice_name0, choice_value0
    );
    init_historical_choice(
        choice1, CHOICE_HISTORICAL_STRIDE, choice_name1, choice_value1
    );
    dcc_application_command_option_builder_t choice_option;
    dcc_application_command_option_builder_init(&choice_option);
    choice_option.present = DCC_APPLICATION_COMMAND_OPTION_BUILDER_PRESENT_NAME |
        DCC_APPLICATION_COMMAND_OPTION_BUILDER_PRESENT_DESCRIPTION |
        DCC_APPLICATION_COMMAND_OPTION_BUILDER_PRESENT_TYPE |
        DCC_APPLICATION_COMMAND_OPTION_BUILDER_PRESENT_CHOICES;
    ...
    CHECK(dcc_application_command_builder_set_name(
              &choice_command, "historical-choices"
          ) == DCC_OK &&
              dcc_application_command_builder_set_description(
                  &choice_command, "Historical choice stride"
              ) == DCC_OK &&
              dcc_application_command_builder_set_type(
                  &choice_command, DCC_APPLICATION_COMMAND_CHAT_INPUT
              ) == DCC_OK &&
              dcc_application_command_builder_set_options(
                  &choice_command, &choice_option, 1U
              ) == DCC_OK &&
              dcc_command_registry_add_builder(&registry, &choice_command) == DCC_OK,
    ...
    char *json = NULL;
    CHECK(dcc_application_command_builder_build_json(
              &state->entries[0].command, &json
          ) == DCC_OK &&
...
static int check_contextual_command_validation(void) {
    ...
    dcc_command_registry_t registry;
    dcc_command_registry_init(&registry);
    CHECK(dcc_command_registry_add_builder(&registry, &partial) == DCC_ERR_INVALID_ARG,
          "registry rejects empty partial command");
    CHECK(dcc_application_command_builder_set_name(&partial, "missing-description") == DCC_OK &&
              dcc_command_registry_add_builder(&registry, &partial) == DCC_ERR_INVALID_ARG,
          "registry rejects default chat command without description");
    CHECK(dcc_application_command_builder_set_description(&partial, "Description") == DCC_OK,
    ...
    CHECK(dcc_application_command_builder_set_name(&user, "inspect-user") == DCC_OK &&
              dcc_application_command_builder_set_type(
                  &user, DCC_APPLICATION_COMMAND_USER
              ) == DCC_OK &&
    ...
    CHECK(dcc_application_command_builder_set_name(&message, "inspect-message") == DCC_OK &&
              dcc_application_command_builder_set_type(
                  &message, DCC_APPLICATION_COMMAND_MESSAGE
              ) == DCC_OK &&
              dcc_command_registry_add_builder(&registry, &message) == DCC_OK,
          "registry accepts MESSAGE command without description");
    CHECK(dcc_application_command_builder_set_description(&message, "not allowed") == DCC_OK &&
              dcc_command_registry_add_builder(&registry, &message) == DCC_ERR_INVALID_ARG,
    ...
    CHECK(dcc_application_command_builder_set_name(&primary, "launch") == DCC_OK &&
              dcc_application_command_builder_set_type(
                  &primary, DCC_APPLICATION_COMMAND_PRIMARY_ENTRY_POINT
              ) == DCC_OK &&
              dcc_command_registry_add_builder(&registry, &primary) == DCC_ERR_INVALID_ARG &&
              dcc_application_command_builder_set_description(&primary, "Launch") == DCC_OK &&
