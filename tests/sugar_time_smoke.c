...
static int check_checked_duration(void) {
  ...
  if (DCC_DURATION_CHECKED(UINT64_MAX, DCC_TIME_MS_PER_SECOND, &duration_ms) !=
      DCC_ERR_INVALID_ARG) {
    return 1;
  }
...
static int check_timeout_checked(void) {
  ...
  if (DCC_TIMEOUT_CHECKED((uint64_t)UINT32_MAX + 1U, &timeout_ms) !=
      DCC_ERR_INVALID_ARG) {
    return 1;
  }
...
static int check_sugar_structs(void) {
  ...
  if (cooldown.bucket != DCC_APP_COOLDOWN_USER || cooldown.limit != 3U ||
      cooldown.window_ms != UINT64_C(10000)) {
    return 1;
  }

  dcc_app_options_t options = DCC_APP_OPTIONS_AUTO_DEFER_EPHEMERAL(
      "token", DCC_INTENTS_DEFAULT, DCC_MS(1500U));
  if (options.auto_defer_after_ms != 1500U ||
      options.auto_defer_ephemeral != 1U) {
    return 1;
  }

  if (DCC_FLOW_AUTO_DEFER(NULL, NULL, DCC_SECONDS(2U)) != NULL) {
    return 1;
  }
  return 0;
}
