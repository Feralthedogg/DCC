...
static int response_state_runner_start(dcc_client_t *client,
...
#if defined(_WIN32)
  runner->thread =
      CreateThread(NULL, 0U, response_state_runner_main, runner, 0U, NULL);
  return runner->thread != NULL;
#else
  return pthread_create(&runner->thread, NULL, response_state_runner_main,
                        runner) == 0;
#endif
}
...
static dcc_status_t
response_state_intercept(dcc_client_t *client, const char *method,
  ...
  if (seen != NULL) {
    (void)atomic_fetch_add_explicit(&seen->requests, 1U, memory_order_relaxed);
  }
  if (cb != NULL) {
...
#if !defined(_WIN32)
static void response_state_reentrant_cb(dcc_client_t *client,
                                        const dcc_rest_result_t *response,
                                        void *user_data) {
  (void)client;
  response_state_reentrant_seen_t *seen =
      (response_state_reentrant_seen_t *)user_data;
  if (seen == NULL || response == NULL || response->http_status != 204U) {
    return;
  }
  (void)atomic_fetch_add_explicit(&seen->outer_callbacks, 1U,
...
static int app_smoke_check_same_context_reentry(void) {
  ...
  dcc_status_t status = dcc_ctx_reply_text(&ctx, "outer reply",
                                           response_state_reentrant_cb, &seen);
  ...
  int passed =
      status == DCC_OK && drain_status == DCC_OK && join_status == 0 &&
      atomic_load_explicit(&seen.watchdog_timed_out, memory_order_acquire) ==
          0U &&
      atomic_load_explicit(&seen.nested_status, memory_order_acquire) ==
          DCC_OK &&
...
#else
static int app_smoke_check_same_context_reentry(void) { return 1; }
#endif
...
static int app_smoke_check_auto_defer_local_retry(void) {
  ...
  ctx.flow = NULL;
  interaction.id = 3U;
  interaction.token = "response-state-autocomplete";
  if (dcc_flow_create(client, &interaction, &ctx.flow) != DCC_OK)
    return 1;
  ...
  if (status == DCC_OK) {
    status = dcc_autocomplete_builder_set_choices(&autocomplete, &choice, 1U);
  }
  if (status == DCC_OK) {
...
int app_smoke_check_response_state(void) {
  dcc_ctx_t ctx;
  memset(&ctx, 0, sizeof(ctx));
  dcc_client_options_t state_options = {
      .size = sizeof(state_options), .token = "", .intents = 0U};
  dcc_client_t *state_client = NULL;
  if (dcc_client_create(&state_options, &state_client) != DCC_OK)
    return 0;
  dcc_interaction_t interaction = {
      .id = 1U, .application_id = 2U, .token = "state"};
  ...
  if (dcc_ctx_response_state(NULL) != DCC_INTERACTION_FLOW_FAILED ||
      ...
      dcc_ctx_response_sent(NULL) || dcc_ctx_deferred(NULL) ||
      dcc_ctx_replied(NULL) || dcc_ctx_followed_up(NULL) ||
      !dcc_ctx_response_failed(NULL) || dcc_ctx_can_followup(NULL) ||
      dcc_ctx_can_edit_original(NULL) ||
      DCC_CTX_RESPONSE_STATE(&ctx) != DCC_INTERACTION_FLOW_READY ||
      DCC_CTX_RESPONDED(&ctx) || DCC_CTX_DEFERRED(&ctx) ||
      DCC_CTX_REPLIED(&ctx) || DCC_CTX_FOLLOWED_UP(&ctx) ||
      DCC_CTX_RESPONSE_FAILED(&ctx) || DCC_CTX_CAN_FOLLOWUP(&ctx) ||
  ...
  if (dcc_flow_mark_initial(ctx.flow, DCC_INTERACTION_FLOW_DEFERRED_EPHEMERAL,
                            DCC_OK) != DCC_OK ||
  ...
  ctx.flow = NULL;
  interaction.id++;
  interaction.token = "state-local";
  if (dcc_flow_create(state_client, &interaction, &ctx.flow) != DCC_OK)
    return 0;
  if (dcc_flow_mark_initial(ctx.flow, DCC_INTERACTION_FLOW_REPLIED,
                            DCC_ERR_INVALID_ARG) != DCC_ERR_INVALID_ARG ||
  ...
  dcc_flow_destroy(ctx.flow);
  dcc_client_destroy(state_client);
  int auto_ok = app_smoke_check_auto_defer_local_retry();
  int reentry_ok = auto_ok ? app_smoke_check_same_context_reentry() : 0;
  return auto_ok && reentry_ok;
}
