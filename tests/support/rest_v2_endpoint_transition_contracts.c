...
static dcc_status_t transition_intercept(
    dcc_client_t *client,
    const char *method,
    const char *path,
    const void *body,
    ...
    if (index < 2U) {
        (void)snprintf(state->methods[index], sizeof(state->methods[index]), "%s", method);
        (void)snprintf(state->paths[index], sizeof(state->paths[index]), "%s", path);
...
static void transition_legacy_callback(
    dcc_client_t *client,
...
static dcc_status_t transition_load_old(
...
static void transition_managed_callback(
    dcc_client_t *client,
    const dcc_rest_response_t *response,
...
static void *transition_submit(void *user_data) {
    ...
    switch (state->kind) {
        case TRANSITION_CALL_MESSAGE:
            state->call_status = dcc_app_send_json(
                &state->app,
                600U,
                state->payload,
                transition_legacy_callback,
                state
            );
            break;
...
static int transition_one(
    dcc_client_t *client,
    transition_call_kind_t kind,
    const char *original_body,
    const char *first_method,
    const char *first_path,
    const char *second_method,
    ...
    if (dcc_message_builder_set_content(&state.message, "managed-owned") != DCC_OK) {
        free(state.payload);
        return 1;
    }
    ...
    int returned_before_release = entered &&
        endpoint_wait_for_atomic(&state.returned, 1U, 100U);
    ...
    int ok = entered && returned_before_release && state.call_status == DCC_OK &&
        atomic_load_explicit(&state.request_count, memory_order_acquire) == expected_requests &&
        ...
        (kind == TRANSITION_CALL_MANAGED || strstr(state.bodies[0], original_body) != NULL);
    ...
    if (!ok) {
        fprintf(
            stderr,
            "transition kind=%d entered=%d returned=%d status=%s requests=%u callbacks=%u "
            ...
            state.methods[0], state.paths[0], state.bodies[0],
            state.methods[1], state.paths[1], state.bodies[1]
        );
    }
    free(state.payload);
    return ok ? 0 : 1;
...
int endpoint_transition_submission_contract(
    ...
    if (transition_one(
        ...
        transition_one(
            ...
            "raw-webhook-owned",
            "POST",
            "/webhooks/601/raw-token?wait=true",
            NULL,
            NULL
        ) != 0 ||
        transition_one(
            ...
            "",
            "DELETE",
            "/channels/610/messages/611",
            "POST",
            "/channels/610/messages"
        ) != 0) {
...
int endpoint_transition_rejection_contract(dcc_client_t *client) {
    ...
    state.app.client = client;
    atomic_init(&state.callback_calls, 0U);
    dcc_status_t status = dcc_app_send_json(
        &state.app,
        600U,
        "{}",
        transition_legacy_callback,
    ...
    dcc_status_t null_client = dcc_endpoint_submit_legacy_raw(
        NULL, DCC_REST_POST, "/local-reject", "wait=true",
        "application/json", "{}", 2U,
    ...
    dcc_status_t invalid_body_pair = dcc_endpoint_submit_legacy_raw(
        client, DCC_REST_POST, "/local-reject", "wait=true",
        NULL, "{}", 2U,
        transition_legacy_callback, &state
    );
    if (status != DCC_ERR_STATE || null_client != DCC_ERR_INVALID_ARG ||
        ...
        fprintf(
            stderr,
            "transition local rejection status=%s null=%s method=%s body=%s callbacks=%u\n",
            dcc_status_string(status),
            dcc_status_string(null_client),
