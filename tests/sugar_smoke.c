...
static void sugar_dummy_typed_handler(dcc_ctx_t *ctx, void *args,
                                      void *user_data) {
  (void)ctx;
  (void)args;
  (void)user_data;
...
DCC_SLASH_ARGS_IMPL(sugar_typed_impl_handler, sugar_typed_impl_args_t,
                    command) {
  (void)ctx;
  if (command->value != NULL && strcmp(command->value, "ok") == 0 &&
      user_data != NULL) {
    *((uint32_t *)user_data) += command->count;
  }
}

DCC_BUTTON_ARGS_MUT_IMPL(sugar_typed_mut_impl_handler, sugar_typed_impl_args_t,
                         button) {
  (void)ctx;
  (void)user_data;
...
static void sugar_error_handler(dcc_app_t *app, const dcc_error_t *error,
                                void *user_data) {
...
static dcc_status_t sugar_status_ok(void) { return DCC_OK; }

static dcc_status_t sugar_status_failed(void) { return DCC_ERR_STATE; }
...
static void sugar_dummy_event_handler(dcc_app_t *app, const dcc_event_t *event,
                                      void *user_data) {
  (void)app;
  (void)event;
  (void)user_data;
...
static void sugar_dummy_ready_handler(dcc_app_t *app,
...
static void sugar_dummy_message_handler(dcc_app_t *app,
...
static void sugar_dummy_message_command_handler(dcc_app_t *app,
                                                const dcc_message_t *message,
                                                const char *args,
                                                const dcc_event_t *event,
                                                void *user_data) {
...
static void sugar_guard_compile_smoke(dcc_ctx_t *ctx) {
  ...
  if (ctx != NULL) {
    ...
    DCC_REQUIRE_PERMISSIONS(ctx, DCC_PERMISSION_MANAGE_MESSAGES,
                            "Missing permissions.");
    DCC_CTX_REQUIRE_PERMISSIONS(ctx, DCC_PERMISSION_MANAGE_MESSAGES,
                                "Missing permissions.");
    DCC_CTX_REQUIRE_CAN(ctx, DCC_PERMISSION_MANAGE_MESSAGES,
                        "Missing permissions.");
    DCC_CTX_REQUIRE_CAN_ANY(ctx, DCC_PERMISSION_MODERATION_BASIC,
                            "Missing permissions.");
    ...
    DCC_CTX_REQUIRE_ALL_ROLES(ctx, roles, DCC_ARRAY_LEN(roles),
                              "Missing role.");
    ...
    DCC_CTX_REQUIRE_STRING_OPTION(required_name, ctx, "name",
                                  "Missing option.");
    ...
    DCC_CTX_REQUIRE_INTEGER_OPTION(required_count, ctx, "count",
                                   "Missing option.");
    DCC_CTX_REQUIRE_NUMBER_OPTION(required_score, ctx, "score",
                                  "Missing option.");
    DCC_CTX_REQUIRE_BOOL_OPTION(required_public, ctx, "public",
                                "Missing option.");
    DCC_CTX_REQUIRE_BOOLEAN_OPTION(required_enabled, ctx, "enabled",
                                   "Missing option.");
    ...
    DCC_CTX_REQUIRE_MEMBER_OPTION(required_member, ctx, "member",
                                  "Missing option.");
    DCC_CTX_REQUIRE_ROLE_OPTION(required_role, ctx, "role", "Missing option.");
    DCC_CTX_REQUIRE_CHANNEL_OPTION(required_channel, ctx, "channel",
                                   "Missing option.");
    DCC_CTX_REQUIRE_MESSAGE_OPTION(required_message, ctx, "message",
                                   "Missing option.");
    DCC_CTX_REQUIRE_ATTACHMENT_OPTION(required_attachment, ctx, "attachment",
                                      "Missing option.");
    ...
    DCC_CTX_REQUIRE_STRING_FIELD(required_field, ctx, "field",
                                 "Missing field.");
    DCC_CTX_REQUIRE_TEXT_FIELD(required_field_text, ctx, "field",
                               "Missing field.");
    DCC_CTX_REQUIRE_STRING_FORM(required_form, ctx, "field", "Missing field.");
    DCC_CTX_REQUIRE_TEXT_FORM(required_form_text, ctx, "field",
                              "Missing field.");
    DCC_CTX_REQUIRE_BOOL_FIELD(required_field_bool, ctx, "visible",
                               "Missing field.");
    DCC_CTX_REQUIRE_BOOLEAN_FIELD(required_field_boolean, ctx, "visible",
                                  "Missing field.");
    DCC_CTX_REQUIRE_CHECKBOX_FIELD(required_field_checkbox, ctx, "visible",
                                   "Missing field.");
    DCC_CTX_REQUIRE_BOOL_FORM(required_form_bool, ctx, "visible",
                              "Missing field.");
    DCC_CTX_REQUIRE_BOOLEAN_FORM(required_form_boolean, ctx, "visible",
                                 "Missing field.");
    DCC_CTX_REQUIRE_CHECKBOX_FORM(required_form_checkbox, ctx, "visible",
                                  "Missing field.");
    DCC_CTX_REQUIRE_VALUES_FIELD(required_values, required_value_count, ctx,
                                 "choices", "Choose a value.");
    DCC_CTX_REQUIRE_VALUES_FORM(required_form_values, required_form_value_count,
                                ctx, "choices", "Choose a value.");
    DCC_CTX_REQUIRE_FIRST_VALUE_FIELD(required_first_value, ctx, "choices",
                                      "Choose a value.");
    DCC_CTX_REQUIRE_FIRST_VALUE_FORM(required_form_first_value, ctx, "choices",
                                     "Choose a value.");
    ...
    DCC_CTX_REQUIRED_VALUES_FIELD(default_values, default_value_count, ctx,
                                  "choices");
    DCC_CTX_REQUIRED_VALUES_FORM(default_form_values, default_form_value_count,
                                 ctx, "choices");
    ...
    DCC_CTX_REQUIRE_SELECTED_FIRST(required_selected_first, ctx,
                                   "Choose a value.");
    DCC_CTX_REQUIRE_SELECT_FIRST(required_select_first, ctx, "Choose a value.");
    DCC_CTX_REQUIRE_SELECTED_VALUE(required_selected_value, ctx, 0U,
                                   "Choose a value.");
    DCC_CTX_REQUIRE_SELECT_VALUE(required_select_value, ctx, 0U,
                                 "Choose a value.");
    DCC_CTX_REQUIRE_SELECTED_VALUES(required_selected_values,
                                    required_selected_count, ctx,
                                    "Choose a value.");
    DCC_CTX_REQUIRE_SELECT_VALUES(required_select_values, required_select_count,
                                  ctx, "Choose a value.");
    ...
    DCC_CTX_REQUIRED_SELECTED_VALUES(default_selected_values,
                                     default_selected_count, ctx);
    DCC_CTX_REQUIRED_SELECT_VALUES(default_select_values, default_select_count,
                                   ctx);
    ...
    DCC_CTX_REQUIRE_CUSTOM_ID_PREFIX_BAD_INPUT(ctx, "custom.",
                                               "Wrong component.");
    DCC_CTX_REQUIRE_CUSTOM_ID_VALUE(required_custom_id, ctx,
                                    "Wrong component.");
    DCC_CTX_REQUIRE_CUSTOM_ID_SUFFIX(required_custom_suffix, ctx, "custom.",
                                     "Wrong component.");
    DCC_CTX_REQUIRE_ACTION(required_action, ctx, "custom.", "Wrong component.");
    DCC_CTX_REQUIRE_COMPONENT_ACTION(required_component_action, ctx, "custom",
                                     "Wrong component.");
    DCC_CTX_REQUIRE_ACTION_IN_NS(required_action_ns, ctx, "custom",
                                 "Wrong component.");
    DCC_CTX_REQUIRE_ACTION_IN_NAMESPACE(required_action_namespace, ctx,
                                        "custom", "Wrong component.");
    ...
    DCC_CTX_REQUIRED_ACTION_IN_NAMESPACE(default_action_namespace, ctx,
                                         "custom");
    ...
    DCC_REQUIRE_CAN(ctx, DCC_PERMISSION_MANAGE_MESSAGES,
                    "Missing permissions.");
    DCC_REQUIRE_CAN_ANY(ctx, DCC_PERMISSION_MODERATION_BASIC,
                        "Missing permissions.");
    ...
    DCC_REQUIRE_STRING_OPTION(required_name_alias, ctx, "name",
                              "Missing option.");
    DCC_REQUIRE_TEXT_OPTION(required_text_alias, ctx, "text",
                            "Missing option.");
    DCC_REQUIRE_INT_OPTION(required_limit_alias, ctx, "limit",
                           "Missing option.");
    DCC_REQUIRE_INTEGER_OPTION(required_count_alias, ctx, "count",
                               "Missing option.");
    DCC_REQUIRE_NUMBER_OPTION(required_score_alias, ctx, "score",
                              "Missing option.");
    DCC_REQUIRE_BOOL_OPTION(required_public_alias, ctx, "public",
                            "Missing option.");
    DCC_REQUIRE_BOOLEAN_OPTION(required_enabled_alias, ctx, "enabled",
                               "Missing option.");
    DCC_REQUIRE_SNOWFLAKE_OPTION(required_id_alias, ctx, "id",
                                 "Missing option.");
    DCC_REQUIRE_USER_OPTION(required_user_alias, ctx, "user",
                            "Missing option.");
    DCC_REQUIRE_MEMBER_OPTION(required_member_alias, ctx, "member",
                              "Missing option.");
    DCC_REQUIRE_ROLE_OPTION(required_role_alias, ctx, "role",
                            "Missing option.");
    DCC_REQUIRE_CHANNEL_OPTION(required_channel_alias, ctx, "channel",
                               "Missing option.");
    DCC_REQUIRE_MESSAGE_OPTION(required_message_alias, ctx, "message",
                               "Missing option.");
    DCC_REQUIRE_ATTACHMENT_OPTION(required_attachment_alias, ctx, "attachment",
                                  "Missing option.");
    ...
    DCC_REQUIRE_STRING_FIELD(required_field_alias, ctx, "field",
                             "Missing field.");
    DCC_REQUIRE_TEXT_FIELD(required_field_text_alias, ctx, "field",
                           "Missing field.");
    DCC_REQUIRE_STRING_FORM(required_form_alias, ctx, "field",
                            "Missing field.");
    DCC_REQUIRE_TEXT_FORM(required_form_text_alias, ctx, "field",
                          "Missing field.");
    DCC_REQUIRE_BOOL_FIELD(required_field_bool_alias, ctx, "visible",
                           "Missing field.");
    DCC_REQUIRE_BOOLEAN_FIELD(required_field_boolean_alias, ctx, "visible",
                              "Missing field.");
    DCC_REQUIRE_CHECKBOX_FIELD(required_field_checkbox_alias, ctx, "visible",
                               "Missing field.");
    DCC_REQUIRE_BOOL_FORM(required_form_bool_alias, ctx, "visible",
                          "Missing field.");
    DCC_REQUIRE_BOOLEAN_FORM(required_form_boolean_alias, ctx, "visible",
                             "Missing field.");
    DCC_REQUIRE_CHECKBOX_FORM(required_form_checkbox_alias, ctx, "visible",
                              "Missing field.");
    DCC_REQUIRE_VALUES_FIELD(required_values_alias, required_value_count_alias,
                             ctx, "choices", "Choose a value.");
    DCC_REQUIRE_VALUES_FORM(required_form_values_alias,
                            required_form_value_count_alias, ctx, "choices",
                            "Choose a value.");
    DCC_REQUIRE_FIRST_VALUE_FIELD(required_first_value_alias, ctx, "choices",
                                  "Choose a value.");
    DCC_REQUIRE_FIRST_VALUE_FORM(required_form_first_value_alias, ctx,
                                 "choices", "Choose a value.");
    ...
    DCC_REQUIRED_VALUES_FIELD(default_values_alias, default_value_count_alias,
                              ctx, "choices");
    DCC_REQUIRED_VALUES_FORM(default_form_values_alias,
                             default_form_value_count_alias, ctx, "choices");
    DCC_REQUIRED_FIRST_VALUE_FIELD(default_first_value_alias, ctx, "choices");
    DCC_REQUIRED_FIRST_VALUE_FORM(default_form_first_value_alias, ctx,
                                  "choices");
    ...
    DCC_REQUIRE_SELECTED_FIRST(required_selected_first_alias, ctx,
                               "Choose a value.");
    DCC_REQUIRE_SELECT_FIRST(required_select_first_alias, ctx,
                             "Choose a value.");
    DCC_REQUIRE_SELECTED_VALUE(required_selected_value_alias, ctx, 0U,
                               "Choose a value.");
    DCC_REQUIRE_SELECT_VALUE(required_select_value_alias, ctx, 0U,
                             "Choose a value.");
    DCC_REQUIRE_SELECTED_VALUES(required_selected_values_alias,
                                required_selected_count_alias, ctx,
                                "Choose a value.");
    DCC_REQUIRE_SELECT_VALUES(required_select_values_alias,
                              required_select_count_alias, ctx,
                              "Choose a value.");
    ...
    DCC_REQUIRED_SELECTED_VALUES(default_selected_values_alias,
                                 default_selected_count_alias, ctx);
    DCC_REQUIRED_SELECT_VALUES(default_select_values_alias,
                               default_select_count_alias, ctx);
    ...
    DCC_REQUIRE_CUSTOM_ID_VALUE(required_custom_id_alias, ctx,
                                "Wrong component.");
    DCC_REQUIRE_CUSTOM_ID_SUFFIX(required_custom_suffix_alias, ctx, "custom.",
                                 "Wrong component.");
    DCC_REQUIRE_ACTION(required_action_alias, ctx, "custom.",
                       "Wrong component.");
    DCC_REQUIRE_COMPONENT_ACTION(required_component_action_alias, ctx, "custom",
                                 "Wrong component.");
    DCC_REQUIRE_ACTION_IN_NS(required_action_ns_alias, ctx, "custom",
                             "Wrong component.");
    DCC_REQUIRE_ACTION_IN_NAMESPACE(required_action_namespace_alias, ctx,
                                    "custom", "Wrong component.");
    ...
    DCC_REQUIRED_COMPONENT_ACTION(default_component_action_alias, ctx,
                                  "custom");
    DCC_REQUIRED_ACTION_IN_NS(default_action_ns_alias, ctx, "custom");
    DCC_REQUIRED_ACTION_IN_NAMESPACE(default_action_namespace_alias, ctx,
                                     "custom");
...
static void sugar_dummy_cleanup(void *user_data) { (void)user_data; }
...
static int run_sugar_context_display_smoke(void) {
  ...
  if (dcc_client_create(&client_options, &client) != DCC_OK || client == NULL) {
    fprintf(stderr,
            "failed to create client for sugar context display smoke\n");
    return 1;
  }
  ...
  if (!failed) {
    failed =
        ...
        strcmp(DCC_CTX_ARG_USER_DISPLAY_NAME(&ctx, "user"), "Option Nick") !=
            0 ||
        strcmp(DCC_CTX_ARG_MEMBER_DISPLAY_NAME(&ctx, "user"), "Option Nick") !=
            0 ||
        strcmp(DCC_CTX_ARG_ROLE_NAME(&ctx, "role"), "admin") != 0 ||
        strcmp(DCC_CTX_ARG_CHANNEL_NAME(&ctx, "channel"), "general") != 0 ||
        strcmp(DCC_CTX_AUTHOR_AVATAR_URL_EXT(&ctx, "webp", 256U),
               DCC_CDN_BASE_URL "/guilds/100/users/10/avatars/"
                                "author_member_avatar.webp?size=256") != 0 ||
        strcmp(DCC_CTX_TARGET_AVATAR_URL_EXT(&ctx, "png", 128U),
               DCC_CDN_BASE_URL
               "/avatars/11/target_avatar.png?size=128") != 0 ||
        strcmp(
            DCC_CTX_ARG_USER_AVATAR_URL_EXT(&ctx, "user", "png", 64U),
            DCC_CDN_BASE_URL
            "/guilds/100/users/12/avatars/option_member_avatar.png?size=64") !=
            0 ||
        strcmp(DCC_CTX_GUILD_ICON_URL_EXT(&ctx, "png", 512U),
               DCC_CDN_BASE_URL "/icons/100/guild_icon.png?size=512") != 0 ||
        strcmp(DCC_CTX_GUILD_NAME_OR(NULL, "fallback"), "fallback") != 0 ||
        strcmp(DCC_CTX_AUTHOR_DISPLAY_NAME_OR(NULL, "fallback"), "fallback") !=
            0 ||
        strcmp(DCC_CTX_ARG_USER_DISPLAY_NAME_OR(NULL, "user", "fallback"),
               "fallback") != 0 ||
        strcmp(DCC_CTX_ARG_ROLE_NAME_OR(NULL, "role", "fallback"),
               "fallback") != 0 ||
        strcmp(DCC_CTX_ARG_CHANNEL_NAME_OR(NULL, "channel", "fallback"),
               "fallback") != 0 ||
...
static int run_sugar_reply_smoke(void) {
  dcc_modal_builder_t modal =
      DCC_MODAL("reply.modal", "Reply modal",
                DCC_ACTION_ROW(DCC_MODAL_TEXT_INPUT("name", "Name", 1U)));
  dcc_modal_flow_t modal_flow = DCC_MODAL_FLOW(
      "reply.flow", "Reply flow", sugar_field_args_t, sugar_dummy_typed_handler,
      NULL, DCC_NO_FORM_ARGS(), DCC_NO_VALIDATORS(),
      DCC_ACTION_ROW(DCC_MODAL_TEXT_INPUT("name", "Name", 1U)));
  ...
  return (DCC_REPLY(NULL, DCC_MESSAGE_TEXT("pong")) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_CB(NULL, DCC_MESSAGE_TEXT("pong"), NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          ...
          DCC_REPLY_PUBLIC_CB(NULL, "pong", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          ...
          DCC_REPLY_EPHEMERAL_CB(NULL, "secret", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_EPHEMERAL(NULL, "secret") == DCC_ERR_INVALID_ARG &&
          DCC_EPHEMERAL_CB(NULL, "secret", NULL, NULL) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_EMBED(NULL, "Title", "Body", 0x57F287U, 1U) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_EMBED_CB(NULL, "Title", "Body", 0x57F287U, 1U, NULL,
                             NULL) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_PUBLIC_EMBED(NULL, "Title", "Body", 0x57F287U) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_PUBLIC_EMBED_CB(NULL, "Title", "Body", 0x57F287U, NULL,
                                    NULL) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_EPHEMERAL_EMBED(NULL, "Title", "Body", 0x57F287U) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_EPHEMERAL_EMBED_CB(NULL, "Title", "Body", 0x57F287U, NULL,
                                       NULL) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_ERROR(NULL, "Error", "Body") == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_ERROR_CB(NULL, "Error", "Body", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_ERROR(NULL, "Error", "Body") == DCC_ERR_INVALID_ARG &&
          DCC_ERROR_CB(NULL, "Error", "Body", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_VALIDATION_ERROR(NULL, DCC_ERR_INVALID_ARG) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_VALIDATION_ERROR_CB(NULL, DCC_ERR_INVALID_ARG, NULL,
                                        NULL) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_VALIDATION_RESULT(NULL, DCC_ERR_INVALID_ARG,
                                      &validation_error) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_VALIDATION_RESULT_CB(NULL, DCC_ERR_INVALID_ARG,
                                         &validation_error, NULL,
                                         NULL) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_SUCCESS(NULL, "Done", "Body") == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_SUCCESS_CB(NULL, "Done", "Body", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_SUCCESS(NULL, "Done", "Body") == DCC_ERR_INVALID_ARG &&
          DCC_SUCCESS_CB(NULL, "Done", "Body", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          ...
          DCC_UPDATE_MESSAGE(NULL, DCC_MESSAGE_TEXT("updated")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_UPDATE_MESSAGE_CB(NULL, DCC_MESSAGE_TEXT("updated"), NULL,
                                NULL) == DCC_ERR_INVALID_ARG &&
          DCC_UPDATE(NULL, DCC_MESSAGE_TEXT("updated")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_UPDATE_CB(NULL, DCC_MESSAGE_TEXT("updated"), NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_EDIT_ORIGINAL(NULL, DCC_MESSAGE_TEXT("edited")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_EDIT_ORIGINAL_CB(NULL, DCC_MESSAGE_TEXT("edited"), NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_EDIT(NULL, DCC_MESSAGE_TEXT("edited")) == DCC_ERR_INVALID_ARG &&
          DCC_EDIT_CB(NULL, DCC_MESSAGE_TEXT("edited"), NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_DELETE_ORIGINAL(NULL) == DCC_ERR_INVALID_ARG &&
          DCC_DELETE_ORIGINAL_CB(NULL, NULL, NULL) == DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP(NULL, DCC_MESSAGE_TEXT("followup")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_CB(NULL, DCC_MESSAGE_TEXT("followup"), NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_TEXT(NULL, "followup") == DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_PUBLIC(NULL, "followup") == DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_PUBLIC_CB(NULL, "followup", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_EPHEMERAL(NULL, "followup") == DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_EPHEMERAL_TEXT(NULL, "followup") ==
              DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_EPHEMERAL_TEXT_CB(NULL, "followup", NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_SEND(NULL, DCC_MESSAGE_TEXT("send")) == DCC_ERR_INVALID_ARG &&
          DCC_SEND_CB(NULL, DCC_MESSAGE_TEXT("send"), NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_SEND_TEXT(NULL, "send") == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_V2(NULL, DCC_V2_TEXT("pong")) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_V2_CB(NULL, NULL, NULL, DCC_V2_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_EPHEMERAL_V2(NULL, DCC_V2_TEXT("secret")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_V2(NULL, DCC_V2_TEXT("pong")) == DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_V2_CB(NULL, NULL, NULL, DCC_V2_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_EPHEMERAL_V2(NULL, DCC_V2_TEXT("secret")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_PRIVATE_V2(NULL, DCC_V2_TEXT("secret")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_EDIT(NULL, DCC_MESSAGE_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          ...
          DCC_RESPOND_OR_FOLLOWUP(NULL, DCC_MESSAGE_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_FOLLOWUP_TEXT(NULL, "pong") == DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_FOLLOWUP_PRIVATE(NULL, "secret") ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_FOLLOWUP_DONE(NULL, "done") == DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_EDIT_V2(NULL, DCC_V2_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_EDIT_PRIVATE_V2(NULL, DCC_V2_TEXT("secret")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_FOLLOWUP_V2(NULL, DCC_V2_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_UI(NULL, DCC_UI_TEXT("pong")) == DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_PRIVATE_UI(NULL, DCC_UI_TEXT("secret")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_EDIT_UI(NULL, DCC_UI_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_EDIT_PRIVATE_UI(NULL, DCC_UI_TEXT("secret")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_RESPOND_OR_FOLLOWUP_UI(NULL, DCC_UI_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_CTX_RESPOND_UI(NULL, DCC_UI_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_CTX_RESPOND_PRIVATE_UI(NULL, DCC_UI_TEXT("secret")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_CTX_RESPOND_OR_EDIT_TEXT(NULL, "pong") == DCC_ERR_INVALID_ARG &&
          DCC_CTX_RESPOND_OR_EDIT_PRIVATE(NULL, "secret") ==
              DCC_ERR_INVALID_ARG &&
          DCC_CTX_RESPOND_OR_EDIT_UI(NULL, DCC_UI_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_CTX_RESPOND_OR_FOLLOWUP_TEXT(NULL, "pong") ==
              DCC_ERR_INVALID_ARG &&
          DCC_CTX_RESPOND_OR_FOLLOWUP_UI(NULL, DCC_UI_TEXT("pong")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_UPDATE_V2(NULL, DCC_V2_TEXT("updated")) == DCC_ERR_INVALID_ARG &&
          DCC_UPDATE_V2_CB(NULL, NULL, NULL, DCC_V2_TEXT("updated")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_V2(NULL, DCC_V2_TEXT("followup")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_V2_CB(NULL, NULL, NULL, DCC_V2_TEXT("followup")) ==
              DCC_ERR_INVALID_ARG &&
          DCC_FOLLOWUP_EPHEMERAL_V2(NULL, DCC_V2_TEXT("followup")) ==
              DCC_ERR_INVALID_ARG &&
          ...
          DCC_SHOW_MODAL_V1(NULL, "reply.modal.v1", "Reply modal",
                            DCC_ACTION_ROW(DCC_MODAL_TEXT_INPUT(
                                "name", "Name", 1U))) == DCC_ERR_INVALID_ARG &&
          DCC_SHOW_MODAL_V2(NULL, "reply.modal.v2", "Reply modal",
                            DCC_V2_LABEL("Name", DCC_MODAL_V2_TEXT_INPUT(
                                                     "name", "Name", 1U))) ==
              DCC_ERR_INVALID_ARG &&
          DCC_SHOW_MODAL_V2_CB(
              NULL, "reply.modal.v2.cb", "Reply modal", NULL, NULL,
              DCC_V2_LABEL("Name",
                           DCC_MODAL_V2_TEXT_INPUT("name", "Name", 1U))) ==
              DCC_ERR_INVALID_ARG &&
          DCC_SHOW_MODAL_FLOW(NULL, modal_flow) == DCC_ERR_INVALID_ARG &&
          DCC_SHOW_MODAL_FLOW_CB(NULL, modal_flow, NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_AUTOCOMPLETE(NULL, autocomplete) == DCC_ERR_INVALID_ARG &&
          DCC_REPLY_AUTOCOMPLETE_CB(NULL, autocomplete, NULL, NULL) ==
              DCC_ERR_INVALID_ARG &&
          DCC_REPLY_AUTOCOMPLETE_MATCHING(
              NULL, DCC_AUTOCOMPLETE_STRING_CHOICE("Feral", "feral")) ==
              DCC_ERR_INVALID_ARG &&
...
static int run_sugar_message_smoke(void) {
  dcc_embed_builder_t embed = DCC_EMBED_WITH_FIELDS(
      "Status", "Gateway online", DCC_EMBED_FIELD_INLINE("Shard", "0"));
...
static int run_sugar_legacy_component_smoke(void) {
  ...
  int failed = require_contains(json, "\"type\":1") ||
...
static int run_sugar_component_v2_smoke(void) {
  dcc_component_v2_builder_t container = DCC_V2_CONTAINER_ACCENT(
      0x5865F2U,
      DCC_V2_SECTION(DCC_V2_BUTTON_PRIMARY("Refresh", "status.refresh"),
  ...
  int failed = require_contains(json, "\"type\":17") ||
  ...
  dcc_component_v2_builder_t row =
      DCC_V2_ACTION_ROW(DCC_V2_BUTTON_SUCCESS("Yes", "confirm.yes"),
  ...
  failed = require_contains(json, "\"type\":1") ||
  ...
  failed = require_contains(json, "\"flags\":32768") ||
  ...
  if (dcc_message_builder_build_json(&message, &json) != DCC_OK) {
    fprintf(stderr,
            "failed to serialize sugar ephemeral component v2 message\n");
    return 1;
  }
  failed = require_contains(json, "\"flags\":32832") ||
           require_contains(json, "\"components\":[{\"type\":17");
  dcc_message_builder_json_free(json);
  return failed;
}
...
static int run_sugar_modal_smoke(void) {
  dcc_component_builder_t birthday_row = DCC_ACTION_ROW(
      DCC_MODAL_TEXT_INPUT_PLACEHOLDER("birthday", "Birthday", "YYYYMMDD", 1U));
  ...
  int failed = require_contains(json, "\"custom_id\":\"birthday-modal\"") ||
  ...
  dcc_component_v2_builder_t birthday =
      DCC_V2_LABEL("Birthday", DCC_MODAL_V2_TEXT_INPUT_PLACEHOLDER(
                                   "birthday", "Birthday", "YYYYMMDD", 1U));
  dcc_component_v2_builder_t private_age =
      DCC_MODAL_V2_CHECKBOX("hide_age", "Hide age", 0U);
  dcc_modal_builder_t modal = DCC_MODAL_V2("upload-modal", "Upload config",
                                           label, birthday, private_age);
  ...
  failed = require_contains(json, "\"type\":9") ||
...
static int run_sugar_command_smoke(void) {
  ...
  int failed = require_contains(json, "\"name\":\"ping\"") ||
  ...
  dcc_application_command_builder_t search = DCC_SLASH_COMMAND_WITH_OPTIONS(
      "search", "Search members",
      DCC_COMMAND_STRING_OPTION("name", "Member name", 1U),
      DCC_COMMAND_INTEGER_OPTION_RANGE("limit", "Result limit", 0U, 1, 25),
      DCC_COMMAND_CHANNEL_OPTION_TYPES("channel", "Target channel", 0U, 0U,
                                       5U));
  ...
  failed = require_contains(json, "\"name\":\"search\"") ||
           require_contains(json, "\"options\":[") ||
  ...
  dcc_application_command_builder_t moderation = DCC_SLASH_GROUP(
      "moderation", "Moderation tools",
      DCC_CMD_GROUP("admin", "Admin actions",
                    DCC_CMD_SUB("ban", "Ban a member",
                                DCC_OPT_STRING("name", "Member name", 1U))));
  json = NULL;
  if (dcc_application_command_builder_build_json(&moderation, &json) !=
      DCC_OK) {
    fprintf(stderr, "failed to serialize sugar slash group aliases\n");
    return 1;
  }
  failed = require_contains(json, "\"name\":\"moderation\"") ||
           require_contains(json, "\"type\":2") ||
  ...
  dcc_application_command_builder_t management = DCC_CMD_TREE(
  ...
  json = NULL;
  if (dcc_application_command_builder_build_json(&management, &json) !=
      DCC_OK) {
    fprintf(stderr, "failed to serialize sugar command tree aliases\n");
    return 1;
  }
  failed = require_contains(json, "\"name\":\"management\"") ||
           require_contains(json, "\"name\":\"admin\"") ||
  ...
  if (dcc_application_command_option_builder_build_array_json(
          options, DCC_ARRAY_LEN(options), &json) != DCC_OK) {
    fprintf(stderr, "failed to serialize standalone command options\n");
    return 1;
  }
  failed = require_contains(json, "\"autocomplete\":true") ||
...
static int run_sugar_array_compat_smoke(void) {
  ...
  dcc_embed_builder_t embed = DCC_EMBED_WITH_FIELDS_ARRAY(
      "Compat", "Explicit arrays", fields, DCC_ARRAY_LEN(fields));
  dcc_message_builder_t message = DCC_MESSAGE_EMBEDS_ARRAY(&embed, 1U);
  ...
  int failed = require_contains(json, "\"title\":\"Compat\"") ||
  ...
  if (dcc_message_builder_build_json(&component_message, &json) != DCC_OK) {
    fprintf(stderr,
            "failed to serialize sugar array compat components v2 message\n");
    return 1;
  }
  failed = require_contains(json, "\"flags\":32768") ||
...
static int run_sugar_options_smoke(void) {
  ...
  dcc_app_options_t app_options_all = DCC_APP_OPTIONS_ALL("token");
  ...
  dcc_app_options_t bot_alias_options = DCC_APP_OPTIONS_BOT("token");
  dcc_app_options_t bot_all_alias_options = DCC_APP_OPTIONS_BOT_ALL("token");
  dcc_app_options_t dev_alias_options = DCC_APP_OPTIONS_DEV("token");
  dcc_app_options_t dev_all_alias_options = DCC_APP_OPTIONS_DEV_ALL("token");
  ...
  dcc_app_options_t command_sync_app_options =
      DCC_APP_OPTIONS_WITH_COMMAND_SYNC("token", DCC_INTENTS_DEFAULT,
                                        123456789ULL);
  ...
  dcc_app_options_t command_sync_guild_app_options =
      DCC_APP_OPTIONS_GUILD_WITH_COMMAND_SYNC("token", DCC_INTENTS_DEFAULT,
  ...
  dcc_app_module_t module =
      DCC_APP_MODULE_WITH_CLEANUP("sugar.module", sugar_dummy_module_setup,
                                  &bound_string, sugar_dummy_cleanup);
  dcc_app_extension_middleware_t extension_middleware =
      DCC_EXTENSION_MIDDLEWARE(sugar_dummy_middleware, &bound_string);
  dcc_app_route_policy_t route_policy = DCC_ROUTE_POLICY(
      DCC_ROUTE_MIDDLEWARE(
          DCC_EXTENSION_MIDDLEWARE(sugar_dummy_middleware, &bound_string)),
      ...
      DCC_ROUTE_CHECK_MESSAGE(sugar_dummy_check, &bound_string, "Blocked",
  ...
  dcc_app_extension_slash_command_t extension_slash = DCC_EXTENSION_SLASH(
      "sugar_ext", "Sugar extension", sugar_dummy_handler, &bound_string);
  dcc_app_extension_slash_command_t policy_slash =
      DCC_SLASH_DATA_POLICY("sugar_policy", "Sugar policy", sugar_dummy_handler,
                            &bound_string, route_policy);
  dcc_app_extension_slash_command_t command_alias =
      DCC_COMMAND_DATA_POLICY("sugar_command", "Sugar command alias",
                              sugar_dummy_handler, &bound_string, route_policy);
  dcc_app_extension_subcommand_t extension_subcommand =
      DCC_EXTENSION_SUBCOMMAND("sugar", "admin/reload", sugar_dummy_handler,
                               &bound_string);
  dcc_app_extension_subcommand_t policy_subcommand =
      DCC_SUBCOMMAND_DATA_POLICY("sugar", "admin/policy", sugar_dummy_handler,
                                 &bound_string, route_policy);
  dcc_app_extension_autocomplete_t extension_autocomplete =
      DCC_EXTENSION_AUTOCOMPLETE("sugar_ext", sugar_dummy_handler,
                                 &bound_string);
  dcc_app_extension_autocomplete_t policy_autocomplete =
      DCC_AUTOCOMPLETE_HANDLER_DATA_POLICY("sugar_policy", sugar_dummy_handler,
                                           &bound_string, route_policy);
  ...
  dcc_app_extension_component_route_t policy_button =
      DCC_BUTTON_HANDLER_DATA_POLICY("sugar.policy.button", sugar_dummy_handler,
                                     &bound_string, route_policy);
  ...
  dcc_app_extension_component_route_t button_prefix_alias =
      DCC_BUTTON_PREFIX_DATA("sugar.alias.button:", sugar_dummy_handler,
                             &bound_string);
  dcc_app_extension_component_route_t button_ns_alias = DCC_ON_BUTTON_NS_DATA(
      "sugar", "ns.button", sugar_dummy_handler, &bound_string);
  dcc_app_extension_component_route_t button_ns_prefix_alias =
      DCC_ON_BUTTON_NS_PREFIX_DATA_POLICY("sugar",
                                          "ns.page:", sugar_dummy_handler,
                                          &bound_string, route_policy);
  ...
  dcc_app_extension_component_route_t select_prefix_alias =
      DCC_SELECT_PREFIX_DATA("sugar.alias.select:", sugar_dummy_handler,
                             &bound_string);
  dcc_app_extension_component_route_t select_ns_alias = DCC_ON_SELECT_NS_POLICY(
      "sugar", "ns.select", sugar_dummy_handler, route_policy);
  dcc_app_extension_component_route_t select_ns_prefix_alias =
      DCC_ON_SELECT_NS_PREFIX_DATA("sugar", "ns.filter:", sugar_dummy_handler,
                                   &bound_string);
  dcc_app_extension_component_route_t extension_modal =
      DCC_EXTENSION_MODAL_PREFIX("sugar.modal:", sugar_dummy_handler,
                                 &bound_string);
  dcc_app_extension_component_route_t modal_alias = DCC_MODAL_ROUTE_DATA(
      "sugar.alias.modal", sugar_dummy_handler, &bound_string);
  dcc_app_extension_component_route_t modal_prefix_alias =
      DCC_MODAL_PREFIX_ROUTE_DATA("sugar.alias.modal:", sugar_dummy_handler,
                                  &bound_string);
  dcc_app_extension_component_route_t modal_ns_alias = DCC_ON_MODAL_NS_DATA(
      "sugar", "ns.modal", sugar_dummy_handler, &bound_string);
  dcc_app_extension_component_route_t modal_ns_prefix_alias =
      DCC_ON_MODAL_NS_PREFIX_POLICY("sugar", "ns.edit:", sugar_dummy_handler,
                                    route_policy);
  dcc_app_extension_event_t extension_event = DCC_EXTENSION_EVENT(
      DCC_EVENT_RESUMED, sugar_dummy_event_handler, &bound_string);
  ...
  dcc_app_extension_message_command_t extension_message_command =
      DCC_EXTENSION_MESSAGE_COMMAND(
          "!", "ping", sugar_dummy_message_command_handler, &bound_string);
  dcc_app_extension_message_command_t prefix_message_command =
      DCC_PREFIX_COMMAND_DATA("?", "alias", sugar_dummy_message_command_handler,
                              &bound_string);
  ...
  dcc_app_extension_message_command_t text_message_command_fn =
      DCC_ON_TEXT_COMMAND_FN_DATA("~", sugar_prefix_command_alias_handler,
                                  &bound_string);
  dcc_app_extension_task_t extension_task =
      DCC_EXTENSION_TASK_KST("00:00", sugar_dummy_task_handler, &bound_string);
  dcc_app_extension_t cog = DCC_COG(
      "sugar.cog",
      DCC_COG_MIDDLEWARE(
          DCC_EXTENSION_MIDDLEWARE(sugar_dummy_middleware, &bound_string)),
      DCC_COG_COMMANDS(
          DCC_ON_SLASH("cog_ping", "Cog ping", sugar_dummy_handler),
          DCC_ON_SLASH_DATA("cog_state", "Cog state", sugar_dummy_handler,
                            &bound_string)),
      DCC_COG_SUBCOMMANDS(
          DCC_SUBCOMMAND("cog", "admin/reload", sugar_dummy_handler)),
      DCC_COG_AUTOCOMPLETES(
          DCC_ON_AUTOCOMPLETE("cog_ping", sugar_dummy_handler)),
      DCC_COG_COMPONENTS(DCC_ON_BUTTON("cog.button", sugar_dummy_handler),
                         DCC_ON_BUTTON_PREFIX_DATA(
                             "cog.page:", sugar_dummy_handler, &bound_string),
                         DCC_ON_SELECT("cog.select", sugar_dummy_handler),
                         DCC_ON_SELECT_PREFIX_DATA(
                             "cog.filter:", sugar_dummy_handler, &bound_string),
                         DCC_ON_MODAL("cog.modal", sugar_dummy_handler),
                         DCC_ON_MODAL_PREFIX_DATA(
                             "cog.edit:", sugar_dummy_handler, &bound_string)),
      DCC_COG_EVENTS(
          ...
          DCC_ON_MESSAGE_DELETE_DATA(sugar_dummy_message_handler,
                                     &bound_string),
      ...
      DCC_COG_MESSAGE_COMMANDS(
          ...
          DCC_MESSAGE_COMMAND_DATA("?", "state",
                                   sugar_dummy_message_command_handler,
                                   &bound_string)),
      DCC_COG_TASKS(DCC_TASK_LOOP_MS(100U, sugar_dummy_task_handler),
                    DCC_TASK_LOOP_SECONDS_DATA(5U, sugar_dummy_task_handler,
                                               &bound_string),
                    DCC_TASK_LOOP_KST("00:00", sugar_dummy_task_handler),
                    DCC_TASK_LOOP_DAY_AT_KST_DATA(
                        9U, 30U, sugar_dummy_task_handler, &bound_string)));
  ...
  dcc_interaction_t wait_interaction = {.id = 456ULL,
                                        .guild_id = 789ULL,
                                        .channel_id = 321ULL,
                                        .user_id = 654ULL,
                                        .custom_id = "page:next",
                                        .values = wait_values,
                                        .values_count =
                                            DCC_ARRAY_LEN(wait_values)};
  dcc_message_t wait_message = {.id = 987ULL,
                                .guild_id = 789ULL,
                                .channel_id = 321ULL,
                                .author = {.id = 654ULL},
                                .content = "waited"};
  ...
  dcc_ctx_option_field_binding_t option_field_bindings[] = {
      ...
      DCC_CTX_OPTION_FIELD_SNOWFLAKE(sugar_field_args_t, user_id, "user",
                                     123ULL),
      ...
      DCC_CTX_OPTION_FIELD_REQUIRED_MESSAGE(sugar_field_args_t, message,
                                            "message"),
      DCC_CTX_OPTION_FIELD_ATTACHMENT(sugar_field_args_t, attachment,
                                      "attachment"),
  };
  dcc_ctx_form_field_binding_t form_field_bindings[] = {
      DCC_CTX_FORM_FIELD_REQUIRED_STRING(sugar_field_args_t, birthday,
                                         "birthday"),
      DCC_CTX_FORM_FIELD_BOOL(sugar_field_args_t, visible, "visible", 1U),
      DCC_CTX_FORM_FIELD_REQUIRED_VALUES(sugar_field_args_t, tags, tag_count,
                                         "tags"),
  };
  dcc_ctx_component_field_binding_t component_field_bindings[] = {
      DCC_CTX_COMPONENT_FIELD_REQUIRED_CUSTOM_ID_SUFFIX(sugar_field_args_t,
                                                        name, "typed.button:"),
      DCC_CTX_COMPONENT_FIELD_REQUIRED_FIRST_VALUE(sugar_field_args_t,
                                                   birthday),
      DCC_CTX_COMPONENT_FIELD_REQUIRED_VALUES(sugar_field_args_t, tags,
                                              tag_count),
  };
  ...
  dcc_status_t validation_status = DCC_CTX_VALIDATE_FIELDS(
      &valid_args, &validation_error,
      DCC_VALIDATE_STRING_REQUIRED(sugar_field_args_t, name, "name"),
      DCC_VALIDATE_INT_RANGE(sugar_field_args_t, limit, "limit", 1, 100),
      DCC_VALIDATE_BIRTHDAY(sugar_field_args_t, birthday, "birthday"));
  dcc_status_t invalid_status = DCC_CTX_VALIDATE_FIELDS(
      &invalid_args, &invalid_error,
      DCC_VALIDATE_BIRTHDAY(sugar_field_args_t, birthday, "birthday"));
  ...
  dcc_ctx_option_field_binding_list_t typed_bindings =
      DCC_CTX_OPTION_FIELD_BINDINGS(
          DCC_CTX_OPTION_FIELD_REQUIRED_STRING(sugar_field_args_t, name,
                                               "name"),
          DCC_CTX_OPTION_FIELD_INT(sugar_field_args_t, limit, "limit", 10));
  dcc_ctx_option_field_binding_list_t short_typed_bindings = DCC_ARGS(
  ...
  dcc_ctx_form_field_binding_list_t typed_form_bindings =
      DCC_CTX_FORM_FIELD_BINDINGS(DCC_CTX_FORM_FIELD_REQUIRED_STRING(
          sugar_field_args_t, birthday, "birthday"));
  dcc_ctx_form_field_binding_list_t short_form_bindings = DCC_FORM_ARGS(
      DCC_FORM_ARG_REQUIRED_STRING(sugar_field_args_t, birthday, "birthday"),
      DCC_FORM_ARG_BOOL(sugar_field_args_t, visible, "visible", 1U));
  dcc_ctx_form_field_binding_list_t readable_form_bindings = DCC_FORM_ARGS(
  ...
  dcc_ctx_component_field_binding_list_t typed_component_bindings =
      DCC_CTX_COMPONENT_FIELD_BINDINGS(
          DCC_CTX_COMPONENT_FIELD_REQUIRED_CUSTOM_ID_SUFFIX(
              sugar_field_args_t, name, "typed.button:"),
          DCC_CTX_COMPONENT_FIELD_REQUIRED_FIRST_VALUE(sugar_field_args_t,
                                                       birthday));
  dcc_ctx_component_field_binding_list_t short_component_bindings =
      DCC_COMPONENT_ARGS(
          DCC_COMPONENT_ARG_REQUIRED_CUSTOM_ID_SUFFIX(sugar_field_args_t, name,
                                                      "typed.button:"),
  ...
  dcc_ctx_component_field_binding_list_t readable_component_bindings =
      DCC_COMPONENT_ARGS(
          DCC_COMPONENT_REQUIRED_ACTION(sugar_field_args_t, name,
                                        "typed.button:"),
          DCC_COMPONENT_REQUIRED_VALUE(sugar_field_args_t, birthday),
          DCC_COMPONENT_REQUIRED_VALUES(sugar_field_args_t, tags, tag_count));
  dcc_ctx_field_validator_list_t typed_validators = DCC_CTX_FIELD_VALIDATORS(
      DCC_VALIDATE_STRING_REQUIRED(sugar_field_args_t, name, "name"),
      DCC_VALIDATE_INT_RANGE(sugar_field_args_t, limit, "limit", 1, 100));
  dcc_ctx_field_validator_list_t short_validators = DCC_VALIDATORS(
      DCC_VALIDATE_STRING_REQUIRED(sugar_field_args_t, name, "name"),
      DCC_VALIDATE_INT_RANGE(sugar_field_args_t, limit, "limit", 1, 100));
  dcc_command_route_t typed_slash = DCC_TYPED_SLASH(
      &typed_command, sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_bindings, typed_validators);
  dcc_command_route_t policy_typed_slash = DCC_TYPED_SLASH_POLICY(
      &typed_command, sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_bindings, typed_validators, route_policy);
  dcc_command_route_t guarded_typed_slash = DCC_TYPED_SLASH_GUARDED(
      &typed_command, sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_bindings, typed_validators, DCC_ONLY_GUILD(),
      DCC_CAN(DCC_PERMISSION_MANAGE_MESSAGES));
  dcc_command_route_t inline_typed_slash = DCC_TYPED_SLASH_OPTIONS(
      "typed_inline", "Typed inline command", sugar_field_args_t,
      sugar_dummy_typed_handler, short_typed_bindings, short_validators,
      DCC_OPT_STRING("name", "Name", 1U),
      DCC_OPT_INT_RANGE("limit", "Limit", 0U, 1, 25));
  dcc_command_route_t inline_typed_slash_data = DCC_TYPED_SLASH_OPTIONS_DATA(
      "typed_inline_data", "Typed inline command with data", sugar_field_args_t,
      sugar_dummy_typed_handler, &bound_string, short_typed_bindings,
      DCC_NO_VALIDATORS(), DCC_OPT_BOOL("public", "Show publicly", 0U));
  dcc_command_route_t inline_typed_slash_no_options =
      DCC_TYPED_SLASH_NO_OPTIONS("typed_empty", "Typed command without options",
                                 sugar_field_args_t, sugar_dummy_typed_handler,
                                 DCC_NO_ARGS(), DCC_NO_VALIDATORS());
  dcc_command_route_t no_options_command_route =
      DCC_COMMAND_ROUTE_NO_OPTIONS_DATA(
          "typed_empty_route", "Route-first command without options",
          sugar_dummy_typed_handler, &bound_string);
  dcc_command_route_t guarded_params_slash = DCC_COMMAND_ROUTE_DATA_GUARDED(
      "typed_guarded_params", "Typed guarded params command",
      sugar_field_args_t, sugar_dummy_typed_handler, &bound_string,
      SUGAR_FIELD_PARAMS, DCC_ONLY_GUILD(),
      DCC_CAN(DCC_PERMISSION_MANAGE_MESSAGES));
  dcc_application_command_builder_t params_command_fn_schema =
      DCC_COMMAND_PARAMS_FN(sugar_dummy_typed_handler,
                            "Typed function-name schema", sugar_field_args_t,
                            SUGAR_FIELD_PARAMS);
  dcc_command_route_t params_slash_fn = DCC_COMMAND_ROUTE_FN_DATA_GUARDED(
      sugar_dummy_typed_handler, "Typed function-name params command",
      sugar_field_args_t, &bound_string, SUGAR_FIELD_PARAMS, DCC_ONLY_GUILD());
  dcc_app_route_id_t typed_route = DCC_APP_ROUTE_INVALID;
  dcc_command_route_t typed_slash_route = DCC_TYPED_SLASH_ROUTE(
      &typed_command, sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, &typed_route, typed_bindings, typed_validators);
  dcc_application_command_builder_t typed_subcommand_parent =
      DCC_SLASH_COMMAND_WITH_OPTIONS(
          "typed_parent", "Typed parent command",
  ...
  dcc_subcommand_route_t typed_subcommand =
      DCC_TYPED_SUBCOMMAND_BUILDER_ROUTE_POLICY(
          &typed_subcommand_parent, "admin/reload", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, &typed_subcommand_route_id,
          DCC_CTX_OPTION_FIELD_BINDINGS(DCC_CTX_OPTION_FIELD_REQUIRED_STRING(
              sugar_field_args_t, name, "name")),
          DCC_CTX_FIELD_VALIDATORS(
              DCC_VALIDATE_STRING_REQUIRED(sugar_field_args_t, name, "name")),
          route_policy);
  dcc_subcommand_route_t named_typed_subcommand = DCC_TYPED_SUBCOMMAND(
      "typed_parent", "admin/inspect", sugar_field_args_t,
      sugar_dummy_typed_handler, NULL, DCC_NO_ARGS(), DCC_NO_VALIDATORS());
  dcc_subcommand_route_t guarded_params_subcommand =
      DCC_SUBCOMMAND_ROUTE_DATA_GUARDED(
          "typed_parent", "admin/guarded", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, SUGAR_FIELD_PARAMS,
          DCC_HAS_ROLE(123ULL));
  dcc_subcommand_route_t params_subcommand_fn =
      DCC_SUBCOMMAND_ROUTE_FN_DATA_GUARDED(
          "typed_parent", sugar_dummy_typed_handler, sugar_field_args_t,
          &bound_string, SUGAR_FIELD_PARAMS, DCC_ONLY_GUILD());
  dcc_subcommand_route_t in_params_subcommand_fn =
      DCC_SUBCOMMAND_ROUTE_IN_FN_DATA_GUARDED(
          "typed_parent", "admin", sugar_dummy_typed_handler,
          sugar_field_args_t, &bound_string, SUGAR_FIELD_PARAMS,
          DCC_ONLY_GUILD());
  dcc_subcommand_route_t in_params_subcommand =
      DCC_SUBCOMMAND_ROUTE_IN_DATA_POLICY(
          "typed_parent", "admin", "policy", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, SUGAR_FIELD_PARAMS,
          route_policy);
  dcc_subcommand_route_t builder_in_subcommand =
      DCC_ON_SUBCOMMAND_BUILDER_PARAMS_IN_DATA_VALIDATED(
          &typed_subcommand_parent, "admin", "sync", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, SUGAR_FIELD_PARAMS,
          short_validators);
  ...
  dcc_autocomplete_route_t typed_autocomplete =
      DCC_TYPED_AUTOCOMPLETE_BUILDER_ROUTE_POLICY(
          &typed_command, sugar_field_args_t, sugar_dummy_typed_handler,
          &bound_string, &typed_autocomplete_route_id,
          DCC_CTX_OPTION_FIELD_BINDINGS(DCC_CTX_OPTION_FIELD_REQUIRED_STRING(
              sugar_field_args_t, name, "name")),
          DCC_CTX_FIELD_VALIDATORS(
              DCC_VALIDATE_STRING_REQUIRED(sugar_field_args_t, name, "name")),
          route_policy);
  dcc_autocomplete_route_t named_typed_autocomplete = DCC_TYPED_AUTOCOMPLETE(
      "typed_parent", sugar_field_args_t, sugar_dummy_typed_handler, NULL,
      DCC_NO_ARGS(), DCC_NO_VALIDATORS());
  dcc_autocomplete_route_t guarded_params_autocomplete =
      DCC_AUTOCOMPLETE_PARAMS_DATA_GUARDED(
          "typed_guarded_params", sugar_field_args_t, sugar_dummy_typed_handler,
          &bound_string, SUGAR_FIELD_PARAMS, DCC_RATE_LIMIT_USER(1U, 500U));
  dcc_autocomplete_route_t params_autocomplete_fn =
      DCC_AUTOCOMPLETE_PARAMS_FN_DATA_GUARDED(
          sugar_dummy_typed_handler, sugar_field_args_t, &bound_string,
          SUGAR_FIELD_PARAMS, DCC_ONLY_GUILD());
  ...
  dcc_autocomplete_route_t in_subcommand_autocomplete =
      DCC_ON_SUBCOMMAND_AUTOCOMPLETE_PARAMS_IN_DATA(
          "typed_parent", "admin", "auto", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, SUGAR_FIELD_PARAMS);
  dcc_autocomplete_route_t builder_in_subcommand_autocomplete =
      DCC_ON_SUBCOMMAND_AUTOCOMPLETE_BUILDER_PARAMS_IN_DATA(
          &typed_subcommand_parent, "admin", "complete", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, SUGAR_FIELD_PARAMS);
  dcc_modal_route_t typed_modal = DCC_TYPED_MODAL(
      "typed.modal", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_form_bindings, DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_modal_route_t policy_typed_modal = DCC_TYPED_MODAL_POLICY(
      "typed.policy.modal", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_form_bindings, DCC_CTX_NO_FIELD_VALIDATORS(),
      route_policy);
  dcc_modal_route_t guarded_typed_modal = DCC_TYPED_MODAL_GUARDED(
      "typed.guarded.modal", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_form_bindings, DCC_CTX_NO_FIELD_VALIDATORS(),
      DCC_ONLY_DM());
  dcc_modal_route_t typed_modal_ns = DCC_TYPED_MODAL_NS(
      "typed.ns", "modal", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_form_bindings, DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_modal_route_t typed_modal_ns_prefix = DCC_TYPED_MODAL_NS_PREFIX_GUARDED(
      "typed.ns", "modal:", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_form_bindings, DCC_CTX_NO_FIELD_VALIDATORS(),
      DCC_ONLY_GUILD());
  dcc_modal_route_t params_modal_ns = DCC_MODAL_NS_PARAMS_DATA(
      "typed.ns", "modal.params", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, SUGAR_MODAL_PARAMS);
  dcc_modal_route_t params_modal_ns_prefix =
      DCC_MODAL_NS_PREFIX_PARAMS_DATA_POLICY(
          "typed.ns", "modal.params:", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, SUGAR_MODAL_PARAMS,
          route_policy);
  dcc_modal_route_t params_modal_ns_fn = DCC_MODAL_NS_PARAMS_FN_DATA(
      "typed.fn.modal", sugar_dummy_typed_handler, sugar_field_args_t,
      &bound_string, SUGAR_MODAL_PARAMS);
  dcc_modal_route_t params_modal_ns_prefix_fn =
      DCC_MODAL_NS_PREFIX_PARAMS_FN_DATA_GUARDED(
          "typed.fn.modal", sugar_dummy_typed_handler, sugar_field_args_t,
          &bound_string, SUGAR_MODAL_PARAMS, DCC_ONLY_GUILD());
  dcc_modal_route_t typed_modal_prefix = DCC_TYPED_MODAL_PREFIX(
      "typed:", sugar_field_args_t, sugar_dummy_typed_handler, &bound_string,
      typed_form_bindings, typed_validators);
  dcc_modal_flow_t params_modal_flow_ns = DCC_MODAL_V2_FLOW_NS_PARAMS_DATA(
      "typed.ns", "modal.flow", "Typed namespace flow", sugar_field_args_t,
      sugar_dummy_typed_handler, &bound_string, SUGAR_MODAL_PARAMS,
      DCC_V2_LABEL("Birthday",
                   DCC_UI_INPUT_NS("typed.ns", "birthday", "Birthday", 1U)));
  dcc_modal_flow_t params_modal_flow_ns_fn = DCC_MODAL_FLOW_NS_PARAMS_FN_DATA(
      "typed.fn.flow", "Typed namespace function flow", sugar_field_args_t,
      sugar_dummy_typed_handler, &bound_string, SUGAR_MODAL_PARAMS,
  ...
  dcc_modal_flow_t params_modal_v2_flow_ns_fn =
      DCC_MODAL_V2_FLOW_NS_PARAMS_FN_DATA(
          "typed.fn.v2.flow", "Typed namespace function v2 flow",
          sugar_field_args_t, sugar_dummy_typed_handler, &bound_string,
          SUGAR_MODAL_PARAMS,
          DCC_V2_LABEL(
              "Birthday",
  ...
  dcc_modal_flow_t modal_flow = DCC_MODAL_FLOW(
      "typed.flow", "Typed flow", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_form_bindings, DCC_CTX_NO_FIELD_VALIDATORS(),
      DCC_ACTION_ROW(DCC_MODAL_TEXT_INPUT("birthday", "Birthday", 1U)));
  dcc_modal_flow_t modal_v2_flow = DCC_MODAL_V2_FLOW(
      "typed.v2.flow", "Typed v2 flow", sugar_field_args_t,
      sugar_dummy_typed_handler, &bound_string, typed_form_bindings,
      typed_validators,
      DCC_V2_LABEL("Birthday",
                   DCC_MODAL_V2_TEXT_INPUT("birthday", "Birthday", 1U)));
  dcc_button_route_t typed_button = DCC_TYPED_BUTTON_PREFIX(
      "typed.button:", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_component_bindings, typed_validators);
  dcc_button_route_t typed_button_ns = DCC_TYPED_BUTTON_NS(
      "typed.ns", "button", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_component_bindings, typed_validators);
  dcc_button_route_t typed_button_ns_prefix =
      DCC_TYPED_BUTTON_NS_PREFIX_GUARDED(
          "typed.ns", "button:", sugar_field_args_t, sugar_dummy_typed_handler,
          &bound_string, readable_component_bindings, typed_validators,
          DCC_HAS_ANY_ROLE(900ULL, 901ULL));
  dcc_button_route_t params_button_ns = DCC_BUTTON_NS_PARAMS_DATA(
      "typed.ns", "button.params", sugar_field_args_t,
      sugar_dummy_typed_handler, &bound_string, SUGAR_COMPONENT_PARAMS);
  dcc_button_route_t params_button_ns_prefix =
      DCC_BUTTON_NS_PREFIX_PARAMS_DATA_POLICY(
          "typed.ns", "button.params:", sugar_field_args_t,
          sugar_dummy_typed_handler, &bound_string, SUGAR_COMPONENT_PARAMS,
          route_policy);
  dcc_button_route_t params_button_ns_fn = DCC_BUTTON_NS_PARAMS_FN_DATA(
      "typed.fn.button", sugar_dummy_typed_handler, sugar_field_args_t,
      &bound_string, SUGAR_COMPONENT_PARAMS);
  dcc_button_route_t params_button_ns_prefix_fn =
      DCC_BUTTON_NS_PREFIX_PARAMS_FN_DATA_GUARDED(
          "typed.fn.button", sugar_dummy_typed_handler, sugar_field_args_t,
          &bound_string, SUGAR_COMPONENT_PARAMS, DCC_ONLY_GUILD());
  dcc_button_route_t policy_typed_button = DCC_TYPED_BUTTON_POLICY(
      "typed.policy.button", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_component_bindings, typed_validators, route_policy);
  dcc_button_route_t guarded_typed_button = DCC_TYPED_BUTTON_PREFIX_GUARDED(
      "typed.guarded.button:", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, readable_component_bindings, typed_validators,
      DCC_HAS_ANY_ROLE(900ULL, 901ULL), DCC_RATE_LIMIT_USER(1U, 500U));
  dcc_select_route_t typed_select = DCC_TYPED_SELECT(
      "typed.select", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_component_bindings, DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_select_route_t typed_select_ns = DCC_TYPED_SELECT_NS(
      "typed.ns", "select", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_component_bindings, DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_select_route_t typed_select_ns_prefix =
      DCC_TYPED_SELECT_NS_PREFIX_GUARDED(
          "typed.ns", "select:", sugar_field_args_t, sugar_dummy_typed_handler,
          &bound_string, readable_component_bindings,
          DCC_CTX_NO_FIELD_VALIDATORS(), DCC_OWNER_ONLY(902ULL));
  dcc_select_route_t params_select_ns = DCC_SELECT_NS_PARAMS_DATA(
      "typed.ns", "select.params", sugar_field_args_t,
      sugar_dummy_typed_handler, &bound_string, SUGAR_COMPONENT_PARAMS);
  dcc_select_route_t params_select_ns_prefix =
      DCC_SELECT_NS_PREFIX_PARAMS_DATA_POLICY(
          ...
          sugar_dummy_typed_handler, &bound_string, SUGAR_COMPONENT_PARAMS,
          route_policy);
  dcc_select_route_t params_select_ns_fn = DCC_SELECT_NS_PARAMS_FN_DATA(
      "typed.fn.select", sugar_dummy_typed_handler, sugar_field_args_t,
      &bound_string, SUGAR_COMPONENT_PARAMS);
  dcc_select_route_t params_select_ns_prefix_fn =
      DCC_SELECT_NS_PREFIX_PARAMS_FN_DATA_GUARDED(
          "typed.fn.select", sugar_dummy_typed_handler, sugar_field_args_t,
          &bound_string, SUGAR_COMPONENT_PARAMS, DCC_ONLY_GUILD());
  dcc_select_route_t policy_typed_select = DCC_TYPED_SELECT_POLICY(
      "typed.policy.select", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, typed_component_bindings, DCC_CTX_NO_FIELD_VALIDATORS(),
      route_policy);
  dcc_select_route_t guarded_typed_select = DCC_TYPED_SELECT_GUARDED(
      "typed.guarded.select", sugar_field_args_t, sugar_dummy_typed_handler,
      &bound_string, readable_component_bindings, DCC_CTX_NO_FIELD_VALIDATORS(),
      DCC_OWNER_ONLY(902ULL));
  dcc_app_extension_t typed_cog = DCC_COG(
      "sugar.typed_cog", DCC_COG_COMMAND_ROUTES(inline_typed_slash_no_options),
      DCC_COG_SUBCOMMAND_ROUTES(named_typed_subcommand),
      DCC_COG_AUTOCOMPLETE_ROUTES(named_typed_autocomplete),
      DCC_COG_MODAL_ROUTES(typed_modal), DCC_COG_BUTTON_ROUTES(typed_button),
      DCC_COG_SELECT_ROUTES(typed_select));
  dcc_app_view_t app_view = DCC_PERSISTENT_VIEW(
      DCC_VIEW_BUTTON("app.persistent.button", sugar_dummy_handler,
                      &bound_string),
      DCC_VIEW_SELECT_PREFIX("app.persistent.select:", sugar_dummy_handler,
                             &bound_string));
  dcc_component_session_t sugar_component_session = {
      .size = sizeof(sugar_component_session),
  };
  dcc_app_definition_t app_definition = DCC_APP(
      "sugar.app", DCC_APP_MODULES(module), DCC_APP_EXTENSIONS(cog, typed_cog),
      DCC_APP_COMMAND_ROUTES(typed_slash),
      DCC_APP_SUBCOMMAND_ROUTES(typed_subcommand),
      DCC_APP_AUTOCOMPLETE_ROUTES(typed_autocomplete,
                                  typed_subcommand_autocomplete),
      DCC_APP_MODAL_ROUTES(typed_modal), DCC_APP_BUTTON_ROUTES(typed_button),
      DCC_APP_SELECT_ROUTES(typed_select),
      DCC_APP_MESSAGE_COMMANDS(DCC_MESSAGE_COMMAND_DATA(
          "!", "top", sugar_dummy_message_command_handler, &bound_string)),
      DCC_APP_EVENTS(
          DCC_ON_READY_DATA(sugar_dummy_ready_handler, &bound_string),
          DCC_ON_MESSAGE_UPDATE(sugar_dummy_message_handler)),
      DCC_APP_TASKS(DCC_TASK_LOOP_MINUTES(10U, sugar_dummy_task_handler),
                    DCC_TASK_LOOP_HOURS_DATA(1U, sugar_dummy_task_handler,
                                             &bound_string)),
      DCC_APP_VIEWS(app_view),
      DCC_APP_COMPONENT_SESSIONS(DCC_COMPONENT_SESSION_ROUTE(
          &sugar_component_session, sugar_dummy_handler, &bound_string)),
      DCC_APP_MIDDLEWARE(extension_middleware), DCC_APP_GUILD_ONLY(),
      ...
      DCC_APP_CHECK_MESSAGE(sugar_dummy_check, &bound_string, "App blocked",
                            "This app is blocked."),
      DCC_APP_COOLDOWN_USER(3U, 2000U), DCC_APP_AUTO_SYNC(),
      DCC_APP_AUTO_DEFER_EPHEMERAL(1500U), DCC_APP_DEFAULT_ERRORS(),
  ...
  dcc_app_definition_t dm_app_definition =
      DCC_APP("sugar.dm_app", DCC_APP_DM_ONLY());
  dcc_app_definition_t nsfw_app_definition =
      DCC_APP("sugar.nsfw_app", DCC_APP_NSFW_ONLY());
  dcc_app_definition_t dev_app_definition =
      DCC_APP("sugar.dev_app", DCC_APP_DEV_MODE());
  dcc_app_definition_t dev_plan_app_definition =
      DCC_APP("sugar.dev_plan_app", DCC_APP_DEV_MODE_PLAN_MS(2500U));
  dcc_app_view_t view = DCC_APP_VIEW(
  ...
  dcc_autocomplete_builder_t autocomplete =
      DCC_AUTOCOMPLETE_CHOICES(DCC_AUTOCOMPLETE_STRING_CHOICE("Feral", "feral"),
  ...
  dcc_component_session_check_t session_check = DCC_COMPONENT_SESSION_CHECK(
      "dcc:v1:session:next:nonce:sig", 1000U, 789ULL, 111ULL, 222ULL);
  ...
  dcc_rest_firewall_request_t firewall_request = DCC_REST_FIREWALL_REQUEST(
      "POST", "/interactions/1/token/callback", 1000U, 1U);
  dcc_replay_record_t replay = DCC_REPLAY_GATEWAY_RECORD(
  ...
  dcc_poll_builder_t poll = DCC_POLL("Deploy?", 24U, DCC_POLL_ANSWER("Now"),
                                     DCC_POLL_ANSWER("Later"));
  dcc_component_builder_t legacy_select = DCC_STRING_SELECT_BUILDER(
      "legacy.select", DCC_SELECT_OPTION("One", "one"),
  ...
  dcc_component_builder_t legacy_select_ns = DCC_STRING_SELECT_BUILDER_NS(
      "birthday", "schedule", DCC_SELECT_OPTION("June", "06"));
  dcc_component_builder_t legacy_input_ns = DCC_TEXT_INPUT_PLACEHOLDER_NS(
      "birthday", "date", "Birthday", DCC_TEXT_INPUT_SHORT, "YYYYMMDD", 1U);
  dcc_component_v2_builder_t gallery =
      DCC_V2_MEDIA_GALLERY(DCC_V2_MEDIA("attachment://one.png", "One"),
  ...
  dcc_component_v2_builder_t string_select =
      DCC_V2_STRING_SELECT("v2.string", DCC_SELECT_OPTION("Alpha", "alpha"),
                           DCC_SELECT_OPTION("Beta", "beta"));
  dcc_component_v2_builder_t v2_button_ns =
      DCC_V2_BUTTON_PRIMARY_NS("Register", "birthday", "register");
  dcc_component_v2_builder_t string_select_ns = DCC_V2_STRING_SELECT_NS(
      "birthday", "schedule", DCC_SELECT_OPTION("June", "06"));
  dcc_component_v2_builder_t user_select_ns =
      DCC_V2_USER_SELECT_NS("birthday", "user");
  dcc_component_v2_builder_t channel_select = DCC_V2_CHANNEL_SELECT(
      "v2.channel", DCC_V2_DEFAULT_VALUES(DCC_V2_DEFAULT_CHANNEL(333ULL)),
      DCC_V2_CHANNEL_TYPES(0U, 5U));
  dcc_component_v2_builder_t channel_select_ns = DCC_V2_CHANNEL_SELECT_NS(
      "birthday", "channel",
      DCC_V2_DEFAULT_VALUES(DCC_V2_DEFAULT_CHANNEL(333ULL)),
      DCC_V2_CHANNEL_TYPES(0U, 5U));
  dcc_component_v2_builder_t radio =
      DCC_V2_RADIO_GROUP("v2.radio", DCC_SELECT_OPTION("Fast", "fast"),
                         DCC_SELECT_OPTION("Safe", "safe"));
  dcc_component_v2_builder_t checkbox_group =
      DCC_V2_CHECKBOX_GROUP("v2.checks", DCC_SELECT_OPTION("Logs", "logs"),
                            DCC_SELECT_OPTION("Metrics", "metrics"));
  dcc_component_v2_builder_t text_input_ns = DCC_V2_TEXT_INPUT_PLACEHOLDER_NS(
      "birthday", "date", "Birthday", DCC_TEXT_INPUT_SHORT, "YYYYMMDD", 1U);
  dcc_component_v2_builder_t file_upload_ns =
      DCC_V2_FILE_UPLOAD_NS("birthday", "proof");
  dcc_component_v2_builder_t radio_ns = DCC_V2_RADIO_GROUP_NS(
      "birthday", "visibility", DCC_SELECT_OPTION("Public", "public"));
  ...
  dcc_component_v2_builder_t ui_select_ns = DCC_UI_STRING_SELECT_NS(
      "birthday", "month", DCC_SELECT_OPTION("June", "06"));
  dcc_component_v2_builder_t ui_input_ns = DCC_UI_INPUT_PLACEHOLDER_NS(
      "birthday", "date", "Birthday", "YYYYMMDD", 1U);
  ...
  dcc_managed_message_options_t managed_keep =
      DCC_MANAGED_MESSAGE_KEEP_PREVIOUS_OPTIONS(444ULL, &managed_message, NULL,
                                                NULL, NULL);
  ...
  dcc_current_user_params_t user_params = {
      ...
      .version = DCC_CURRENT_USER_PARAMS_VERSION,
      .present = DCC_CURRENT_USER_PRESENT_USERNAME,
      .username = "sugar-user",
  };
  ...
  if (client.size != sizeof(client) || client.intents != DCC_INTENT_GUILDS ||
      client_all.intents != DCC_INTENTS_ALL || sharded_client.shard_id != 1U ||
      ...
      dev_guild_alias_options.command_sync.command_registry.guild_id !=
          999ULL ||
      ...
      dev_guild_all_alias_options.command_sync.command_registry.guild_id !=
          999ULL ||
      dev_guild_all_alias_options.command_sync_on_ready != 1U ||
      command_sync.application_id != 123456789ULL || command_sync.apply != 1U ||
      command_sync.once != 1U || command_sync.command_registry.guild_id != 0U ||
      ...
      command_sync_global_auto.command_registry.size !=
          sizeof(dcc_command_registry_options_t) ||
      ...
      command_sync_guild_app_options.command_registry.guild_id !=
          987654321ULL ||
      command_sync_guild_app_options.command_sync.command_registry.guild_id !=
          987654321ULL ||
      ...
      route_policy.middleware_count != 1U || route_policy.middlewares == NULL ||
      ...
      route_policy.guild_only != 1U || route_policy.channel_type_count != 2U ||
      ...
      route_policy.check_count != 1U || route_policy.checks == NULL ||
      ...
      strcmp(route_policy.checks[0].description, "This route is blocked.") !=
          0 ||
      route_policy.cooldown.bucket != DCC_APP_COOLDOWN_USER ||
      route_policy.cooldown.limit != 2U ||
      route_policy.cooldown.window_ms != 1000U ||
      dm_route_policy.dm_only != 1U || dm_route_policy.guild_only != 0U ||
      nsfw_route_policy.nsfw_only != 1U || nsfw_route_policy.dm_only != 0U ||
      ...
      policy_subcommand.policy.required_permissions !=
      ...
      strcmp(DCC_COMPONENT_ACTION_PREFIX("sugar", "literal:"),
             "sugar.literal:") != 0 ||
      ...
      strcmp(params_modal_ns_fn.custom_id,
             "typed.fn.modal.sugar_dummy_typed_handler") != 0 ||
      params_modal_ns_fn.user_data != &bound_string ||
      params_modal_ns_fn.bindings.count != 1U ||
      strcmp(params_modal_ns_prefix_fn.custom_id,
             "typed.fn.modal.sugar_dummy_typed_handler") != 0 ||
      params_modal_ns_prefix_fn.prefix != 1U ||
      params_modal_ns_prefix_fn.policy.guild_only != 1U ||
      strcmp(params_modal_flow_ns.modal.custom_id, "typed.ns.modal.flow") !=
          0 ||
      strcmp(params_modal_flow_ns.route.custom_id, "typed.ns.modal.flow") !=
          0 ||
      params_modal_flow_ns.modal.components_v2_count != 1U ||
      strcmp(params_modal_flow_ns_fn.modal.custom_id,
             "typed.fn.flow.sugar_dummy_typed_handler") != 0 ||
      strcmp(params_modal_flow_ns_fn.route.custom_id,
             "typed.fn.flow.sugar_dummy_typed_handler") != 0 ||
      params_modal_flow_ns_fn.modal.components_count != 1U ||
      params_modal_flow_ns_fn.route.user_data != &bound_string ||
      strcmp(params_modal_v2_flow_ns_fn.modal.custom_id,
             "typed.fn.v2.flow.sugar_dummy_typed_handler") != 0 ||
      strcmp(params_modal_v2_flow_ns_fn.route.custom_id,
             "typed.fn.v2.flow.sugar_dummy_typed_handler") != 0 ||
      ...
      strcmp(params_button_ns_prefix.custom_id, "typed.ns.button.params:") !=
          0 ||
      params_button_ns_prefix.prefix != 1U ||
      params_button_ns_prefix.policy.guild_only != 1U ||
      strcmp(params_button_ns_fn.custom_id,
             "typed.fn.button.sugar_dummy_typed_handler") != 0 ||
      params_button_ns_fn.user_data != &bound_string ||
      params_button_ns_fn.bindings.count != 2U ||
      strcmp(params_button_ns_prefix_fn.custom_id,
             "typed.fn.button.sugar_dummy_typed_handler") != 0 ||
      ...
      strcmp(params_select_ns_prefix.custom_id, "typed.ns.select.params:") !=
          0 ||
      params_select_ns_prefix.prefix != 1U ||
      params_select_ns_prefix.policy.guild_only != 1U ||
      strcmp(params_select_ns_fn.custom_id,
             "typed.fn.select.sugar_dummy_typed_handler") != 0 ||
      params_select_ns_fn.user_data != &bound_string ||
      params_select_ns_fn.bindings.count != 2U ||
      strcmp(params_select_ns_prefix_fn.custom_id,
             "typed.fn.select.sugar_dummy_typed_handler") != 0 ||
      ...
      extension_message_command.handler !=
          sugar_dummy_message_command_handler ||
      ...
      strcmp(prefix_message_command_fn.name,
             "sugar_prefix_command_alias_handler") != 0 ||
      ...
      strcmp(text_message_command_fn.name,
             "sugar_prefix_command_alias_handler") != 0 ||
      ...
      strcmp(cog.name, "sugar.cog") != 0 || cog.middleware_count != 1U ||
      cog.middlewares == NULL ||
      cog.middlewares[0].middleware != sugar_dummy_middleware ||
      cog.slash_command_count != 2U || cog.slash_commands == NULL ||
      ...
      cog.subcommand_count != 1U || cog.subcommands == NULL ||
      strcmp(cog.subcommands[0].subcommand_path, "admin/reload") != 0 ||
      cog.autocomplete_count != 1U || cog.autocompletes == NULL ||
      strcmp(cog.autocompletes[0].name, "cog_ping") != 0 ||
      cog.component_route_count != 6U || cog.component_routes == NULL ||
      ...
      cog.event_count != 5U || cog.events == NULL ||
      ...
      cog.message_command_count != 2U || cog.message_commands == NULL ||
      ...
      cog.task_count != 4U || cog.tasks == NULL ||
      ...
      cog.tasks[3].hour != 9U || cog.tasks[3].minute != 30U ||
      cog.tasks[3].user_data != &bound_string ||
      strcmp(extension.name, "sugar.extension") != 0 ||
      route_group.size != sizeof(route_group) || route_group.app != NULL ||
      ...
      wait_component_prefix_filter.size !=
          sizeof(wait_component_prefix_filter) ||
      ...
      strcmp(DCC_WAIT_MESSAGE_CONTENT(&wait_message_result, ""), "waited") !=
          0 ||
      ...
      bind_int.type != DCC_CTX_OPTION_BIND_INTEGER || bind_int.required != 1U ||
      ...
      bind_user.type != DCC_CTX_OPTION_BIND_USER || bind_user.required != 1U ||
      bind_user.out != &bound_user ||
      bind_member.type != DCC_CTX_OPTION_BIND_MEMBER ||
      bind_member.required != 0U || bind_member.out != &bound_member ||
      bind_role.type != DCC_CTX_OPTION_BIND_ROLE || bind_role.required != 1U ||
      ...
      form_bindings[0].required != 1U || form_bindings[0].out != &form_string ||
      ...
      option_field_bindings[0].field_offset !=
          offsetof(sugar_field_args_t, name) ||
      ...
      option_field_bindings[5].field_offset !=
          offsetof(sugar_field_args_t, user) ||
      ...
      form_field_bindings[0].field_offset !=
          offsetof(sugar_field_args_t, birthday) ||
      form_field_bindings[1].fallback_boolean != 1U ||
      form_field_bindings[2].field_offset !=
          offsetof(sugar_field_args_t, tags) ||
      form_field_bindings[2].count_offset !=
          offsetof(sugar_field_args_t, tag_count) ||
      form_field_bindings[2].has_count_offset != 1U ||
      component_field_bindings[0].type !=
          DCC_CTX_COMPONENT_BIND_CUSTOM_ID_SUFFIX ||
      component_field_bindings[0].field_offset !=
          offsetof(sugar_field_args_t, name) ||
      strcmp(component_field_bindings[0].custom_id_prefix, "typed.button:") !=
          0 ||
      component_field_bindings[1].type != DCC_CTX_COMPONENT_BIND_FIRST_VALUE ||
      component_field_bindings[1].required != 1U ||
      component_field_bindings[2].field_offset !=
          offsetof(sugar_field_args_t, tags) ||
      component_field_bindings[2].count_offset !=
          offsetof(sugar_field_args_t, tag_count) ||
      ...
      validators[1].min_length != 1U || validators[1].max_length != 16U ||
      validators[2].min_integer != 1 || validators[2].max_integer != 100 ||
      validators[3].min_number != 0.0 || validators[3].max_number != 1.0 ||
      validators[4].type != DCC_CTX_FIELD_VALIDATE_DATE_MMDD_OR_YYYYMMDD ||
      validation_status != DCC_OK || validation_error.status != DCC_OK ||
      invalid_status != DCC_ERR_INVALID_ARG ||
      strcmp(invalid_error.field, "birthday") != 0 ||
      typed_bindings.count != 2U || typed_bindings.items == NULL ||
      short_typed_bindings.count != 8U || short_typed_bindings.items == NULL ||
      ...
      typed_form_bindings.count != 1U || short_form_bindings.count != 2U ||
      ...
      inline_typed_slash.command->options[0].type !=
          DCC_APPLICATION_COMMAND_OPTION_STRING ||
      inline_typed_slash.command->options[0].required != 1U ||
      inline_typed_slash.command->options[1].type !=
      ...
      inline_typed_slash_data.command->options[0].type !=
          DCC_APPLICATION_COMMAND_OPTION_BOOLEAN ||
      ...
      strcmp(no_options_command_route.command->name, "typed_empty_route") !=
          0 ||
      ...
      guarded_params_slash.policy.required_permissions !=
          DCC_PERMISSION_MANAGE_MESSAGES ||
      ...
      typed_subcommand.policy.required_permissions !=
          DCC_PERMISSION_MANAGE_MESSAGES ||
      ...
      strcmp(params_subcommand_fn.subcommand_path,
             "sugar_dummy_typed_handler") != 0 ||
      ...
      strcmp(in_params_subcommand_fn.subcommand_path,
             "admin/sugar_dummy_typed_handler") != 0 ||
      ...
      strcmp(guarded_params_autocomplete.command_name,
             "typed_guarded_params") != 0 ||
      guarded_params_autocomplete.bindings.count != 2U ||
      guarded_params_autocomplete.policy.cooldown.bucket !=
          DCC_APP_COOLDOWN_USER ||
      guarded_params_autocomplete.policy.cooldown.limit != 1U ||
      strcmp(params_autocomplete_fn.command_name,
             "sugar_dummy_typed_handler") != 0 ||
      ...
      strcmp(typed_subcommand_autocomplete.subcommand_path, "admin/reload") !=
          0 ||
      ...
      strcmp(builder_in_subcommand_autocomplete.subcommand_path,
             "admin/complete") != 0 ||
      ...
      typed_modal.bindings.count != 1U || typed_modal.validators.count != 0U ||
      ...
      typed_button.prefix != 1U || typed_button.bindings.count != 2U ||
      ...
      typed_select.prefix != 0U || typed_select.bindings.count != 2U ||
      ...
      strcmp(typed_cog.typed_slash_commands[0].command->name, "typed_empty") !=
          0 ||
      typed_cog.typed_subcommand_count != 1U ||
      typed_cog.typed_subcommands == NULL ||
      strcmp(typed_cog.typed_subcommands[0].subcommand_path, "admin/inspect") !=
          0 ||
      typed_cog.typed_autocomplete_count != 1U ||
      typed_cog.typed_autocompletes == NULL ||
      strcmp(typed_cog.typed_autocompletes[0].command_name, "typed_parent") !=
          0 ||
      typed_cog.typed_modal_count != 1U || typed_cog.typed_modals == NULL ||
      strcmp(typed_cog.typed_modals[0].custom_id, "typed.modal") != 0 ||
      typed_cog.typed_button_count != 1U || typed_cog.typed_buttons == NULL ||
      strcmp(typed_cog.typed_buttons[0].custom_id, "typed.button:") != 0 ||
      typed_cog.typed_select_count != 1U || typed_cog.typed_selects == NULL ||
      strcmp(typed_cog.typed_selects[0].custom_id, "typed.select") != 0 ||
      strcmp(app_definition.name, "sugar.app") != 0 ||
      app_definition.module_count != 1U || app_definition.modules == NULL ||
      ...
      app_definition.event_count != 2U || app_definition.events == NULL ||
      ...
      app_definition.task_count != 2U || app_definition.tasks == NULL ||
      ...
      app_definition.view_count != 1U || app_definition.views == NULL ||
      app_definition.views[0].item_count != 2U ||
      app_definition.views[0].items == NULL ||
      strcmp(app_definition.views[0].items[0].custom_id,
             "app.persistent.button") != 0 ||
      ...
      app_definition.component_sessions[0].session !=
          &sugar_component_session ||
      ...
      app_definition.check_count != 1U || app_definition.checks == NULL ||
      ...
      strcmp(app_definition.checks[0].description, "This app is blocked.") !=
          0 ||
      ...
      dm_app_definition.dm_only != 1U || dm_app_definition.guild_only != 0U ||
      nsfw_app_definition.nsfw_only != 1U ||
      nsfw_app_definition.dm_only != 0U || view.item_count != 3U ||
      view.items == NULL || view.items[0].type != DCC_APP_VIEW_BUTTON ||
      view.items[1].type != DCC_APP_VIEW_SELECT_PREFIX ||
      view.items[2].type != DCC_APP_VIEW_MODAL || registry.guild_id != 123ULL ||
      registry.dry_run != 1U || registry_delete_stale.delete_stale != 1U ||
      registration.guild_id != 0U || guild_registration.guild_id != 456ULL ||
      ...
      session_options.lock_user != 1U || session_options.user_id != 789ULL ||
      session_check.guild_id != 222ULL || listener.listen_buttons != 1U ||
      listener.listen_selects != 1U || listener.listen_forms != 1U ||
      canary.canary_percent != 5U ||
      hot_reload.backend != DCC_HOT_RELOAD_BACKEND_ISOLATED_WORKER ||
      hot_reload.worker_canary_enabled != 1U ||
      firewall.invalid_request_hard_limit !=
      ...
      DCC_APP_PUBLISH_LATEST(NULL, &managed, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_PUBLISH_LATEST_STORE_TEXT(NULL, "latest", 333ULL, "latest", NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_PUBLISH_LATEST_STORE_V2(NULL, "latest", 333ULL, NULL, NULL,
                                      DCC_V2_TEXT("latest")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_PUBLISH_LATEST_STORE_UI(NULL, "latest", 333ULL, NULL, NULL,
                                      DCC_UI_TEXT("latest")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_PUBLISH_LATEST(NULL, &managed, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_SEND(NULL, 333ULL, DCC_MESSAGE_TEXT("send")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_SEND_TEXT(NULL, 333ULL, "send") != DCC_ERR_INVALID_ARG ||
      DCC_INFER_GUILD_FROM_CHANNEL(NULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_channel(NULL, 333ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_channels(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_channel(NULL, 222ULL, "{\"name\":\"general\"}", NULL,
                                   NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_channel_params(NULL, 222ULL, &channel_params, NULL,
                                          NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_channel(NULL, 333ULL, "{\"name\":\"general\"}", NULL,
                             NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_channel_params(NULL, 333ULL, &channel_params, NULL,
                                    NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_channel(NULL, 333ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_trigger_channel_typing(NULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_follow_news_channel(NULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_set_channel_voice_status(NULL, 333ULL, "Streaming", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_current_user_voice_state(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_current_user_voice_state(NULL, 222ULL,
                                              "{\"suppress\":true}", NULL,
                                              NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_current_user_voice_state_params(
          NULL, &voice_state_params, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_user_voice_state(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_user_voice_state(NULL, 222ULL, 333ULL,
                                      "{\"suppress\":true}", NULL,
                                      NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_user_voice_state_params(NULL, &voice_state_params, NULL,
                                             NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_voice_regions(NULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_voice_regions(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_channel_positions(
          NULL, 222ULL, "[{\"id\":\"333\",\"position\":1}]", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_channel_positions_params(
          NULL, 222ULL, &channel_positions_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_channel_invites(NULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_channel_invite(NULL, 333ULL, "{\"max_age\":3600}", NULL,
                                    NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_channel_invite_params(NULL, 333ULL, &invite_params, NULL,
                                           NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_channel_permission(NULL, 333ULL, 444ULL,
                                        "{\"allow\":\"1024\"}", NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_channel_permission_params(NULL, 333ULL,
                                               &permission_overwrite, NULL,
                                               NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_channel_permission(NULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_CHANNEL(NULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_CHANNELS(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_CHANNEL(NULL, 222ULL, "{\"name\":\"general\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_CHANNEL_PARAMS(NULL, 222ULL, &channel_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CHANNEL(NULL, 333ULL, "{\"name\":\"general\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CHANNEL_PARAMS(NULL, 333ULL, &channel_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_CHANNEL(NULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_TRIGGER_CHANNEL_TYPING(NULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_FOLLOW_NEWS_CHANNEL(NULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_SET_CHANNEL_VOICE_STATUS(NULL, 333ULL, "Streaming") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_CURRENT_USER_VOICE_STATE(NULL, 222ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_MY_VOICE_STATE(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CURRENT_USER_VOICE_STATE(
          NULL, 222ULL, "{\"suppress\":true}") != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_MY_VOICE_STATE(NULL, 222ULL, "{\"suppress\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CURRENT_USER_VOICE_STATE_PARAMS(NULL, &voice_state_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_MY_VOICE_STATE_PARAMS(NULL, &voice_state_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_USER_VOICE_STATE(NULL, 222ULL, 333ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_USER_VOICE_STATE(
          NULL, 222ULL, 333ULL, "{\"suppress\":true}") != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_USER_VOICE_STATE_PARAMS(NULL, &voice_state_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_VOICE_REGIONS(NULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_VOICE_REGIONS(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_REORDER_GUILD_CHANNELS(NULL, 222ULL,
                                     "[{\"id\":\"333\",\"position\":1}]") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_REORDER_GUILD_CHANNELS_PARAMS(
          NULL, 222ULL, &channel_positions_params) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_CHANNEL_INVITES(NULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_CHANNEL_INVITE(NULL, 333ULL, "{\"max_age\":3600}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_CHANNEL_INVITE_PARAMS(NULL, 333ULL, &invite_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CHANNEL_PERMISSION(NULL, 333ULL, 444ULL,
                                      "{\"allow\":\"1024\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CHANNEL_PERMISSION_PARAMS(
          NULL, 333ULL, &permission_overwrite) != DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_CHANNEL_PERMISSION(NULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_SEND(NULL, 333ULL, DCC_MESSAGE_TEXT("send")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_SEND_TEXT(NULL, 333ULL, "send") != DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_SEND_SAFE(NULL, 333ULL, "@everyone safe") !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_SEND_UI(NULL, 333ULL, DCC_UI_TEXT("send")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_FETCH(NULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_EDIT(NULL, 333ULL, "{\"name\":\"general\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_EDIT_PARAMS(NULL, 333ULL, &channel_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_CHANNEL_SET_VOICE_STATUS(NULL, 333ULL, "Streaming") !=
          DCC_ERR_INVALID_ARG ||
      DCC_MY_VOICE_STATE_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_MY_VOICE_STATE_EDIT(NULL, 222ULL, "{\"suppress\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_MY_VOICE_STATE_EDIT_PARAMS(NULL, &voice_state_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_USER_VOICE_STATE_FETCH(NULL, 222ULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_USER_VOICE_STATE_EDIT(NULL, 222ULL, 333ULL, "{\"suppress\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_USER_VOICE_STATE_EDIT_PARAMS(NULL, &voice_state_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_CHANNEL_INVITE_CREATE(NULL, 333ULL, "{\"max_age\":3600}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_INVITE_CREATE_PARAMS(NULL, 333ULL, &invite_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_PERMISSION_EDIT(NULL, 333ULL, 444ULL,
                                  "{\"allow\":\"1024\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_PERMISSION_EDIT_PARAMS(NULL, 333ULL, &permission_overwrite) !=
          DCC_ERR_INVALID_ARG ||
      DCC_CHANNEL_PERMISSION_DELETE(NULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_CHANNELS_REORDER(NULL, 222ULL,
                                 "[{\"id\":\"333\",\"position\":1}]") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_CHANNELS_REORDER_PARAMS(
          NULL, 222ULL, &channel_positions_params) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_CHANNELS_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_CHANNEL_CREATE(NULL, 222ULL, "{\"name\":\"general\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_CHANNEL_CREATE_PARAMS(NULL, 222ULL, &channel_params) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_roles(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_role(NULL, 222ULL, "{\"name\":\"role\"}", NULL,
                                NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_role_params(NULL, &role_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_role(NULL, 222ULL, 444ULL, "{\"name\":\"role\"}",
                                NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_role_params(NULL, &role_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_role_positions(NULL, 222ULL, role_positions,
                                          DCC_ARRAY_LEN(role_positions), NULL,
                                          NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_guild_role(NULL, 222ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_ROLES(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_ROLE(NULL, 222ULL, "{\"name\":\"role\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_ROLE_PARAMS(NULL, &role_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_ROLE(NULL, 222ULL, 444ULL, "{\"name\":\"role\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_ROLE_PARAMS(NULL, &role_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_REORDER_GUILD_ROLES(NULL, 222ULL, role_positions,
      ...
      DCC_GUILD_ROLES_REORDER(NULL, 222ULL, role_positions,
                              DCC_ARRAY_LEN(role_positions)) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_ROLE_CREATE(NULL, 222ULL, "{\"name\":\"role\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_ROLE_CREATE_PARAMS(NULL, &role_params) != DCC_ERR_INVALID_ARG ||
      DCC_ROLE_EDIT(NULL, 222ULL, 444ULL, "{\"name\":\"role\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_ROLE_EDIT_PARAMS(NULL, &role_params) != DCC_ERR_INVALID_ARG ||
      DCC_ROLE_DELETE(NULL, 222ULL, 444ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_MESSAGE(NULL, 333ULL, 444ULL, DCC_MESSAGE_TEXT("edit")) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_MESSAGE_EDIT(NULL, 333ULL, 444ULL, DCC_MESSAGE_TEXT("edit")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_EDIT_TEXT(NULL, 333ULL, 444ULL, "edit") !=
          DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_EDIT_UI(NULL, 333ULL, 444ULL, DCC_UI_TEXT("edit")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_DELETE(NULL, 333ULL, 444ULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_crosspost_message(NULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_pin_message(NULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_unpin_message(NULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_add_message_reaction(NULL, 333ULL, 444ULL, "thumbsup", NULL,
                                   NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_own_message_reaction(NULL, 333ULL, 444ULL, "thumbsup",
                                          NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_user_message_reaction(NULL, 333ULL, 444ULL, "thumbsup",
                                           555ULL, NULL,
                                           NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_all_message_reactions(NULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_delete_all_message_reactions_for_emoji(NULL, 333ULL, 444ULL,
                                                     "thumbsup", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_ADD_MESSAGE_REACTION(NULL, 333ULL, 444ULL, "thumbsup") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_OWN_MESSAGE_REACTION(NULL, 333ULL, 444ULL, "thumbsup") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_USER_MESSAGE_REACTION(NULL, 333ULL, 444ULL, "thumbsup",
                                           555ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CLEAR_MESSAGE_REACTIONS(NULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CLEAR_MESSAGE_REACTIONS_FOR_EMOJI(
          NULL, 333ULL, 444ULL, "thumbsup") != DCC_ERR_INVALID_ARG ||
      ...
      DCC_MESSAGE_REACT(NULL, 333ULL, 444ULL, "thumbsup") !=
          DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_UNREACT(NULL, 333ULL, 444ULL, "thumbsup") !=
          DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_REMOVE_REACTION(NULL, 333ULL, 444ULL, "thumbsup", 555ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_CLEAR_REACTIONS(NULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_CLEAR_REACTION_EMOJI(NULL, 333ULL, 444ULL, "thumbsup") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_THREAD_FROM_MESSAGE(NULL, 333ULL, 444ULL, "thread") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_THREAD_FROM_MESSAGE_PARAMS(
          NULL, 333ULL, 444ULL, &thread_params) != DCC_ERR_INVALID_ARG ||
      DCC_MESSAGE_THREAD(NULL, 333ULL, 444ULL, "thread") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_ADD_MEMBER_ROLE(NULL, 222ULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_REMOVE_MEMBER_ROLE(NULL, 222ULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_MEMBER_ADD_ROLE(NULL, 222ULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_MEMBER_REMOVE_ROLE(NULL, 222ULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_MEMBER_ADD_ROLE(NULL, 222ULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_MEMBER_REMOVE_ROLE(NULL, 222ULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_member(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_set_member_timeout(NULL, 222ULL, 333ULL, "2026-06-23T00:00:00Z",
                                 NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_clear_member_timeout(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_move_member(NULL, 222ULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_remove_member(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_MEMBER_FETCH(NULL, 222ULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_MEMBER_TIMEOUT(NULL, 222ULL, 333ULL, "2026-06-23T00:00:00Z") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_GUILD_MEMBER_TIMEOUT(NULL, 222ULL, 333ULL, "2026-06-23T00:00:00Z") !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_bans(NULL, 222ULL, NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_bans_page(NULL, 222ULL, 0U, 0U, 50U, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_ban(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_ban(NULL, 222ULL, 333ULL,
                               "{\"delete_message_seconds\":60}", NULL,
                               NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_ban_seconds(NULL, 222ULL, 333ULL, 60U, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_ban_params(NULL, &ban_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_delete_guild_ban(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_audit_log(NULL, 222ULL, NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_audit_log_page(NULL, 222ULL, 333ULL, 20U, 0U, 0U, 50U,
                                       NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_current_guild_member(NULL, 222ULL, "{\"nick\":\"sugar\"}",
                                          NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_current_guild_member_params(
          NULL, &current_member_params, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_set_current_guild_member_nickname(NULL, 222ULL,
                                                "{\"nick\":\"sugar\"}", NULL,
                                                NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_set_current_guild_member_nickname_params(
          NULL, &nickname_params, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_prune_count_options(NULL, 222ULL, 7U, prune_roles,
                                            DCC_ARRAY_LEN(prune_roles), NULL,
                                            NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_begin_guild_prune_options(NULL, 222ULL, 7U, prune_roles,
                                        DCC_ARRAY_LEN(prune_roles), 1U, NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_begin_guild_prune_params(NULL, &prune_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_integrations(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_integration(NULL, 222ULL, 333ULL,
                                       "{\"expire_behavior\":0}", NULL,
                                       NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_guild_integration(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_sync_guild_integration(NULL, 222ULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_widget(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_widget(NULL, 222ULL, "{\"enabled\":true}", NULL,
                                  NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_widget_params(NULL, 222ULL, &widget_params, NULL,
                                         NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_vanity_url(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_onboarding(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_onboarding(NULL, 222ULL, "{\"enabled\":true}", NULL,
                                      NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_onboarding_params(NULL, &onboarding_params, NULL,
                                             NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_welcome_screen(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_welcome_screen(NULL, 222ULL, "{\"enabled\":true}",
                                          NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_welcome_screen_params(
          NULL, 222ULL, &welcome_params, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_auto_moderation_rules(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_auto_moderation_rule(NULL, 222ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_auto_moderation_rule(NULL, 222ULL, "{\"name\":\"spam\"}",
                                          NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_auto_moderation_rule_params(NULL, 222ULL,
                                                 &auto_mod_rule_params, NULL,
                                                 NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_auto_moderation_rule(NULL, 222ULL, 444ULL,
                                          "{\"enabled\":false}", NULL,
                                          NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_auto_moderation_rule_params(NULL, 222ULL,
                                                 &auto_mod_rule_params, NULL,
                                                 NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_auto_moderation_rule(NULL, 222ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_emojis(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_emoji(NULL, 222ULL, 555ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_emoji(NULL, 222ULL, "{\"name\":\"sugar\"}", NULL,
                                 NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_emoji_params(NULL, 222ULL, &emoji_params, NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_emoji(NULL, 222ULL, 555ULL, "{\"name\":\"sugar\"}",
                                 NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_emoji_params(NULL, 222ULL, &emoji_params, NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_guild_emoji(NULL, 222ULL, 555ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_stickers(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_sticker(NULL, 222ULL, 666ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_sticker_multipart(
          NULL, 222ULL, sticker_fields, DCC_ARRAY_LEN(sticker_fields),
          &sticker_file, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_sticker_params(NULL, &sticker_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_sticker(NULL, 222ULL, 666ULL, "{\"name\":\"sugar\"}",
                                   NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_guild_sticker(NULL, 222ULL, 666ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      ...
      dcc_app_guild_message_search(NULL, 222ULL, "content=sugar", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_guild_message_search_params(NULL, 222ULL, &message_search_params,
                                          NULL, NULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_BANS(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_BANS_QUERY(NULL, 222ULL, "limit=50") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_BANS_PAGE(NULL, 222ULL, 0U, 0U, 50U) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_BAN(NULL, 222ULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_BAN_MEMBER(NULL, 222ULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_BAN_MEMBER_SECONDS(NULL, 222ULL, 333ULL, 60U) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_BAN_MEMBER_JSON(NULL, 222ULL, 333ULL,
      ...
      DCC_APP_EDIT_CURRENT_MEMBER(NULL, 222ULL, "{\"nick\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CURRENT_MEMBER_PARAMS(NULL, &current_member_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_SET_MY_NICK_JSON(NULL, 222ULL, "{\"nick\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_SET_MY_NICK_PARAMS(NULL, &nickname_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_SET_MY_NICK(NULL, 222ULL, "sugar") != DCC_ERR_INVALID_ARG ||
      DCC_CURRENT_MEMBER_EDIT(NULL, 222ULL, "{\"nick\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_CURRENT_MEMBER_EDIT_PARAMS(NULL, &current_member_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_CURRENT_MEMBER_SET_NICK(NULL, 222ULL, "sugar") !=
          DCC_ERR_INVALID_ARG ||
      DCC_ME_EDIT(NULL, 222ULL, "{\"nick\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_GET_GUILD_PRUNE_COUNT_OPTIONS(NULL, 222ULL, 7U, prune_roles,
                                            DCC_ARRAY_LEN(prune_roles)) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_BEGIN_GUILD_PRUNE(NULL, 222ULL, 7U, 1U) != DCC_ERR_INVALID_ARG ||
      DCC_APP_BEGIN_GUILD_PRUNE_OPTIONS(NULL, 222ULL, 7U, prune_roles,
                                        DCC_ARRAY_LEN(prune_roles),
                                        1U) != DCC_ERR_INVALID_ARG ||
      DCC_APP_BEGIN_GUILD_PRUNE_PARAMS(NULL, &prune_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_AUDIT_LOG(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_AUDIT_LOG_QUERY(NULL, 222ULL, "limit=50") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_AUDIT_LOG_PAGE(NULL, 222ULL, 333ULL, 20U, 0U, 0U,
                                       50U) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_INTEGRATIONS(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_INTEGRATION(NULL, 222ULL, 333ULL,
                                     "{\"expire_behavior\":0}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_GUILD_INTEGRATION(NULL, 222ULL, 333ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_SYNC_GUILD_INTEGRATION(NULL, 222ULL, 333ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_WIDGET(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_WIDGET(NULL, 222ULL, "{\"enabled\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_WIDGET_PARAMS(NULL, 222ULL, &widget_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_VANITY_URL(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_ONBOARDING(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_ONBOARDING(NULL, 222ULL, "{\"enabled\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_ONBOARDING_PARAMS(NULL, &onboarding_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_WELCOME_SCREEN(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_WELCOME_SCREEN(NULL, 222ULL, "{\"enabled\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_WELCOME_SCREEN_PARAMS(NULL, 222ULL, &welcome_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_CREATE_AUTO_MOD_RULE(NULL, 222ULL, "{\"name\":\"spam\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_AUTO_MOD_RULE_PARAMS(
          NULL, 222ULL, &auto_mod_rule_params) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_AUTO_MOD_RULE(NULL, 222ULL, 444ULL, "{\"enabled\":false}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_AUTO_MOD_RULE_PARAMS(NULL, 222ULL, &auto_mod_rule_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_AUTO_MOD_RULE(NULL, 222ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_EMOJIS(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_EMOJI(NULL, 222ULL, 555ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_EMOJI(NULL, 222ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_EMOJI_PARAMS(NULL, 222ULL, &emoji_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_EMOJI(NULL, 222ULL, 555ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_EMOJI_PARAMS(NULL, 222ULL, &emoji_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_CREATE_GUILD_STICKER_MULTIPART(
          NULL, 222ULL, sticker_fields, DCC_ARRAY_LEN(sticker_fields),
          &sticker_file) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_STICKER_PARAMS(NULL, &sticker_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_STICKER(NULL, 222ULL, 666ULL,
                                 "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_GUILD_STICKER(NULL, 222ULL, 666ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_STICKER(NULL, 666ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_STICKER_PACKS(NULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GUILD_MESSAGE_SEARCH(NULL, 222ULL, "content=sugar") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GUILD_MESSAGE_SEARCH_PARAMS(
          NULL, 222ULL, &message_search_params) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_BANS_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_BANS_FETCH_PAGE(NULL, 222ULL, 0U, 0U, 50U) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_GUILD_INTEGRATION_EDIT(NULL, 222ULL, 333ULL,
                                 "{\"expire_behavior\":0}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_INTEGRATION_DELETE(NULL, 222ULL, 333ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_INTEGRATION_SYNC(NULL, 222ULL, 333ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_WIDGET_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_WIDGET_EDIT(NULL, 222ULL, "{\"enabled\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_WIDGET_EDIT_PARAMS(NULL, 222ULL, &widget_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_VANITY_URL_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_ONBOARDING_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_ONBOARDING_EDIT(NULL, 222ULL, "{\"enabled\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_ONBOARDING_EDIT_PARAMS(NULL, &onboarding_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_WELCOME_SCREEN_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_WELCOME_SCREEN_EDIT(NULL, 222ULL, "{\"enabled\":true}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_WELCOME_SCREEN_EDIT_PARAMS(NULL, 222ULL, &welcome_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_AUTO_MODERATION_RULE_FETCH(NULL, 222ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_AUTO_MOD_RULE_CREATE(NULL, 222ULL, "{\"name\":\"spam\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_AUTO_MOD_RULE_CREATE_PARAMS(NULL, 222ULL, &auto_mod_rule_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_AUTO_MOD_RULE_EDIT(NULL, 222ULL, 444ULL, "{\"enabled\":false}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_AUTO_MOD_RULE_EDIT_PARAMS(NULL, 222ULL, &auto_mod_rule_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_GUILD_EMOJI_CREATE(NULL, 222ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_EMOJI_CREATE_PARAMS(NULL, 222ULL, &emoji_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_EMOJI_EDIT(NULL, 222ULL, 555ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_EMOJI_EDIT_PARAMS(NULL, 222ULL, &emoji_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_GUILD_STICKER_CREATE_MULTIPART(
          NULL, 222ULL, sticker_fields, DCC_ARRAY_LEN(sticker_fields),
          &sticker_file) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_STICKER_CREATE_PARAMS(NULL, &sticker_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_STICKER_EDIT(NULL, 222ULL, 666ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_GUILD_MESSAGES_SEARCH(NULL, 222ULL, "content=sugar") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_MESSAGES_SEARCH_PARAMS(NULL, 222ULL, &message_search_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_PRUNE_COUNT(NULL, 222ULL, 7U) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_PRUNE_COUNT_OPTIONS(NULL, 222ULL, 7U, prune_roles,
                                    DCC_ARRAY_LEN(prune_roles)) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_PRUNE_BEGIN(NULL, 222ULL, 7U, 1U) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_PRUNE_BEGIN_OPTIONS(NULL, 222ULL, 7U, prune_roles,
                                    DCC_ARRAY_LEN(prune_roles),
                                    1U) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_PRUNE_BEGIN_PARAMS(NULL, &prune_params) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_scheduled_events(NULL, 222ULL, NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_scheduled_events_with_user_count(
          NULL, 222ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_scheduled_event(NULL, 222ULL, 555ULL, NULL, NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_scheduled_event_with_user_count(
          NULL, 222ULL, 555ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_scheduled_event(NULL, 222ULL, "{\"name\":\"event\"}",
                                           NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_scheduled_event_params(
          NULL, &scheduled_event_params, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_scheduled_event(NULL, 222ULL, 555ULL,
                                           "{\"name\":\"event\"}", NULL,
                                           NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_scheduled_event_params(
          NULL, &scheduled_event_params, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_guild_scheduled_event(NULL, 222ULL, 555ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_scheduled_event_users(NULL, 222ULL, 555ULL, NULL, NULL,
                                              NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_scheduled_event_users_page(NULL, 222ULL, 555ULL, 25U,
                                                   0U, 0U, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENTS(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENTS_QUERY(
          NULL, 222ULL, "with_user_count=true") != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENTS_WITH_USER_COUNT(NULL, 222ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENT(NULL, 222ULL, 555ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENT_QUERY(NULL, 222ULL, 555ULL,
                                              "with_user_count=true") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENT_WITH_USER_COUNT(NULL, 222ULL, 555ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_SCHEDULED_EVENT(
          NULL, 222ULL, "{\"name\":\"event\"}") != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_SCHEDULED_EVENT_PARAMS(
          NULL, &scheduled_event_params) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_SCHEDULED_EVENT(NULL, 222ULL, 555ULL,
                                         "{\"name\":\"event\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_SCHEDULED_EVENT_PARAMS(
          NULL, &scheduled_event_params) != DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_GUILD_SCHEDULED_EVENT(NULL, 222ULL, 555ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENT_USERS(NULL, 222ULL, 555ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENT_USERS_QUERY(
          NULL, 222ULL, 555ULL, "limit=25") != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_SCHEDULED_EVENT_USERS_PAGE(
          NULL, 222ULL, 555ULL, 25U, 0U, 0U) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_SCHEDULED_EVENTS_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_SCHEDULED_EVENTS_FETCH_WITH_USER_COUNT(NULL, 222ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_FETCH(NULL, 222ULL, 555ULL) != DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_FETCH_WITH_USER_COUNT(NULL, 222ULL, 555ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_CREATE(NULL, 222ULL, "{\"name\":\"event\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_CREATE_PARAMS(NULL, &scheduled_event_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_EDIT(NULL, 222ULL, 555ULL, "{\"name\":\"event\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_EDIT_PARAMS(NULL, &scheduled_event_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_DELETE(NULL, 222ULL, 555ULL) != DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_USERS_FETCH(NULL, 222ULL, 555ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_SCHEDULED_EVENT_USERS_FETCH_PAGE(NULL, 222ULL, 555ULL, 25U, 0U, 0U) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_channel_webhooks(NULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_webhooks(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_webhook(NULL, 666ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_webhook_with_token(NULL, 666ULL, "token", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_webhook(NULL, 333ULL, "{\"name\":\"sugar\"}", NULL,
                             NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_webhook_params(NULL, 333ULL, &webhook_builder, NULL,
                                    NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook(NULL, 666ULL, "{\"name\":\"sugar\"}", NULL,
                             NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_params(NULL, 666ULL, &webhook_builder, NULL,
                                    NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_with_token(NULL, 666ULL, "token",
                                        "{\"name\":\"sugar\"}", NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_with_token_params(NULL, 666ULL, "token",
                                               &webhook_builder, NULL,
                                               NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_webhook(NULL, 666ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_webhook_with_token(NULL, 666ULL, "token", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_execute_webhook(NULL, 666ULL, "token", NULL,
                              "{\"content\":\"hello\"}", NULL,
                              NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_execute_webhook_options(NULL, 666ULL, "token", 1U, 0U, 0U,
                                      "{\"content\":\"hello\"}", NULL,
                                      NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_execute_webhook_message(NULL, 666ULL, "token", 1U, 0U, 0U,
                                      &managed_message, NULL,
                                      NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_execute_webhook_text(NULL, 666ULL, "token", 1U, 0U, "hello", NULL,
                                   NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_execute_webhook_multipart(NULL, 666ULL, "token", NULL, "{}", NULL,
                                        0U, NULL,
                                        NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_execute_webhook_multipart_options(NULL, 666ULL, "token", 1U, 0U,
                                                0U, "{}", NULL, 0U, NULL,
                                                NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_webhook_message(NULL, 666ULL, "token", 777ULL, NULL, NULL,
                                  NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_webhook_message_thread(NULL, 666ULL, "token", 777ULL, 888ULL,
                                         NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_message(NULL, 666ULL, "token", 777ULL, NULL,
                                     "{\"content\":\"edit\"}", NULL,
                                     NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_message_thread(
          NULL, 666ULL, "token", 777ULL, 888ULL, "{\"content\":\"edit\"}", NULL,
          NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_message_builder(NULL, 666ULL, "token", 777ULL,
                                             NULL, &managed_message, NULL,
                                             NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_message_builder_thread(
          NULL, 666ULL, "token", 777ULL, 888ULL, &managed_message, NULL,
          NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_message_multipart(NULL, 666ULL, "token", 777ULL,
                                               NULL, "{}", NULL, 0U, NULL,
                                               NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_webhook_message_multipart_thread(
          NULL, 666ULL, "token", 777ULL, 888ULL, "{}", NULL, 0U, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_delete_webhook_message(NULL, 666ULL, "token", 777ULL, NULL, NULL,
                                     NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_webhook_message_thread(NULL, 666ULL, "token", 777ULL,
                                            888ULL, NULL,
                                            NULL) != DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_CREATE_WEBHOOK(NULL, 333ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_WEBHOOK_PARAMS(NULL, 333ULL, &webhook_builder) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK(NULL, 666ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_PARAMS(NULL, 666ULL, &webhook_builder) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_TOKEN(NULL, 666ULL, "token",
                                 "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_DELETE_WEBHOOK_TOKEN(NULL, 666ULL, "token") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EXECUTE_WEBHOOK(NULL, 666ULL, "token", NULL,
                              "{\"content\":\"hello\"}") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_EXECUTE_WEBHOOK_MESSAGE(NULL, 666ULL, "token", 1U, 0U, 0U,
          ...
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EXECUTE_WEBHOOK_TEXT(NULL, 666ULL, "token", 1U, 0U, "hello") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EXECUTE_WEBHOOK_MULTIPART(NULL, 666ULL, "token", NULL, "{}", NULL,
                                        0U) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EXECUTE_WEBHOOK_MULTIPART_OPTIONS(NULL, 666ULL, "token", 1U, 0U,
                                                0U, "{}", NULL,
                                                0U) != DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_WEBHOOK_MESSAGE(NULL, 666ULL, "token", 777ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_WEBHOOK_MESSAGE_QUERY(NULL, 666ULL, "token", 777ULL,
                                        "thread_id=888") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_WEBHOOK_MESSAGE_THREAD(NULL, 666ULL, "token", 777ULL,
                                         888ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_MESSAGE(NULL, 666ULL, "token", 777ULL,
                                   "{\"content\":\"edit\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_MESSAGE_QUERY(
          NULL, 666ULL, "token", 777ULL,
          "thread_id=888", "{\"content\":\"edit\"}") != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_MESSAGE_THREAD(NULL, 666ULL, "token", 777ULL, 888ULL,
                                          "{\"content\":\"edit\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_MESSAGE_BUILDER(NULL, 666ULL, "token", 777ULL,
                                           DCC_MESSAGE_TEXT("edit")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_MESSAGE_BUILDER_THREAD(
          NULL, 666ULL, "token", 777ULL, 888ULL, DCC_MESSAGE_TEXT("edit")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_MESSAGE_MULTIPART(NULL, 666ULL, "token", 777ULL,
                                             NULL, "{}", NULL,
                                             0U) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_WEBHOOK_MESSAGE_MULTIPART_THREAD(
          NULL, 666ULL, "token", 777ULL, 888ULL, "{}", NULL, 0U) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_WEBHOOK_MESSAGE(NULL, 666ULL, "token", 777ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_WEBHOOK_MESSAGE_QUERY(NULL, 666ULL, "token", 777ULL,
                                           "thread_id=888") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_WEBHOOK_MESSAGE_THREAD(NULL, 666ULL, "token", 777ULL,
                                            888ULL) != DCC_ERR_INVALID_ARG ||
      ...
      DCC_WEBHOOK_CREATE(NULL, 333ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_CREATE_PARAMS(NULL, 333ULL, &webhook_builder) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_EDIT(NULL, 666ULL, "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_EDIT_PARAMS(NULL, 666ULL, &webhook_builder) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_EDIT_TOKEN(NULL, 666ULL, "token", "{\"name\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_EDIT_TOKEN_PARAMS(NULL, 666ULL, "token", &webhook_builder) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_DELETE(NULL, 666ULL) != DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_DELETE_TOKEN(NULL, 666ULL, "token") != DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_JSON(NULL, 666ULL, "token", "{\"content\":\"hello\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_JSON_QUERY(NULL, 666ULL, "token", "wait=true",
                                  "{\"content\":\"hello\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_OPTIONS(NULL, 666ULL, "token", 1U, 0U, 0U,
                               "{\"content\":\"hello\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND(NULL, 666ULL, "token", DCC_MESSAGE_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_WAIT(NULL, 666ULL, "token", DCC_MESSAGE_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_THREAD(NULL, 666ULL, "token", 888ULL,
                              DCC_MESSAGE_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_TEXT(NULL, 666ULL, "token", "hello") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_TEXT_WAIT(NULL, 666ULL, "token", "hello") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_UI(NULL, 666ULL, "token", DCC_UI_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_SEND_UI_WAIT(NULL, 666ULL, "token", DCC_UI_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_MESSAGE_FETCH(NULL, 666ULL, "token", 777ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_MESSAGE_FETCH_THREAD(NULL, 666ULL, "token", 777ULL, 888ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_MESSAGE_EDIT_JSON(NULL, 666ULL, "token", 777ULL,
                                    "{\"content\":\"edit\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_MESSAGE_EDIT(NULL, 666ULL, "token", 777ULL,
                               DCC_MESSAGE_TEXT("edit")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_MESSAGE_EDIT_THREAD(NULL, 666ULL, "token", 777ULL, 888ULL,
                                      DCC_MESSAGE_TEXT("edit")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_MESSAGE_DELETE(NULL, 666ULL, "token", 777ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_WEBHOOK_MESSAGE_DELETE_THREAD(NULL, 666ULL, "token", 777ULL,
                                        888ULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_current_application(NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_current_user(NULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_current_user(NULL, "{\"username\":\"sugar\"}", NULL,
                                  NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_current_user_params(NULL, &user_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_user(NULL, 333ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_current_user_connections(NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_current_user_guilds(NULL, NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_current_user_dms(NULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_dm_channel(NULL, "{\"recipient_id\":\"333\"}", NULL,
                                NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_dm_channel_params(NULL, &dm_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_add_group_dm_recipient(NULL, 333ULL, 444ULL,
                                     "{\"access_token\":\"token\"}", NULL,
                                     NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_add_group_dm_recipient_params(NULL, &group_dm_recipient_params,
                                            NULL,
                                            NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_remove_group_dm_recipient(NULL, 333ULL, 444ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_direct_message(NULL, 333ULL, "{\"content\":\"hello\"}",
                                    NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_direct_message_builder(
          NULL, 333ULL, &managed_message, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_direct_message_text(NULL, 333ULL, "hello", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_leave_guild(NULL, 222ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_invites(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_invite(NULL, "abc", NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      ...
      dcc_app_create_stage_instance(
          NULL, "{\"channel_id\":\"333\",\"topic\":\"sugar\"}", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_stage_instance_params(NULL, &stage_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_stage_instance(NULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_modify_stage_instance(NULL, 333ULL, "{\"topic\":\"sugar\"}", NULL,
                                    NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_stage_instance_params(NULL, &stage_params, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_delete_stage_instance(NULL, 333ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_template(NULL, "abc", NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_from_template(NULL, "abc", "{\"name\":\"guild\"}",
                                         NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_from_template_params(NULL, &template_params, NULL,
                                                NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_guild_templates(NULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_template(NULL, 222ULL, "{\"name\":\"template\"}",
                                    NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_guild_template_params(NULL, &template_params, NULL,
                                           NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_template(NULL, 222ULL, "abc",
                                    "{\"name\":\"template\"}", NULL,
                                    NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_modify_guild_template_params(NULL, &template_params, NULL,
                                           NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_sync_guild_template(NULL, 222ULL, "abc", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_delete_guild_template(NULL, 222ULL, "abc", NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_gateway_bot(NULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_skus(NULL, 111ULL, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_get_entitlements(NULL, 111ULL, NULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_get_entitlements_page(NULL, 111ULL, 333ULL, entitlement_sku_ids,
                                    DCC_ARRAY_LEN(entitlement_sku_ids), 0U, 0U,
                                    25U, 222ULL, 1U, NULL,
                                    NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_test_entitlement(NULL, 111ULL, "{\"sku_id\":\"222\"}",
                                      NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_create_test_entitlement_params(
          NULL, &test_entitlement_params, NULL, NULL) != DCC_ERR_INVALID_ARG ||
      dcc_app_delete_test_entitlement(NULL, 111ULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_consume_entitlement(NULL, 111ULL, 222ULL, NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_GET_ENTITLEMENTS_QUERY(NULL, 111ULL, "limit=25") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_ENTITLEMENTS_PAGE(NULL, 111ULL, 333ULL, entitlement_sku_ids,
                                    DCC_ARRAY_LEN(entitlement_sku_ids), 0U, 0U,
                                    25U, 222ULL, 1U) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_TEST_ENTITLEMENT(NULL, 111ULL, "{\"sku_id\":\"222\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_TEST_ENTITLEMENT_PARAMS(NULL, &test_entitlement_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_TEST_ENTITLEMENT(NULL, 111ULL, 222ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CONSUME_ENTITLEMENT(NULL, 111ULL, 222ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_CURRENT_USER(NULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CURRENT_USER(NULL, "{\"username\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_CURRENT_USER_PARAMS(NULL, &user_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_GET_CURRENT_USER_GUILDS_QUERY(NULL, "limit=25") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_CURRENT_USER_DMS(NULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_DM_CHANNEL(NULL, "{\"recipient_id\":\"333\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_DM_CHANNEL_PARAMS(NULL, &dm_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_ADD_GROUP_DM_RECIPIENT(NULL, 333ULL, 444ULL,
                                     "{\"access_token\":\"token\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_ADD_GROUP_DM_RECIPIENT_PARAMS(NULL, &group_dm_recipient_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_REMOVE_GROUP_DM_RECIPIENT(NULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_DIRECT_MESSAGE(NULL, 333ULL, "{\"content\":\"hello\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_DIRECT_MESSAGE_BUILDER(
          NULL, 333ULL, DCC_MESSAGE_TEXT("hello")) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_DIRECT_MESSAGE_TEXT(NULL, 333ULL, "hello") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_APP_CREATE_GUILD_FROM_TEMPLATE(NULL, "abc", "{\"name\":\"guild\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_FROM_TEMPLATE_PARAMS(NULL, &template_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_GET_GUILD_TEMPLATES(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_TEMPLATE(NULL, 222ULL, "{\"name\":\"template\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_CREATE_GUILD_TEMPLATE_PARAMS(NULL, &template_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_TEMPLATE(NULL, 222ULL, "abc",
                                  "{\"name\":\"template\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_EDIT_GUILD_TEMPLATE_PARAMS(NULL, &template_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_APP_SYNC_GUILD_TEMPLATE(NULL, 222ULL, "abc") != DCC_ERR_INVALID_ARG ||
      DCC_APP_DELETE_GUILD_TEMPLATE(NULL, 222ULL, "abc") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_ENTITLEMENTS_FETCH_QUERY(NULL, 111ULL, "limit=25") !=
          DCC_ERR_INVALID_ARG ||
      DCC_ENTITLEMENTS_FETCH_PAGE(NULL, 111ULL, 333ULL, entitlement_sku_ids,
                                  DCC_ARRAY_LEN(entitlement_sku_ids), 0U, 0U,
                                  25U, 222ULL, 1U) != DCC_ERR_INVALID_ARG ||
      DCC_TEST_ENTITLEMENT_CREATE(NULL, 111ULL, "{\"sku_id\":\"222\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_TEST_ENTITLEMENT_CREATE_PARAMS(NULL, &test_entitlement_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_TEST_ENTITLEMENT_DELETE(NULL, 111ULL, 222ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_ENTITLEMENT_CONSUME(NULL, 111ULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_ME_FETCH(NULL) != DCC_ERR_INVALID_ARG ||
      DCC_ME_EDIT_USER(NULL, "{\"username\":\"sugar\"}") !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_GROUP_DM_RECIPIENT_ADD(NULL, 333ULL, 444ULL,
                                 "{\"access_token\":\"token\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GROUP_DM_RECIPIENT_ADD_PARAMS(NULL, &group_dm_recipient_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GROUP_DM_RECIPIENT_REMOVE(NULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_USER_DM_SEND_JSON(NULL, 333ULL, "{\"content\":\"hello\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_USER_DM_SEND(NULL, 333ULL, DCC_MESSAGE_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_USER_DM_SEND_TEXT(NULL, 333ULL, "hello") != DCC_ERR_INVALID_ARG ||
      DCC_USER_DM_SEND_UI(NULL, 333ULL, DCC_UI_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_DM_SEND(NULL, 333ULL, DCC_MESSAGE_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      DCC_DM_SEND_TEXT(NULL, 333ULL, "hello") != DCC_ERR_INVALID_ARG ||
      DCC_DM_SEND_UI(NULL, 333ULL, DCC_UI_TEXT("hello")) !=
          DCC_ERR_INVALID_ARG ||
      ...
      DCC_TEMPLATE_CREATE_GUILD(NULL, "abc", "{\"name\":\"guild\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_TEMPLATE_CREATE_GUILD_PARAMS(NULL, &template_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_TEMPLATES_FETCH(NULL, 222ULL) != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_TEMPLATE_CREATE(NULL, 222ULL, "{\"name\":\"template\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_TEMPLATE_CREATE_PARAMS(NULL, &template_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_TEMPLATE_EDIT(NULL, 222ULL, "abc", "{\"name\":\"template\"}") !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_TEMPLATE_EDIT_PARAMS(NULL, &template_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_TEMPLATE_SYNC(NULL, 222ULL, "abc") != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_TEMPLATE_DELETE(NULL, 222ULL, "abc") != DCC_ERR_INVALID_ARG ||
      DCC_GUILD_MEMBER_CLEAR_TIMEOUT(NULL, 222ULL, 333ULL) !=
          DCC_ERR_INVALID_ARG ||
      DCC_GUILD_MEMBER_MOVE(NULL, 222ULL, 333ULL, 444ULL) !=
          DCC_ERR_INVALID_ARG ||
...
static int run_sugar_context_smoke(void) {
  ...
  const char **field_values =
      DCC_FIELD_VALUES_RAW(&ctx, "choices", &field_value_count);

  if (values != component_values || value_count != 2U ||
      DCC_CTX_ARG(&ctx, "name") != &options[0] ||
      DCC_CTX_ARG_PRESENT(&ctx, "name") != 1U ||
      strcmp(DCC_CTX_ARG_STRING(&ctx, "name"), "feral") != 0 ||
      strcmp(DCC_CTX_ARG_STRING_OR(&ctx, "missing", "fallback"), "fallback") !=
          0 ||
      ...
      strcmp(DCC_CTX_COMPONENT_ACTION_OR(&ctx, "missing", "fallback"),
             "fallback") != 0 ||
      ...
      field_values != form_values || field_value_count != 2U ||
      DCC_CTX_FORM_VALUE_COUNT(&ctx, "choices") != 2U ||
      strcmp(DCC_CTX_FORM_VALUE(&ctx, "choices", 1U), "blue") != 0 ||
      strcmp(DCC_CTX_FORM_FIRST_VALUE(&ctx, "choices", "fallback"), "red") !=
          0 ||
      ...
      strcmp(DCC_FIELD_FIRST_VALUE_OR(&ctx, "missing", "fallback"),
             "fallback") != 0 ||
  ...
  if (DCC_CTX_HANDLE_ERROR(NULL, DCC_OK, NULL) != DCC_OK ||
      ...
      error_state.count != 1U || error_state.status != DCC_ERR_STATE ||
  ...
  if (error_state.after_try != 0U || error_state.count != 2U ||
  ...
  value_count = 777U;
  if (DCC_CTX_SELECTED_VALUES(NULL, &value_count) != NULL ||
      value_count != 0U) {
  ...
  if (DCC_CTX_APP(NULL) != NULL || DCC_CTX_CLIENT(NULL) != NULL ||
      DCC_CTX_EVENT(NULL) != NULL || DCC_CTX_INTERACTION(NULL) != NULL ||
      DCC_CTX_FLOW(NULL) != NULL || DCC_CTX_EVENT_TYPE(NULL) != DCC_EVENT_MAX ||
      DCC_CTX_INTERACTION_ID(NULL) != 0U ||
      DCC_CTX_APPLICATION_ID(NULL) != 0U || DCC_CTX_GUILD_ID(NULL) != 0U ||
      DCC_CTX_CHANNEL_ID(NULL) != 0U || DCC_CTX_COMMAND_ID(NULL) != 0U ||
      DCC_CTX_MESSAGE_ID(NULL) != 0U || DCC_CTX_TARGET_ID(NULL) != 0U ||
      DCC_CTX_USER_ID(NULL) != 0U || DCC_CTX_AUTHOR_ID(NULL) != 0U ||
      DCC_CTX_USER_DATA(NULL) != NULL || DCC_CTX_TOKEN(NULL) != NULL ||
      DCC_CTX_NAME(NULL) != NULL || DCC_CTX_COMMAND_NAME(NULL) != NULL ||
      DCC_CTX_CUSTOM_ID(NULL) != NULL || DCC_CTX_SUBCOMMAND(NULL) != NULL ||
      ...
      DCC_CTX_RESPONDED(NULL) != 0U || DCC_CTX_RESPONSE_SENT(NULL) != 0U ||
      DCC_CTX_DEFERRED(NULL) != 0U || DCC_CTX_REPLIED(NULL) != 0U ||
      DCC_CTX_FOLLOWED_UP(NULL) != 0U || DCC_CTX_RESPONSE_FAILED(NULL) != 1U ||
      ...
      DCC_CTX_MEMBER_HAS_PERMISSIONS(NULL, DCC_PERMISSION_MANAGE_MESSAGES) !=
          0U ||
      DCC_CTX_HAS_PERMISSIONS(NULL, DCC_PERMISSION_MANAGE_MESSAGES) != 0U ||
      DCC_CTX_IS_GUILD(NULL) != 0U || DCC_CTX_IS_DM(NULL) != 0U ||
      ...
      strcmp(DCC_CTX_COMPONENT_ACTION_OR(NULL, "birthday", "fallback"),
             "fallback") != 0 ||
      ...
      strcmp(DCC_CTX_ARG_STRING_OR(NULL, "name", "fallback"), "fallback") !=
          0 ||
      ...
      strcmp(DCC_CTX_OPTION_TEXT_OR(NULL, "name", "fallback"), "fallback") !=
          0 ||
      ...
      DCC_PARAM_FOCUSED(NULL) != NULL || DCC_PARAM_FOCUSED_NAME(NULL) != NULL ||
      ...
      strcmp(DCC_CTX_FORM_STRING_OR(NULL, "birthday", "20070101"),
             "20070101") != 0 ||
      strcmp(DCC_CTX_FORM_TEXT(NULL, "birthday", "20070101"), "20070101") !=
          0 ||
      strcmp(DCC_CTX_FORM_TEXT_OR(NULL, "birthday", "20070101"), "20070101") !=
          0 ||
      ...
      strcmp(DCC_CTX_FIELD_TEXT_OR(NULL, "birthday", "20070101"), "20070101") !=
          0 ||
      ...
      strcmp(DCC_FIELD_TEXT_OR(NULL, "birthday", "20070101"), "20070101") !=
          0 ||
      ...
      strcmp(DCC_CTX_FIELD_FIRST_VALUE_OR(NULL, "choices", "fallback"),
             "fallback") != 0 ||
      ...
      DCC_SELECTED_COUNT(NULL) != 0U || DCC_SELECTED_VALUE(NULL, 0U) != NULL ||
      DCC_SELECTED_FIRST(NULL) != NULL ||
      strcmp(DCC_SELECTED_FIRST_OR(NULL, "fallback"), "fallback") != 0 ||
      DCC_SELECT_COUNT(NULL) != 0U || DCC_SELECT_VALUE_AT(NULL, 0U) != NULL ||
      ...
      DCC_CTX_THREAD_FROM_MESSAGE_PARAMS(NULL, &thread_params) !=
          DCC_ERR_INVALID_ARG ||
      DCC_CTX_CREATE_THREAD(NULL, "thread") != DCC_ERR_INVALID_ARG ||
      DCC_CTX_CREATE_THREAD_PARAMS(NULL, &thread_params) !=
          DCC_ERR_INVALID_ARG ||
      ...
      strcmp(dcc_sugar_ctx_text_suffix_after("prefix:value", "prefix:"),
             "value") != 0) {
...
static int run_sugar_store_smoke(void) {
  ...
  if (DCC_CTX_STORE(NULL) != NULL || DCC_CTX_HAS_STORE(NULL) != 0U ||
      DCC_APP_HAS_STORE(NULL) != 0U || DCC_CTX_STORE(&ctx) != NULL ||
  ...
  if (DCC_APP_STORE_FILE(&app, path) != DCC_OK ||
      ...
      DCC_CTX_HAS_STORE(&ctx) != 1U || DCC_APP_HAS_STORE(&app) != 1U ||
      ...
      DCC_CTX_STORE_GET(&ctx, "name", value, sizeof(value), &value_len) !=
          DCC_OK ||
      value_len != 3U || strcmp(value, "dcc") != 0 ||
      DCC_CTX_STORE_DELETE(&ctx, "name") != DCC_OK ||
      DCC_APP_STORE_GET(&app, "name", value, sizeof(value), NULL) !=
          DCC_ERR_NOT_FOUND) {
...
static int run_sugar_typed_handler_impl_smoke(void) {
  sugar_typed_impl_args_t args = {.value = "ok", .count = 7U};
  uint32_t total = 0U;
