...
static dcc_status_t stop_wait_server(
    ...
    dcc_status_t stop_status = dcc_interaction_server_stop(server);
    if (!wait_started) {
        return stop_status;
    }
...
int main(void) {
    ...
    dcc_cluster_options_t cluster_options = {
        .size = sizeof(cluster_options),
        .client_options = {
    ...
    dcc_status_t status = dcc_cluster_create(&cluster_options, &cluster);
    if (status != DCC_OK || cluster == NULL) {
    ...
    if (dcc_cluster_add_status_route(NULL, "/clusterz", cluster) != DCC_ERR_INVALID_ARG ||
        dcc_cluster_add_status_route(server, "/clusterz", NULL) != DCC_ERR_INVALID_ARG ||
        dcc_cluster_add_status_route(server, "/clusterz", cluster) != DCC_OK) {
        fprintf(stderr, "cluster status route setup contract failed\n");
    ...
    if (!send_get(port, "/clusterz", &response) ||
        response.status != 200 ||
        strstr(response.body, "\"recovery\":{") == NULL ||
        strstr(response.body, "\"operation\":{") == NULL ||
        strstr(response.body, "\"supervisor\":{") == NULL ||
        strstr(response.body, "\"health\":\"starting\"") == NULL ||
        strstr(response.body, "\"type\":\"none\"") == NULL ||
        strstr(response.body, "\"decisions\":0") == NULL ||
        strstr(response.body, "\"last_status\":\"ok\"") == NULL) {
        fprintf(stderr, "cluster status route response mismatch: %d %s\n", response.status, response.body);
        (void)stop_wait_server(server, wait_thread, wait_started);
        dcc_interaction_server_destroy(server);
        dcc_cluster_destroy(cluster);
        return 1;
    ...
    if (dcc_cluster_add_status_route(server, "/late-clusterz", cluster) != DCC_ERR_STATE) {
        fprintf(stderr, "cluster status route runtime mutation contract failed\n");
        (void)stop_wait_server(server, wait_thread, wait_started);
        dcc_interaction_server_destroy(server);
        dcc_cluster_destroy(cluster);
        return 1;
