...
static void app_smoke_infer_guild_cb(dcc_app_t *app, dcc_snowflake_t channel_id,
                                     dcc_snowflake_t guild_id,
                                     dcc_status_t status, void *user_data) {
  (void)app;
  app_smoke_infer_guild_state_t *state =
      (app_smoke_infer_guild_state_t *)user_data;
  if (state == NULL) {
    return;
  }
  state->called++;
...
static dcc_status_t middleware(dcc_ctx_t *ctx, void *user_data) {
  ...
  if (dcc_ctx_app(ctx) == NULL || dcc_ctx_client(ctx) == NULL ||
      ...
      DCC_CTX_RESPONDED(ctx) || DCC_CTX_DEFERRED(ctx) || DCC_CTX_REPLIED(ctx) ||
...
static dcc_status_t route_middleware(dcc_ctx_t *ctx, void *user_data) {
  ...
  if (DCC_CTX_EVENT_TYPE(ctx) != DCC_EVENT_BUTTON_CLICK ||
      !DCC_CTX_HAS_CUSTOM_ID(ctx, "page:next") || action == NULL ||
...
static void on_slash(dcc_ctx_t *ctx, void *user_data) {
  ...
  dcc_status_t bind_status = DCC_CTX_BIND_OPTIONS(
      ctx, DCC_CTX_OPTION_REQUIRED_STRING("name", &bound_name),
  ...
  dcc_status_t missing_status = DCC_CTX_BIND_OPTIONS(
      ctx, DCC_CTX_OPTION_REQUIRED_STRING("missing", &missing_required));
  app_smoke_slash_args_t args;
  memset(&args, 0, sizeof(args));
  dcc_status_t field_bind_status = DCC_CTX_BIND_OPTION_FIELDS(
      ctx, &args,
      DCC_CTX_OPTION_FIELD_REQUIRED_STRING(app_smoke_slash_args_t, name,
                                           "name"),
      ...
      DCC_CTX_OPTION_FIELD_REQUIRED_USER(app_smoke_slash_args_t, user_object,
                                         "user"),
      DCC_CTX_OPTION_FIELD_REQUIRED_MEMBER(app_smoke_slash_args_t,
                                           member_object, "user"),
      DCC_CTX_OPTION_FIELD_REQUIRED_ROLE(app_smoke_slash_args_t, role_object,
                                         "role"),
      DCC_CTX_OPTION_FIELD_REQUIRED_CHANNEL(app_smoke_slash_args_t,
                                            channel_object, "channel"),
      DCC_CTX_OPTION_FIELD_REQUIRED_MESSAGE(app_smoke_slash_args_t,
                                            message_object, "message"),
      DCC_CTX_OPTION_FIELD_REQUIRED_ATTACHMENT(app_smoke_slash_args_t,
                                               attachment_object, "attachment"),
      DCC_CTX_OPTION_FIELD_INT(app_smoke_slash_args_t, missing_count,
                               "missing_count", 99));
  ...
  dcc_status_t validation_status = DCC_CTX_VALIDATE_FIELDS(
      ...
      DCC_VALIDATE_NUMBER_RANGE(app_smoke_slash_args_t, ratio, "ratio", 0.0,
                                1.0));
  ...
  const dcc_attachment_t *option_attachment =
      DCC_CTX_OPT_ATTACHMENT(ctx, "attachment");
  ...
  if (DCC_CTX_EVENT_TYPE(ctx) != DCC_EVENT_SLASH_COMMAND ||
      ...
      DCC_CTX_OPT_PRESENT(ctx, "missing") != 0U || bind_status != DCC_OK ||
      strcmp(bound_name, "feral") != 0 || bound_count != 7 ||
      bound_ratio < 0.24 || bound_ratio > 0.26 || bound_flag != 1U ||
      bound_user != 42U || bound_user_object != option_user ||
      ...
      bound_attachment_object != option_attachment || fallback_count != 99 ||
      missing_status != DCC_ERR_NOT_FOUND || field_bind_status != DCC_OK ||
      ...
      strcmp(args.name, "feral") != 0 || args.count != 7 || args.ratio < 0.24 ||
      args.ratio > 0.26 || args.flag != 1U || args.user != 42U ||
      args.user_object != option_user || args.member_object != option_member ||
      args.role_object != option_role ||
      args.channel_object != option_channel ||
      args.message_object != option_message ||
      args.attachment_object != option_attachment || args.missing_count != 99 ||
      DCC_CTX_CLIENT(ctx) == NULL || DCC_CTX_APP(ctx) == NULL ||
      DCC_CTX_INTERACTION(ctx) == NULL || DCC_CTX_INTERACTION_ID(ctx) != 100U ||
      DCC_CTX_APPLICATION_ID(ctx) != 200U || DCC_CTX_GUILD_ID(ctx) != 300U ||
      DCC_CTX_CHANNEL_ID(ctx) != 400U || DCC_CTX_COMMAND_ID(ctx) != 500U ||
      DCC_CTX_AUTHOR_ID(ctx) != 803U || DCC_CTX_USER_ID(ctx) != 803U ||
      ctx_guild == NULL || ctx_guild->id != 300U || ctx_channel == NULL ||
      ctx_channel->id != 400U || ctx_user == NULL || ctx_user->id != 803U ||
      DCC_CTX_AUTHOR(ctx) != ctx_user || ctx_member == NULL ||
      ctx_member->user.id != 803U || option_user == NULL ||
      option_user->id != 42U ||
      strcmp(option_user->username, "option-user") != 0 ||
      option_member == NULL || option_member->user.id != 42U ||
      strcmp(option_member->nick, "Option Member") != 0 ||
      option_role == NULL || option_role->id != 910U ||
      strcmp(option_role->name, "Mod") != 0 || option_channel == NULL ||
      option_channel->id != 401U ||
      strcmp(option_channel->name, "general") != 0 || option_message == NULL ||
      option_message->id != 700U ||
      strcmp(option_message->content, "resolved message") != 0 ||
      option_attachment == NULL || option_attachment->id != 88U ||
      ...
      target_user != option_user || target_member != option_member ||
      !DCC_CTX_IS_GUILD(ctx) || DCC_CTX_IS_DM(ctx) ||
      !DCC_CTX_IS_AUTHOR(ctx, 803U) || DCC_CTX_IS_AUTHOR(ctx, 0U) ||
      DCC_CTX_HAS_MEMBER_PERMISSIONS(ctx) != 1U ||
      DCC_CTX_MEMBER_PERMISSIONS(ctx) != DCC_PERMISSION_MANAGE_MESSAGES ||
      DCC_CTX_MEMBER_HAS_PERMISSIONS(ctx, DCC_PERMISSION_MANAGE_MESSAGES) !=
          1U ||
...
static void on_generic_event(dcc_app_t *app, const dcc_event_t *event,
                             void *user_data) {
  app_smoke_state_t *state = (app_smoke_state_t *)user_data;
  state->generic_event_count++;
  if (app == NULL || event == NULL ||
      dcc_event_type(event) != DCC_EVENT_RESUMED) {
    state->bad = 1U;
  }
}

static void on_ready_event(dcc_app_t *app, const dcc_ready_event_t *ready,
                           void *user_data) {
  app_smoke_state_t *state = (app_smoke_state_t *)user_data;
  state->ready_count++;
  if (app == NULL || ready == NULL || ready->shard_id != 1U ||
      ready->shard_count != 2U ||
      strcmp(ready->session_id, "ready-session") != 0) {
    state->bad = 1U;
  }
}

static void on_message_create_event(dcc_app_t *app,
  ...
  if (app == NULL || message == NULL || event == NULL ||
      ...
      message->id != 700U || strcmp(message->content, "hello app") != 0) {
    state->bad = 1U;
  }
}

static void on_extension_ready_event(dcc_app_t *app,
...
static void on_extension_message_update_event(dcc_app_t *app,
                                              const dcc_message_t *message,
  ...
  if (app == NULL || message == NULL || event == NULL ||
      ...
      message->id != 701U || strcmp(message->content, "hello extension") != 0) {
    state->bad = 1U;
  }
}
...
static void on_autocomplete(dcc_ctx_t *ctx, void *user_data) {
  ...
  const dcc_interaction_option_t *focused =
      dcc_interaction_focused_option(dcc_ctx_interaction(ctx));
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_AUTOCOMPLETE || focused == NULL ||
      focused->name == NULL || strcmp(focused->name, "name") != 0 ||
...
static void on_button(dcc_ctx_t *ctx, void *user_data) {
  ...
  if (DCC_CTX_EVENT_TYPE(ctx) != DCC_EVENT_BUTTON_CLICK ||
      DCC_CTX_INTERACTION(ctx) == NULL || DCC_CTX_MESSAGE_ID(ctx) != 600U ||
      !DCC_CTX_CUSTOM_ID_STARTS_WITH(ctx, "page:") ||
      strcmp(DCC_CTX_CUSTOM_ID_OR(ctx, ""), "page:next") != 0 ||
      action == NULL || strcmp(action, "next") != 0) {
    state->bad = 1U;
  }
}
...
static void on_nsfw_button(dcc_ctx_t *ctx, void *user_data) {
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_BUTTON_CLICK ||
      ...
      !dcc_ctx_channel_is_nsfw(ctx) || !DCC_CTX_CHANNEL_IS_NSFW(ctx)) {
    state->bad = 1U;
  }
}
...
static void on_modal(dcc_ctx_t *ctx, void *user_data) {
  ...
  dcc_status_t bind_status = DCC_CTX_BIND_FORM(
      ctx, DCC_CTX_FORM_REQUIRED_STRING("birthday", &birthday),
  ...
  const char *missing_required = NULL;
  dcc_status_t missing_status = DCC_CTX_BIND_FORM(
      ctx, DCC_CTX_FORM_REQUIRED_STRING("missing_required", &missing_required));
  app_smoke_modal_args_t args;
  memset(&args, 0, sizeof(args));
  dcc_status_t field_bind_status = DCC_CTX_BIND_FORM_FIELDS(
      ctx, &args,
      DCC_CTX_FORM_FIELD_REQUIRED_STRING(app_smoke_modal_args_t, birthday,
                                         "birthday"),
      DCC_CTX_FORM_FIELD_REQUIRED_BOOL(app_smoke_modal_args_t, public_birth,
                                       "public"),
      DCC_CTX_FORM_FIELD_REQUIRED_VALUES(app_smoke_modal_args_t, tags,
                                         tag_count, "tags"),
      DCC_CTX_FORM_FIELD_STRING(app_smoke_modal_args_t, fallback, "missing",
                                "fallback"));
  dcc_ctx_validation_error_t validation_error;
  dcc_status_t validation_status = DCC_CTX_VALIDATE_FIELDS(
      &args, &validation_error,
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_FORM_SUBMIT ||
      ...
      dcc_ctx_form_bool(ctx, "public", 0) != 1U || bind_status != DCC_OK ||
      strcmp(birthday, "20070101") != 0 || public_birth != 1U || tags == NULL ||
      tag_count != 2U || strcmp(tags[0], "alpha") != 0 ||
      strcmp(tags[1], "beta") != 0 || strcmp(fallback, "fallback") != 0 ||
      missing_status != DCC_ERR_NOT_FOUND || field_bind_status != DCC_OK ||
      validation_status != DCC_OK || validation_error.status != DCC_OK ||
      invalid_status != DCC_ERR_INVALID_ARG ||
      strcmp(invalid_error.field, "birthday") != 0 ||
      strcmp(args.birthday, "20070101") != 0 || args.public_birth != 1U ||
      args.tags == NULL || args.tag_count != 2U ||
      strcmp(args.tags[0], "alpha") != 0 || strcmp(args.tags[1], "beta") != 0 ||
...
static void on_typed_slash(dcc_ctx_t *ctx, void *args, void *user_data) {
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_SLASH_COMMAND ||
      typed_args == NULL || typed_args->name == NULL ||
      strcmp(typed_args->name, "typed-feral") != 0 || typed_args->limit != 12 ||
      dcc_ctx_user_data(ctx) != state) {
    state->bad = 1U;
  }
}
...
static void on_typed_subcommand(dcc_ctx_t *ctx, void *args, void *user_data) {
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_SLASH_COMMAND ||
      dcc_ctx_subcommand_path(ctx, path, sizeof(path)) != DCC_OK ||
      strcmp(path, "moderation/ban") != 0 || typed_args == NULL ||
      typed_args->name == NULL ||
      strcmp(typed_args->name, "typed-sub-feral") != 0 ||
      typed_args->limit != 15 || dcc_ctx_user_data(ctx) != state) {
    state->bad = 1U;
  }
}
...
static void on_typed_autocomplete(dcc_ctx_t *ctx, void *args, void *user_data) {
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_AUTOCOMPLETE ||
      ...
      typed_args->limit != 10 || dcc_ctx_user_data(ctx) != state) {
    state->bad = 1U;
  }
}

static void on_typed_subcommand_autocomplete(dcc_ctx_t *ctx, void *args,
                                             void *user_data) {
  app_smoke_state_t *state = (app_smoke_state_t *)user_data;
  const app_smoke_typed_slash_args_t *typed_args =
      (const app_smoke_typed_slash_args_t *)args;
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_AUTOCOMPLETE ||
      ...
      typed_args->limit != 18 || dcc_ctx_user_data(ctx) != state) {
    state->bad = 1U;
  }
}
...
static void on_typed_modal(dcc_ctx_t *ctx, void *args, void *user_data) {
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_FORM_SUBMIT || typed_args == NULL ||
      typed_args->birthday == NULL ||
      strcmp(typed_args->birthday, "20080102") != 0 ||
      typed_args->public_birth != 0U || dcc_ctx_user_data(ctx) != state) {
    state->bad = 1U;
  }
}
...
static void on_typed_button(dcc_ctx_t *ctx, void *args, void *user_data) {
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_BUTTON_CLICK || typed_args == NULL ||
      typed_args->action == NULL ||
      strcmp(typed_args->action, "confirm") != 0 ||
      typed_args->component_type != 2U || dcc_ctx_user_data(ctx) != state) {
    state->bad = 1U;
  }
}
...
static void on_typed_select(dcc_ctx_t *ctx, void *args, void *user_data) {
  ...
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_SELECT_CLICK || typed_args == NULL ||
      typed_args->choice == NULL || strcmp(typed_args->choice, "alpha") != 0 ||
      typed_args->values == NULL || typed_args->value_count != 2U ||
...
static void on_error(dcc_app_t *app, const dcc_error_t *error,
                     void *user_data) {
  app_smoke_state_t *state = (app_smoke_state_t *)user_data;
  state->error_count++;
  if (app == NULL || error == NULL || error->size != sizeof(*error) ||
      error->version != DCC_ERROR_VERSION ||
      error->origin != DCC_ERROR_HANDLER || error->status != DCC_ERR_STATE ||
...
static dcc_status_t app_smoke_module_setup(dcc_app_t *app, void *user_data) {
  ...
  if (dcc_app_state(app) != state ||
      DCC_APP_STATE(app_smoke_state_t, app) != state) {
    state->bad = 1U;
    return DCC_ERR_STATE;
  }
...
static void on_session(dcc_ctx_t *ctx, void *user_data) {
  ...
  const dcc_component_session_result_t *result =
      dcc_ctx_component_session_result(ctx);
  if (dcc_ctx_event_type(ctx) != DCC_EVENT_BUTTON_CLICK || result == NULL ||
      dcc_ctx_component_session_status(ctx) !=
          DCC_COMPONENT_SESSION_VERIFY_OK ||
      ...
      dcc_ctx_custom_id(ctx) == NULL || dcc_ctx_custom_id(ctx)[0] == '\0') {
    state->bad = 1U;
  }
}

static dcc_status_t dispatch_interaction(dcc_app_t *app, dcc_event_type_t type,
                                         const dcc_interaction_t *interaction) {
  dcc_event_t event;
  memset(&event, 0, sizeof(event));
  event.type = type;
  event.data.interaction = *interaction;
  if (type == DCC_EVENT_BUTTON_CLICK || type == DCC_EVENT_SELECT_CLICK ||
      type == DCC_EVENT_FORM_SUBMIT)
    event.data.interaction.id = (dcc_snowflake_t)(uintptr_t)interaction;
  if (event.data.interaction.application_id == 0U)
    event.data.interaction.application_id = 200U;
  if (event.data.interaction.token == NULL)
    event.data.interaction.token =
        interaction->custom_id != NULL
            ? interaction->custom_id
            : (interaction->name != NULL ? interaction->name
                                         : "app-smoke-token");
  return dcc_event_bus_dispatch(&dcc_app_client(app)->events,
                                dcc_app_client(app), &event);
}
...
static int app_smoke_check_dotenv(void) {
  ...
  if (dcc_app_load_env_file(NULL, 1U) != DCC_ERR_INVALID_ARG ||
      dcc_app_load_env_file("dcc_app_smoke_missing.env", 1U) !=
          DCC_ERR_NOT_FOUND) {
    return 0;
  }
  if (!app_smoke_write_file(first_path,
  ...
  if (!app_smoke_write_file(
          app_path, "DCC_APP_SMOKE_TOKEN=token\n"
  ...
  dcc_app_env_binding_t intents_binding = DCC_CONFIG_INTENTS_DEFAULT(
      "DCC_APP_SMOKE_INTENTS", DCC_INTENTS_DEFAULT, &bound_intents);
  dcc_intents_t expected_intents = DCC_INTENTS_MESSAGES |
                                   DCC_INTENT_MESSAGE_CONTENT |
                                   DCC_INTENT_GUILD_VOICE_STATES;
  ok =
      ok && dcc_app_load_env_file(app_path, 1U) == DCC_OK &&
      dcc_app_env_get_token("DCC_APP_SMOKE_TOKEN", &resolved_token) == DCC_OK &&
      resolved_token != NULL && strcmp(resolved_token, "token") == 0 &&
      DCC_ENV_TOKEN_NAMED("DCC_APP_SMOKE_TOKEN", &resolved_sugar_token) ==
          DCC_OK &&
      resolved_sugar_token != NULL &&
      strcmp(resolved_sugar_token, "token") == 0 &&
      dcc_app_env_get_u32("DCC_APP_SMOKE_U32", &direct_u32) == DCC_OK &&
      direct_u32 == 42U &&
      DCC_ENV_U32_OR("DCC_APP_SMOKE_MISSING_U32", 55U, &direct_u32_or) ==
          DCC_OK &&
      direct_u32_or == 55U &&
      DCC_ENV_U32_RANGE("DCC_APP_SMOKE_U32", 1U, 100U, &range_u32) == DCC_OK &&
      range_u32 == 42U &&
      DCC_ENV_U32_RANGE_OR("DCC_APP_SMOKE_MISSING_U32_RANGE", 77U, 1U, 100U,
                           &missing_range_u32) == DCC_OK &&
      ...
      DCC_ENV_INTENTS_OR("DCC_APP_SMOKE_MISSING_INTENTS", DCC_INTENT_GUILDS,
                         &missing_intents) == DCC_OK &&
      missing_intents == DCC_INTENT_GUILDS &&
      DCC_ENV_INTENTS("DCC_APP_SMOKE_INTENTS_BAD", &direct_intents) ==
          DCC_ERR_INVALID_ARG &&
      dcc_app_env_bind(&intents_binding, 1U) == DCC_OK &&
      bound_intents == expected_intents &&
      dcc_app_env_get_u32_range("DCC_APP_SMOKE_U32_BAD", 1U, 100U,
                                &range_u32) == DCC_ERR_INVALID_ARG &&
...
int main(void) {
  if (dcc_app_run_with_signals(NULL) != DCC_ERR_INVALID_ARG ||
      ...
      dcc_app_run_from_env_defined_with_signals(NULL, NULL) !=
          DCC_ERR_INVALID_ARG ||
      dcc_app_run_dotenv_defined_with_signals(NULL, NULL) !=
          DCC_ERR_INVALID_ARG) {
  ...
  if (dcc_app_store(NULL) != NULL || dcc_app_store(app) != NULL ||
      dcc_app_store_open_file(NULL, runtime_store_path) !=
          DCC_ERR_INVALID_ARG ||
  ...
  if (DCC_STORE_GET_U64(DCC_APP_STORE(app), "app.store.count", &store_count) !=
          DCC_OK ||
      store_count != 42U) {
  ...
  dcc_guild_t guild = {.id = 300U, .name = "App Smoke Guild"};
  dcc_channel_t command_channel = {.id = 400U,
                                   .guild_id = 300U,
                                   .type = DCC_CHANNEL_TEXT,
                                   .name = "commands"};
  dcc_channel_t nsfw_channel = {
      .id = 401U, .guild_id = 300U, .type = DCC_CHANNEL_TEXT, .nsfw = 1U};
  dcc_channel_t sfw_channel = {
      .id = 402U, .guild_id = 300U, .type = DCC_CHANNEL_TEXT, .nsfw = 0U};
  dcc_channel_t voice_channel = {
      .id = 403U, .guild_id = 300U, .type = DCC_CHANNEL_VOICE};
  dcc_snowflake_t member_role_ids[] = {910U, 911U};
  dcc_member_t member = {.guild_id = 300U,
                         .user = {.id = 803U},
                         .role_ids = member_role_ids,
                         .role_ids_count = DCC_ARRAY_LEN(member_role_ids)};
  dcc_user_t user = {.id = 803U, .username = "author"};
  ...
  status = DCC_INFER_GUILD_FROM_CHANNEL(app, command_channel.id,
                                        app_smoke_infer_guild_cb, &infer_state);
  if (status != DCC_OK || infer_state.called != 1U ||
  ...
  dcc_app_module_t module = DCC_APP_MODULE_WITH_CLEANUP(
      "smoke.module", app_smoke_module_setup, &state, app_smoke_module_cleanup);
  ...
  status = dcc_component_session_options_set_secret(&session_options, secret,
                                                    sizeof(secret) - 1U);
  if (status == DCC_OK) {
    status = dcc_component_session_options_set_ids(&session_options, "appsess",
                                                   "nonce");
  }
  if (status == DCC_OK) {
    status =
        dcc_component_session_options_set_ttl(&session_options, 1000U, 10000U);
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK && (dcc_app_sync_commands_on_ready(app, &ready_sync) !=
                               DCC_ERR_INVALID_ARG ||
                           dcc_app_sync_commands_on_ready(NULL, &ready_sync) !=
                               DCC_ERR_INVALID_ARG)) {
  ...
  dcc_app_extension_slash_command_t extension_commands[] = {DCC_EXTENSION_SLASH(
      "exthello", "Extension hello", on_extension_slash, &state)};
  dcc_app_extension_component_route_t extension_components[] = {
      DCC_EXTENSION_BUTTON_POLICY(
          "extension:open", on_extension_button, &state,
          DCC_ROUTE_POLICY(
              DCC_ROUTE_MIDDLEWARE(
                  DCC_EXTENSION_MIDDLEWARE(policy_middleware, &state)),
              DCC_ROUTE_GUILD_ONLY(), DCC_ROUTE_OWNER(803U),
  ...
  dcc_application_command_builder_t typed_autocomplete_command =
      DCC_SLASH_COMMAND_WITH_OPTIONS(
          "typed_search", "Typed autocomplete command",
          DCC_COMMAND_STRING_OPTION_AUTOCOMPLETE("name", "Name", 1U),
          DCC_COMMAND_INTEGER_OPTION_RANGE("limit", "Limit", 0U, 1, 25));
  dcc_command_route_t typed_slash = DCC_TYPED_SLASH(
      &typed_command, app_smoke_typed_slash_args_t, on_typed_slash, &state,
      DCC_CTX_OPTION_FIELD_BINDINGS(
          DCC_CTX_OPTION_FIELD_REQUIRED_STRING(app_smoke_typed_slash_args_t,
                                               name, "name"),
          DCC_CTX_OPTION_FIELD_INT(app_smoke_typed_slash_args_t, limit, "limit",
                                   10)),
      DCC_CTX_FIELD_VALIDATORS(
          DCC_VALIDATE_STRING_LENGTH(app_smoke_typed_slash_args_t, name, "name",
                                     1U, 32U),
          DCC_VALIDATE_INT_RANGE(app_smoke_typed_slash_args_t, limit, "limit",
                                 1, 25)));
  dcc_autocomplete_route_t typed_autocomplete = DCC_TYPED_AUTOCOMPLETE_BUILDER(
      &typed_autocomplete_command, app_smoke_typed_slash_args_t,
      on_typed_autocomplete, &state,
      DCC_CTX_OPTION_FIELD_BINDINGS(
          DCC_CTX_OPTION_FIELD_REQUIRED_STRING(app_smoke_typed_slash_args_t,
                                               name, "name"),
          DCC_CTX_OPTION_FIELD_INT(app_smoke_typed_slash_args_t, limit, "limit",
  ...
  dcc_application_command_builder_t typed_subcommand_command =
      DCC_SLASH_COMMAND_WITH_OPTIONS(
          "typed_admin", "Typed admin command",
          DCC_COMMAND_SUBCOMMAND_GROUP(
              "moderation", "Moderation",
              DCC_COMMAND_SUBCOMMAND(
                  "ban", "Ban a member",
                  DCC_COMMAND_STRING_OPTION_AUTOCOMPLETE("name", "Name", 1U),
                  DCC_COMMAND_INTEGER_OPTION_RANGE("limit", "Limit", 0U, 1,
                                                   25))));
  dcc_subcommand_route_t typed_subcommand = DCC_TYPED_SUBCOMMAND_BUILDER(
      &typed_subcommand_command, "moderation/ban", app_smoke_typed_slash_args_t,
      on_typed_subcommand, &state,
      DCC_CTX_OPTION_FIELD_BINDINGS(
          DCC_CTX_OPTION_FIELD_REQUIRED_STRING(app_smoke_typed_slash_args_t,
                                               name, "name"),
          DCC_CTX_OPTION_FIELD_INT(app_smoke_typed_slash_args_t, limit, "limit",
  ...
  dcc_autocomplete_route_t typed_subcommand_autocomplete =
      DCC_TYPED_SUBCOMMAND_AUTOCOMPLETE_BUILDER(
          &typed_subcommand_command, "moderation/ban",
          app_smoke_typed_slash_args_t, on_typed_subcommand_autocomplete,
          &state,
          DCC_CTX_OPTION_FIELD_BINDINGS(
              DCC_CTX_OPTION_FIELD_REQUIRED_STRING(app_smoke_typed_slash_args_t,
                                                   name, "name"),
              DCC_CTX_OPTION_FIELD_INT(app_smoke_typed_slash_args_t, limit,
                                       "limit", 10)),
          DCC_CTX_FIELD_VALIDATORS(
              DCC_VALIDATE_STRING_LENGTH(app_smoke_typed_slash_args_t, name,
                                         "name", 1U, 32U),
              DCC_VALIDATE_INT_RANGE(app_smoke_typed_slash_args_t, limit,
                                     "limit", 1, 25)));
  dcc_modal_route_t typed_modal = DCC_TYPED_MODAL(
      "typed.birthday", app_smoke_typed_modal_args_t, on_typed_modal, &state,
      DCC_CTX_FORM_FIELD_BINDINGS(
          DCC_CTX_FORM_FIELD_REQUIRED_STRING(app_smoke_typed_modal_args_t,
                                             birthday, "birthday"),
          DCC_CTX_FORM_FIELD_BOOL(app_smoke_typed_modal_args_t, public_birth,
                                  "public", 1U)),
      DCC_CTX_FIELD_VALIDATORS(DCC_VALIDATE_DATE_YYYYMMDD(
          app_smoke_typed_modal_args_t, birthday, "birthday")));
  dcc_button_route_t typed_button = DCC_TYPED_BUTTON_PREFIX(
      "typed.button:", app_smoke_typed_button_args_t, on_typed_button, &state,
      DCC_CTX_COMPONENT_FIELD_BINDINGS(
          DCC_CTX_COMPONENT_FIELD_REQUIRED_CUSTOM_ID_SUFFIX(
              app_smoke_typed_button_args_t, action, "typed.button:"),
          DCC_CTX_COMPONENT_FIELD_TYPE(app_smoke_typed_button_args_t,
                                       component_type)),
      DCC_CTX_FIELD_VALIDATORS(DCC_VALIDATE_STRING_REQUIRED(
          app_smoke_typed_button_args_t, action, "action")));
  dcc_select_route_t typed_select = DCC_TYPED_SELECT(
      "typed.select", app_smoke_typed_select_args_t, on_typed_select, &state,
      DCC_CTX_COMPONENT_FIELD_BINDINGS(
          DCC_CTX_COMPONENT_FIELD_REQUIRED_FIRST_VALUE(
              app_smoke_typed_select_args_t, choice),
          DCC_CTX_COMPONENT_FIELD_REQUIRED_VALUES(app_smoke_typed_select_args_t,
                                                  values, value_count)),
      DCC_CTX_FIELD_VALIDATORS(DCC_VALIDATE_STRING_REQUIRED(
          app_smoke_typed_select_args_t, choice, "choice")));
  dcc_application_command_builder_t defined_command =
      DCC_SLASH_COMMAND_WITH_OPTIONS(
          "defined", "Defined slash command",
          DCC_COMMAND_STRING_OPTION("name", "Name", 1U));
  dcc_command_route_t defined_slash = DCC_TYPED_SLASH(
      &defined_command, app_smoke_typed_slash_args_t, on_typed_slash, &state,
      DCC_CTX_OPTION_FIELD_BINDINGS(DCC_CTX_OPTION_FIELD_REQUIRED_STRING(
          app_smoke_typed_slash_args_t, name, "name")),
      DCC_CTX_FIELD_VALIDATORS(DCC_VALIDATE_STRING_REQUIRED(
          app_smoke_typed_slash_args_t, name, "name")));
  dcc_subcommand_route_t defined_subcommand = DCC_TYPED_SUBCOMMAND(
      "defined", "admin/reload", app_smoke_typed_slash_args_t,
      on_typed_subcommand, &state, DCC_CTX_NO_OPTION_FIELD_BINDINGS(),
      DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_autocomplete_route_t defined_autocomplete = DCC_TYPED_AUTOCOMPLETE(
      "defined", app_smoke_typed_slash_args_t, on_typed_autocomplete, &state,
      DCC_CTX_NO_OPTION_FIELD_BINDINGS(), DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_modal_route_t defined_modal = DCC_TYPED_MODAL(
      "defined.modal", app_smoke_typed_modal_args_t, on_typed_modal, &state,
      DCC_CTX_FORM_FIELD_BINDINGS(DCC_CTX_FORM_FIELD_REQUIRED_STRING(
          app_smoke_typed_modal_args_t, birthday, "birthday")),
      DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_button_route_t defined_button = DCC_TYPED_BUTTON_PREFIX(
      "defined.button:", app_smoke_typed_button_args_t, on_typed_button, &state,
      DCC_CTX_COMPONENT_FIELD_BINDINGS(
          DCC_CTX_COMPONENT_FIELD_REQUIRED_CUSTOM_ID_SUFFIX(
              app_smoke_typed_button_args_t, action, "defined.button:")),
      DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_select_route_t defined_select = DCC_TYPED_SELECT(
      "defined.select", app_smoke_typed_select_args_t, on_typed_select, &state,
      DCC_CTX_COMPONENT_FIELD_BINDINGS(
          DCC_CTX_COMPONENT_FIELD_REQUIRED_FIRST_VALUE(
              app_smoke_typed_select_args_t, choice)),
      DCC_CTX_NO_FIELD_VALIDATORS());
  dcc_app_definition_t definition =
      DCC_APP("smoke.definition", DCC_APP_TYPED_SLASHES(defined_slash),
              ...
              DCC_APP_EVENTS(DCC_ON_EVENT_DATA(DCC_EVENT_RESUMED,
                                               on_generic_event, &state)),
  ...
  if (status == DCC_OK &&
      (dcc_app_extension_register(NULL, &extension) != DCC_ERR_INVALID_ARG ||
       ...
       dcc_app_extension_register(app, &(dcc_app_extension_t){0}) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK &&
      (dcc_app_slash_typed(NULL, &typed_slash) != DCC_ERR_INVALID_ARG ||
       ...
       dcc_app_subcommand_typed(NULL, &typed_subcommand) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_subcommand_typed(app, NULL) != DCC_ERR_INVALID_ARG ||
       dcc_app_autocomplete_typed(NULL, &typed_autocomplete) !=
           DCC_ERR_INVALID_ARG ||
  ...
  if (status == DCC_OK) {
    status = dcc_app_subcommand(app, "admin", "moderation/ban", on_subcommand,
                                &state);
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_button_prefix_route(app, "page:", on_button, &state,
                                         &page_route);
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_route_use_required_permissions(
        app, page_route, DCC_PERMISSION_MANAGE_MESSAGES);
  }
  dcc_app_route_id_t cooldown_route = DCC_APP_ROUTE_INVALID;
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "cooldown:test", on_cooldown_button,
                                  &state, &cooldown_route);
  }
  dcc_app_cooldown_options_t cooldown = DCC_COOLDOWN_USER(1U, 60000U);
  if (status == DCC_OK &&
      (dcc_app_use_cooldown(NULL, &cooldown) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_cooldown(NULL, cooldown_route, &cooldown) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_cooldown(app, DCC_APP_ROUTE_INVALID, &cooldown) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_cooldown(app, cooldown_route, NULL) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "check:ok", on_check_button, &state,
                                  &check_route);
  }
  dcc_app_route_id_t rejected_check_route = DCC_APP_ROUTE_INVALID;
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "check:deny", on_rejected_check_button,
                                  &state, &rejected_check_route);
  }
  dcc_app_check_t route_check = DCC_CHECK(allow_check, &state);
  dcc_app_check_t rejected_check =
      DCC_CHECK_MESSAGE(reject_check, &state, "Unavailable",
                        "This check rejected the interaction.");
  if (status == DCC_OK &&
      (dcc_app_use_check(NULL, &route_check) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_check(NULL, check_route, &route_check) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_check(app, DCC_APP_ROUTE_INVALID, &route_check) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_check(app, check_route, NULL) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status =
        dcc_app_route_use_check(app, rejected_check_route, &rejected_check);
  }
  dcc_app_route_id_t dm_only_route = DCC_APP_ROUTE_INVALID;
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "dm:only", on_dm_only_button, &state,
                                  &dm_only_route);
  }
  if (status == DCC_OK &&
      (dcc_app_use_dm_only(NULL) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_dm_only(NULL, dm_only_route) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_dm_only(app, DCC_APP_ROUTE_INVALID) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "nsfw:ok", on_nsfw_button, &state,
                                  &nsfw_route);
  }
  dcc_app_route_id_t nsfw_rejected_route = DCC_APP_ROUTE_INVALID;
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "nsfw:deny", on_nsfw_rejected_button,
                                  &state, &nsfw_rejected_route);
  }
  if (status == DCC_OK &&
      (dcc_app_use_nsfw_only(NULL) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_nsfw_only(NULL, nsfw_route) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_nsfw_only(app, DCC_APP_ROUTE_INVALID) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "channel:text", on_channel_type_button,
                                  &state, &channel_type_route);
  }
  dcc_app_route_id_t channel_type_rejected_route = DCC_APP_ROUTE_INVALID;
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "channel:voice",
                                  on_channel_type_rejected_button, &state,
                                  &channel_type_rejected_route);
  }
  uint32_t text_channel_types[] = {DCC_CHANNEL_TEXT};
  if (status == DCC_OK &&
      (dcc_app_use_channel_types(NULL, text_channel_types,
                                 DCC_ARRAY_LEN(text_channel_types)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_use_channel_types(app, NULL,
                                 DCC_ARRAY_LEN(text_channel_types)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_channel_types(
           NULL, channel_type_route, text_channel_types,
           DCC_ARRAY_LEN(text_channel_types)) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_channel_types(
           app, DCC_APP_ROUTE_INVALID, text_channel_types,
           DCC_ARRAY_LEN(text_channel_types)) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_channel_types(app, channel_type_route, NULL,
                                       DCC_ARRAY_LEN(text_channel_types)) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
    status = dcc_app_route_use_channel_types(app, channel_type_route,
                                             text_channel_types,
                                             DCC_ARRAY_LEN(text_channel_types));
  }
  if (status == DCC_OK) {
    status = dcc_app_route_use_channel_types(app, channel_type_rejected_route,
                                             text_channel_types,
                                             DCC_ARRAY_LEN(text_channel_types));
  }
  dcc_app_route_id_t role_route = DCC_APP_ROUTE_INVALID;
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "role:ok", on_role_button, &state,
                                  &role_route);
  }
  dcc_app_route_id_t role_rejected_route = DCC_APP_ROUTE_INVALID;
  if (status == DCC_OK) {
    status = dcc_app_button_route(app, "role:deny", on_role_rejected_button,
                                  &state, &role_rejected_route);
  }
  dcc_snowflake_t required_roles[] = {910U, 911U};
  dcc_snowflake_t rejected_roles[] = {990U, 991U};
  if (status == DCC_OK &&
      (dcc_app_use_required_roles(NULL, required_roles,
                                  DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_use_required_roles(app, NULL, DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_use_any_role(NULL, required_roles,
                            DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_use_any_role(app, NULL, DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_required_roles(NULL, role_route, required_roles,
                                        DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_required_roles(
           app, DCC_APP_ROUTE_INVALID, required_roles,
           DCC_ARRAY_LEN(required_roles)) != DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_required_roles(app, role_route, NULL,
                                        DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_any_role(NULL, role_route, required_roles,
                                  DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_any_role(app, DCC_APP_ROUTE_INVALID, required_roles,
                                  DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_use_any_role(app, role_route, NULL,
                                  DCC_ARRAY_LEN(required_roles)) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
    status = dcc_app_route_use_required_roles(app, role_route, required_roles,
                                              DCC_ARRAY_LEN(required_roles));
  }
  if (status == DCC_OK) {
    status =
        dcc_app_route_use_any_role(app, role_rejected_route, rejected_roles,
                                   DCC_ARRAY_LEN(rejected_roles));
  }
  dcc_app_view_t view =
  ...
  if (status == DCC_OK &&
      (dcc_app_route_group_init(NULL, app, "bad:", &state) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_group_init(&group, NULL, "bad:", &state) !=
           DCC_ERR_INVALID_ARG ||
       dcc_app_route_group_init(&group, app, "", &state) !=
           DCC_ERR_INVALID_ARG)) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_route_group_button_route(&group, "open", on_group_button,
                                              &group_button_route);
  }
  ...
  if (status == DCC_OK &&
      dcc_app_route_group_use(&empty_group, group_middleware, &state) !=
          DCC_ERR_NOT_FOUND) {
    status = DCC_ERR_STATE;
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_route_group_use_required_permissions(
        &group, DCC_PERMISSION_MANAGE_MESSAGES);
  }
  if (status == DCC_OK) {
  ...
  if (status == DCC_OK) {
    status = dcc_app_autocomplete_route(app, "hello", on_autocomplete, &state,
                                        &autocomplete_route);
  }
  if (status == DCC_OK) {
    status = dcc_app_route_use_required_permissions(
        app, autocomplete_route, DCC_PERMISSION_MANAGE_MESSAGES);
  }
  ...
  if (status == DCC_OK) {
    status =
        dcc_app_component_session(app, &session, &listen, on_session, &state);
  }
  if (status != DCC_OK) {
  ...
  if (dispatch_event(app, &fx.resumed_event) != DCC_OK ||
      ...
      dispatch_interaction(app, DCC_EVENT_SLASH_COMMAND, &fx.typed_slash) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_SLASH_COMMAND,
                           &fx.typed_subcommand) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_SLASH_COMMAND, &fx.extension_slash) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_SLASH_COMMAND, &fx.admin) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_AUTOCOMPLETE, &fx.autocomplete) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_AUTOCOMPLETE,
                           &fx.typed_autocomplete) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_AUTOCOMPLETE,
                           &fx.typed_subcommand_autocomplete) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.denied_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.button) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.cooldown_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.cooldown_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.check_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK,
                           &fx.rejected_check_button) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.dm_only_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.nsfw_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK,
                           &fx.nsfw_rejected_button) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK,
                           &fx.channel_type_button) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK,
                           &fx.channel_type_rejected_button) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.role_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK,
                           &fx.role_rejected_button) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.typed_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_SELECT_CLICK, &fx.typed_select) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.extension_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.view_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.module_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.group_button) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_SELECT_CLICK, &fx.group_select) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK,
                           &fx.group_view_button) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_FORM_SUBMIT, &fx.modal) != DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_FORM_SUBMIT, &fx.typed_modal) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_FORM_SUBMIT, &fx.group_modal) !=
          DCC_OK ||
      dispatch_interaction(app, DCC_EVENT_BUTTON_CLICK, &fx.session_button) !=
          DCC_OK) {
  ...
  int ok =
      state.bad == 0U && state.middleware_count == 31U &&
      state.route_middleware_count == 1U &&
      state.policy_middleware_count == 1U && state.policy_check_count == 1U &&
      state.route_check_count == 1U && state.route_check_reject_count == 1U &&
      state.group_middleware_count == 4U && state.generic_event_count == 2U &&
      state.ready_count == 1U && state.message_count == 1U &&
      state.slash_count == 1U && state.typed_slash_count == 1U &&
      state.typed_subcommand_count == 1U &&
      state.typed_autocomplete_count == 1U &&
      state.typed_subcommand_autocomplete_count == 1U &&
      state.extension_slash_count == 1U && state.extension_button_count == 1U &&
      state.extension_ready_count == 1U &&
      state.extension_message_update_count == 1U &&
      state.subcommand_count == 1U && state.autocomplete_count == 1U &&
      state.button_count == 1U && state.check_button_count == 1U &&
      state.cooldown_button_count == 1U && state.dm_only_button_count == 0U &&
      state.nsfw_button_count == 1U && state.nsfw_reject_button_count == 0U &&
      state.channel_type_button_count == 1U &&
      state.channel_type_reject_button_count == 0U &&
      state.role_button_count == 1U && state.role_reject_button_count == 0U &&
      state.typed_button_count == 1U && state.typed_select_count == 1U &&
      state.view_count == 1U && state.group_button_count == 1U &&
      state.group_select_count == 1U && state.group_modal_count == 1U &&
      state.group_view_count == 1U && state.modal_count == 1U &&
      state.typed_modal_count == 1U && state.session_count == 1U &&
      state.error_count == 6U && state.module_setup_count == 1U &&
      state.module_button_count == 1U && state.module_cleanup_count == 0U &&
  ...
  dcc_status_t sync_status = dcc_app_sync_commands_from_json(
      app, 123U, &registry_options, "[]", 2U, &plan, NULL, NULL);
  ok = ok && sync_status == DCC_OK && plan.create_count == 6U &&
       plan.diff_count == 6U;
  ...
  if (!ok) {
    fprintf(
        stderr,
        "app smoke failed: bad=%u middleware=%u route_middleware=%u "
        "policy_middleware=%u policy_check=%u route_check=%u "
        "route_check_reject=%u group_middleware=%u generic=%u ready=%u "
        "message=%u slash=%u typed_slash=%u typed_subcommand=%u "
        "typed_autocomplete=%u typed_subcommand_autocomplete=%u subcommand=%u "
        "autocomplete=%u button=%u check_button=%u cooldown_button=%u "
        "dm_only_button=%u nsfw_button=%u nsfw_reject_button=%u "
        "channel_type_button=%u channel_type_reject_button=%u role_button=%u "
        "role_reject_button=%u typed_button=%u typed_select=%u view=%u "
        "module=%u modal=%u session=%u error=%u module_setup=%u commands=%zu\n",
        state.bad, state.middleware_count, state.route_middleware_count,
        state.policy_middleware_count, state.policy_check_count,
        state.route_check_count, state.route_check_reject_count,
        state.group_middleware_count, state.generic_event_count,
        state.ready_count, state.message_count, state.slash_count,
        state.typed_slash_count, state.typed_subcommand_count,
        state.typed_autocomplete_count,
        state.typed_subcommand_autocomplete_count, state.subcommand_count,
        state.autocomplete_count, state.button_count, state.check_button_count,
        state.cooldown_button_count, state.dm_only_button_count,
        state.nsfw_button_count, state.nsfw_reject_button_count,
        state.channel_type_button_count, state.channel_type_reject_button_count,
        state.role_button_count, state.role_reject_button_count,
        state.typed_button_count, state.typed_select_count, state.view_count,
        state.module_button_count, state.modal_count, state.session_count,
        state.error_count, state.module_setup_count,
  ...
  if (state.module_cleanup_count != 1U || state.app_state_cleanup_count != 1U) {
    fprintf(stderr,
            ...
            state.module_cleanup_count, state.app_state_cleanup_count);
    return 1;
  }
  return 0;
}
