 ...
 * @param message Null-terminated message text owned by DCC for the duration of
 * the call.
 * @param user_data Opaque pointer from dcc_client_options_t::log_user_data.
 */
typedef void (*dcc_log_fn)(dcc_log_level_t level, const char *message,
                           void *user_data);
 ...
 */
typedef void (*dcc_client_error_fn)(dcc_client_t *client,
                                    const dcc_error_t *error, void *user_data);
...
typedef struct dcc_client_options {
  ...
  uint32_t rest_concurrency; /**< Maximum concurrent REST requests; zero uses
                                the default. */
  ...
  const char *gateway_url;   /**< Optional gateway URL override; copied during
                                creation. */
  size_t cache_max_guilds;   /**< Maximum cached guild count, or zero for no
                                explicit cap. */
  size_t cache_max_channels; /**< Maximum cached channel count, or zero for no
                                explicit cap. */
  size_t cache_max_roles;    /**< Maximum cached role count, or zero for no
                                explicit cap. */
  size_t cache_max_members;  /**< Maximum cached member count, or zero for no
                                explicit cap. */
  size_t cache_max_users;    /**< Maximum cached user count, or zero for no
                                explicit cap. */
  size_t cache_max_messages; /**< Maximum cached message count, or zero for no
                                explicit cap. */
  size_t cache_max_voice_states; /**< Maximum cached voice-state count, or zero
                                    for no explicit cap. */
  uint32_t gateway_max_concurrency; /**< Gateway identify concurrency; zero uses
                                       the default. */
  uint32_t infer_guild_id_from_channel; /**< Non-zero allows channel cache
                                           lookups to infer guild IDs. */
...
typedef struct dcc_runtime_stats {
  ...
  uint64_t events_dispatched; /**< Number of events dispatched through the event
                                 bus. */
} dcc_runtime_stats_t;
...
typedef struct dcc_gateway_info {
  ...
  const char
      *resume_url; /**< Resume URL supplied by Discord, when available. */
  uint32_t recommended_shards; /**< Shard count recommended by Discord. */
  uint32_t max_concurrency; /**< Gateway identify concurrency currently known to
                               the client. */
  uint32_t remaining_identifies; /**< Remaining identifies in the current
                                    session-start window. */
  uint32_t total_identifies;     /**< Total identifies allowed in the current
                                    session-start window. */
  uint64_t reset_after_ms; /**< Milliseconds until the session-start window
                              resets. */
  uint64_t
      reset_at_ms; /**< Runtime timestamp for the next session-start reset. */
  const char *session_id; /**< Current gateway session ID, when known. */
  uint64_t seq;           /**< Last gateway sequence number. */
  uint32_t
      identify_delay_ms;    /**< Delay currently applied before identifying. */
  ...
  uint8_t
      can_resume; /**< Non-zero when the client has enough state to resume. */
  uint8_t
      task_running; /**< Non-zero while the gateway runtime task is running. */
  uint8_t
      reconnect_requested; /**< Non-zero when a reconnect has been requested. */
  uint8_t reconnect_resume; /**< Non-zero when the pending reconnect should
                               attempt resume. */
} dcc_gateway_info_t;
...
typedef struct dcc_gateway_request_guild_members_options {
  ...
  const char *query; /**< Optional member search query. NULL defaults to an
                        empty query. */
  uint32_t limit;    /**< Maximum matching members for query requests. */
  uint8_t presences; /**< Non-zero requests presence data when permitted by
                        intents. */
  const dcc_snowflake_t
      *user_ids;        /**< Optional user IDs to request directly. */
  size_t user_id_count; /**< Number of entries in user_ids. */
  const char *nonce; /**< Optional nonce, up to 32 bytes per Discord's Gateway
                        contract. */
...
typedef struct dcc_gateway_presence_update {
  size_t size; /**< Size of this structure in bytes. */
  const char *
      status; /**< Presence status: online, idle, dnd, invisible, or offline. */
 ...
 */
DCC_API dcc_status_t dcc_client_create(const dcc_client_options_t *options,
                                       dcc_client_t **out);
 ...
 */
DCC_API dcc_status_t dcc_client_on_error(dcc_client_t *client,
 ...
 */
DCC_API dcc_status_t dcc_client_gateway_reconnect(dcc_client_t *client,
                                                  uint8_t resume);

/**
 * @brief Sends a gateway voice-state update for the current user.
 */
DCC_API dcc_status_t dcc_client_update_voice_state(dcc_client_t *client,
...
DCC_API dcc_status_t dcc_client_request_soundboard_sounds(
    dcc_client_t *client, const dcc_snowflake_t *guild_ids,
    size_t guild_id_count);

/**
 * @brief Sends Gateway Request Channel Info (opcode 43).
 */
DCC_API dcc_status_t dcc_client_request_channel_info(dcc_client_t *client,
                                                     dcc_snowflake_t guild_id,
...
DCC_API dcc_status_t dcc_client_update_presence(
    dcc_client_t *client, const dcc_gateway_presence_update_t *presence);
 ...
 */
DCC_API dcc_status_t dcc_client_update_presence_raw(dcc_client_t *client,
...
DCC_API dcc_status_t dcc_client_infer_guild_id_from_channel(
    const dcc_client_t *client, dcc_snowflake_t channel_id,
 ...
 */
DCC_API dcc_status_t dcc_client_runtime_stats(dcc_client_t *client,
                                              dcc_runtime_stats_t *out);
 ...
 * The caller must initialize out->size before calling.
 */
DCC_API dcc_status_t dcc_client_gateway_info(const dcc_client_t *client,
                                             dcc_gateway_info_t *out);

/**
 * @brief Returns the cached DM channel ID for a user, or zero when unknown.
 */
DCC_API dcc_snowflake_t dcc_client_get_dm_channel(const dcc_client_t *client,
                                                  dcc_snowflake_t user_id);

/**
 * @brief Stores or updates a user-to-DM-channel mapping in the client cache.
 */
DCC_API dcc_status_t dcc_client_set_dm_channel(dcc_client_t *client,
                                               dcc_snowflake_t user_id,
                                               dcc_snowflake_t channel_id);
