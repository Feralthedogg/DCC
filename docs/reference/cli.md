...
The generated app definition uses `DCC_DEV_BOT(...)`, so it reconciles
...
hand-written app/store plumbing. Routes are declared with the sugar-first `DCC_DEV_BOT(...)`,
...
during development because `DCC_DEV_BOT(...)` runs the same reconciliation
