...
example `dcc-1.1.1-macos-aarch64.tar.gz`.
...
```sh
curl -fsSL https://github.com/Feralthedogg/DCC/releases/download/v1.1.1/install.sh |
  sh -s -- --version 1.1.1 \
    --base-url "https://github.com/Feralthedogg/DCC/releases/download/v1.1.1" \
    --prefix "$HOME/.local"
```
...
```powershell
Invoke-WebRequest "https://github.com/Feralthedogg/DCC/releases/download/v1.1.1/install.ps1" -OutFile install.ps1
.\install.ps1 -Version 1.1.1 -BaseUrl "https://github.com/Feralthedogg/DCC/releases/download/v1.1.1" -Prefix "$env:LOCALAPPDATA\DCC"
```
