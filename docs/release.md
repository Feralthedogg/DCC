...
example `dcc-1.2.0-macos-aarch64.tar.gz`.
...
```sh
curl -fsSL https://github.com/Feralthedogg/DCC/releases/download/v1.2.0/install.sh |
  sh -s -- --version 1.2.0 \
    --base-url "https://github.com/Feralthedogg/DCC/releases/download/v1.2.0" \
    --prefix "$HOME/.local"
```
...
```powershell
Invoke-WebRequest "https://github.com/Feralthedogg/DCC/releases/download/v1.2.0/install.ps1" -OutFile install.ps1
.\install.ps1 -Version 1.2.0 -BaseUrl "https://github.com/Feralthedogg/DCC/releases/download/v1.2.0" -Prefix "$env:LOCALAPPDATA\DCC"
```
