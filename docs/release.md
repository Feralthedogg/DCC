...
example `dcc-1.3.2-macos-aarch64.tar.gz`.
...
```sh
curl -fsSL https://github.com/Feralthedogg/DCC/releases/download/v1.3.2/install.sh |
  sh -s -- --version 1.3.2 \
    --base-url "https://github.com/Feralthedogg/DCC/releases/download/v1.3.2" \
    --prefix "$HOME/.local"
```
...
```powershell
Invoke-WebRequest "https://github.com/Feralthedogg/DCC/releases/download/v1.3.2/install.ps1" -OutFile install.ps1
.\install.ps1 -Version 1.3.2 -BaseUrl "https://github.com/Feralthedogg/DCC/releases/download/v1.3.2" -Prefix "$env:LOCALAPPDATA\DCC"
```
