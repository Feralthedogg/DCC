...
example `dcc-1.3.5-macos-aarch64.tar.gz`.
...
```sh
curl -fsSL https://github.com/Feralthedogg/DCC/releases/download/v1.3.5/install.sh |
  sh -s -- --version 1.3.5 \
    --base-url "https://github.com/Feralthedogg/DCC/releases/download/v1.3.5" \
    --prefix "$HOME/.local"
```
...
```powershell
Invoke-WebRequest "https://github.com/Feralthedogg/DCC/releases/download/v1.3.5/install.ps1" -OutFile install.ps1
.\install.ps1 -Version 1.3.5 -BaseUrl "https://github.com/Feralthedogg/DCC/releases/download/v1.3.5" -Prefix "$env:LOCALAPPDATA\DCC"
```
